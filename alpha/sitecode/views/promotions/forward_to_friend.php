<html>
<head>
  <title>Send forward to Friend</title>
...
<script type="text/javascript">
  ...
  function submitFrm(){
    ...
    <?php }else{ ?>
      url="<?php echo base_url() ?>send_forward/index/<?php echo $campaign_id; ?>/<?php echo $subscriber_id; ?>/<?php echo $forward_friend_id; ?>";
    <?php } ?>
    jQuery.ajax({
      url:url,
      type:"POST",
...
<!--[body]-->
  <section id="send-to-friend-main-container" role="main" class="main-container content-page">
      ...
      <div id="send-to-friend">
        ...
        <?php
          if($is_autoresponder==1){
            ...
            echo "<form action='".base_url()."send_forward/forwardnow/{$campaign_id}/{$subscriber_id}/{$forward_friend_id}' id='frmForwardToFriend' name= 'frmForwardToFriend'  method='post' >";
          }
        ?>
    ...
    <?php
	//echo '<a href="'.site_url("/").'" class="send-to-friend-logo"> <img src="'. $this->config->item('locker').'images/powered-by-logo-blue.png" alt="logo" title="logo" border="0"></a>';
	
	
	// echo '<div class="footlink">Powered by <a href="http://www.'.SYSTEM_DOMAIN_NAME.'" target="_blank">BoldInbox</a></div>';	
