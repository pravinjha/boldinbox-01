<link rel="stylesheet" href="https://www.boldinbox.com/alpha/locker/themes/march2020/styles/dp.css" />
<script language = "javascript" type="text/javascript" src="https://www.boldinbox.com/alpha/locker/themes/march2020/scripts/dp.js"></script> 
	
<script type="text/javascript">
function send_campaign_now(){
  ...
  $('#messageBox').find('.send_mail').on('click',function(){
    parent.submitFrm();
    $.modalBox.close();
  });

  $('#messageBox').find('.cancel_mail').on('click',function(){
    $.modalBox.close();
  });
  jQuery('.subscriptions_check').on('click',function(){
  ...
  */
  ...
  jQuery("#save_exit").on('click',function(){
    $('#save_email').val('1');
    document.form_campaign_send.submit();
  });
  jQuery(".schedule_email").on('click',function(){
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="section-inner">
					<?php echo form_open('campaign_email_setting/index/'.$campaign_data['encrypted_cid'], array('id' => 'form_campaign_send','name'=>'form_campaign_send','class'=>"form-website")); ?>
					...
					<div class="row">						
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class = 'row'>
									<div class="col-lg-4">				
										<button type = 'button' name = 'preview' value = '<< go back to preview' class = 'blue rectangleSm' onclick="javascript:window.location.href='<?php echo site_url('preview/index/'.$campaign_data['encrypted_cid']);?>';"><< Go Back To Preview</button>
									</div>			
									<div class="col-lg-4 align-center">
										<button type='button' name='edit' value='edit campaign' class='blue rectangleSm' onclick="javascript:window.location.href='<?php echo $strEditorURL;?>'" />Edit Campaign</button>
									</div>
									<div class="col-lg-4">
										<button type = 'button' name = 'save_exit' id="save_exit" value = 'save campaign & exit >>' class = 'blue rectangleSm float' />Save Campaign & Exit >></button>
									</div>
								</div>
							</div>	
						</div>
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Email Preheader [Optional]:</div>													
										<textarea name="preheader" id="preheader" class = 'form-control' placeholder = 'Example: Wishing you a safe and happy holiday season!'><?php echo $campaign_data['camapign']['preheader'];?></textarea>	
									</div>
									<div class="col-lg-6 tipBox">
										<div class = 'formLabelTips'>What is an email pre-header?</div>
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Subject Personalization [Optional]:</div>
										<select name="personalization" id="personalization" class = 'form-control' onchange="javascript:addToken(this.value); this.value = '';">
										...
										<div class = 'formLabel'>Email Subject Line:</div>
										<input type = 'text' name = 'email_subject' id = 'email_subject' class = 'form-control' placeholder = 'Example: Great Festive Offer Inside' value="<?php echo $campaign_data['camapign']['email_subject'];?>" maxlength=250 />
									
										<div class = 'formLabel'>From Name:</div>						
										<input type = 'text' name = 'email_from' id='email_from' class = 'form-control' placeholder = 'Example: My Online Shop' maxlength='250' value="<?php echo $campaign_data['camapign']['sender_name'] != "" ? $campaign_data['camapign']['sender_name'] : $campaign_data['email_from'] ; ?>" />
										...
										<div class = 'formLabel'>
											<div class = 'floatLeft'><a href="javascript:void(0);" onclick="javascript:add_another_from_eml();" class = 'font5 font400' id="btn_add_other_eml" class="edit-interval"  data-toggle='modal' data-target='#messageBox'>Add New From Email</a></div>
											<div class = 'floatRight'><a href="javascript:void(0);" title = "Reload 'From Email' List" onclick="javascript:updateFromEmailDpd();"><i class = 'fa fa-refresh'></i></a></div>
										</div>
										<?php } ?>			
										...
										<div class = 'formLabel'>&nbsp;</div>						
										<div class = 'formLabel'>Reply To Email:</div>						
										<select name="reply_to_email" id="reply_to_email" class = 'form-control'>
											<?php
												foreach($campaign_data['email_id'] as $fromEml){
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Select Contact Lists:</div>
											<?php
												  $i=0;
												  ...
												  foreach($subscription_data['subscriptions'] as $subscription){
													if(isset($campaign_data['camapign']['subscription_list']) && in_array($subscription['subscription_id'],$campaign_data['camapign']['subscription_list']))
													  ...
													  $checked=false;
													echo '<div>';
													echo form_checkbox(array('name'=>'subscriptions[]','id'=>'subscriptions','class'=>'subscriptions_check form-control','value'=>$subscription['subscription_id'],'checked'=>$checked )).' '.ucwords(substr($subscription['subscription_title'],0,25))." (".$subscription['number_of_contacts'].")";
													echo '</div>';
													$i++;
												  }
												  if($i<=0) {
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Advance Tracking:</div>
										<div class="contacts-list advance-tracking">
											<?php if(!$campaign_data['is_ga_enabled']){?>
												<div><input type="checkbox" name="is_ga_enabled" class = 'form-control' id="is_ga_enabled" value="1" checked style="margin-right:12px;" />Track campaign using Google Analytics</div>
											<?php }?>
											
											<?php if(!$campaign_data['is_clicktracking']){?>
												<div><input type="checkbox" name="is_clicktracking" class = 'form-control' id="is_clicktracking" value="1" checked style="margin-right:12px" />Track clicks in the campaign</div>
											<?php }?>
										</div>
									</div>
									<div class="col-lg-6 tipBox">
										<div class = 'formLabelTips'>What is advance tracking?</div>
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Would you like to send a Test Email?</div>
										<?php if($campaign_data['camapign']['test_email_count']>=15){ ?>
											<div class="email_msg info">You have reached the maximum allowed (15) tests for this campaign.</div>
										<?php }else{ ?>
										<textarea name="email_address" id="email_address" class = 'form-control' placeholder = 'Enter email addresses separted by comma. Maximum 15 allowed for one campaign, 3 emails at a time.'></textarea>
										<button type = 'button' name = 'send_test_mail' value = 'SEND TEST MAIL' class = 'blue rectangleSm' onclick="send_test_email();">Send Test Mail</button>
										<?php } ?>
									</div>
									<div class="col-lg-6 tipBox">
										<div class = 'formLabelTips'>What is Test mail and why is it important?</div>
										<div class = 'formTips'>Please follow our Support section to learn more about it. <a href = '#'>Click Here</a></div>
									</div>
								</div>
							</div>
						...
						<div class="col-lg-12">
							<div class="campaign-post">
								<div class="row">
									<div class="col-lg-6">
										<div class = 'formLabel'>Ready to send out your email campaign?</div>
										<button type = 'button' name = 'save_exit' value = 'send campaign now' class = 'blue rectangle send_now' onclick="javascript:send_campaign_now();" data-toggle='modal' data-target='#messageBox'>Send Campaign Now</button>
										<button type = 'button' name = 'save_exit' value = 'send campaign later' class = 'blue rectangle' onclick="scheduleFrm();" data-toggle='modal' data-target='#messageBox'>Send Campaign Later</button>
									</div>
									<div class="col-lg-6 tipBox">
										<div class = 'formLabelTips'>Benefits of Scheduling your future campaigns.</div>
										<div class = 'formTips'>Please follow our Support section to learn more about it. <a href = '#'>Click Here</a></div>
									</div>
								</div>
							</div>
