...
<script type="text/javascript">
	...
	function openIpr(){		
		...
		jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/campaign/ipr/",
			type:"POST",
			data:block_data,
...
<?php $ci =& get_instance(); ?>
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo base_url().'bandook/dashboard_stat/sent_campaign_new';?>">
	<table border="0" cellspacing="0" cellpadding="0" class="tbl_forms">
		...
		<tr>			
			...
			<td nowrap><?php echo form_input(array('name'=>'keyword','id'=>'keyword','maxlength'=>100,'size'=>120 ,'value'=>$contacts_array['keyword'])) ; ?></td>					
			<td colspan="1">Pipeline</td>
			<td nowrap><select name="pipeline"  style="width:150px;">
		...
		<?php
			...
			for($i=0; $i < count($arrVMTAPool); $i++){	
				$j=0;
					...
					if($contacts_array['pipeline'] == $arrVMTAPool[$i][$j])$selIt = 'selected';else $selIt = '';
...
<table class="tbl_listing" width="100%"> 
	...
	?>
	<tr <?php if($email['is_deleted'])echo 'style="background-color:#f0f8ff;"'; ?>>
		<td width="5%"><?php echo $key;?></td>
		<td width="15%">		
		<a target="_blank" href="<?php echo CAMPAIGN_DOMAIN.'c/'.$this->is_authorized->encryptor('encrypt',$key); ?>" <?php if($email['is_deleted']==1) echo "style='color:#ff0000;'"?>><?php echo $email['email_subject']; ?></a>
		<br/>
		<?php if($email['is_dmarc'] < 1){?>
		<b>From:</b> <?php echo $email['sender_name'] . ' ['.$email['sender_email'] .']';?>
		<?php }else{?>
		<font color='green'><b>From:</b> <?php echo $email['sender_name'] . ' ['.$email['dmarc_from_email'] .'@PIPELINE_DOMAIN]';?>		</font>	
		<?php } ?>
		<br/>
		<b>ReplyTo:</b> <?php echo $email['sender_name'] . ' ['.$email['reply_to_email']. ']';?>	
		<br/>
		<b>Sent at:</b>	<?php echo date('l F j, Y \a\t g:i a', strtotime( getGMTToLocalTime($email['email_send_date'], date_default_timezone_get())))?>			
		<br/>
		<b>Pipeline:</b>	<?php echo $email['pipeline']; ?>			
		</td>
		<td width="5%">
			<a href="<?php echo  site_url("bandook/users_manage/users_list/".$email['member_username']); ?>"><?php echo $email['member_username'];  ?></a>
		</td>
		<td width="5%">
			<?php echo $email['total_sent_emails'].' / '.$email['campaign_contacts']; ?>
			<?php if ($email['number_of_contacts'] > 0){
					//echo "<br/>Total:&nbsp;{".$email['campaign_contacts'].'}';
					echo "<br/>Seg:&nbsp;{".$email['number_of_contacts'].'}@'.$email['segment_interval'].' mins';
				}
				echo "<br/>Lists: ".	ltrim($email['lists'], ',');
			?>
		</td>
		<td width="5%"><?php echo $email['total_released_emails']; ?></td>
		<td width="5%"><?php echo $email['total_delivered_emails']; ?></td>
		<td width="5%">			
			<?php echo $email['total_read_emails']+ $email['upvote']."<br/>".'['.$email['total_read_emails'].' + '.$email['upvote'].'] ('.$readPercentage.'%)'; ?>
		</td>
		<td width="10%">
		...
		<td width="10%">
			<a class="fancybox" href="<?php echo  site_url('bandook/campaign/ipr/'.$key);?>">IPR</a>
			<?php if($ci->session->userdata('webmaster_id')=='1'){ ?>	
			&nbsp;|&nbsp;
			<a onclick="return confirm('Are you sure to delete campaign')" href="<?php echo  site_url('bandook/campaign/delete/stat/'.$key);?>">Delete</a>
			<?php }?>
		</td>
	</tr>
<?php } 
