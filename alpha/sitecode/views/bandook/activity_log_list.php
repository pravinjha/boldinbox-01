...
<script>
	...
	function revertImport(mid,importId){
		var x = $('#'+importId).html();
		var ajaximg = '<img border="0" src="<?php echo $this->config->item('locker');?>images-front/ajax_loading.gif?v=6-20-13"/>';
		$('#'+importId).html(x+ajaximg);
		jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/activity_log/undo_import/"+mid+"/"+importId,
				  type:"POST",
...
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo base_url().'bandook/activity_log/display';?>">
	...
	<tr>
		...
		<td width="80%">
			...
			<?php if($activity[0]=="login") {
					echo "<a href='". site_url("bandook/users_manage/users_list/".$activity_log['member_username'])."'>".$activity_log['member_username']."</a> Logged-in (".$activity[1].")";
				}else if($activity_log['activity']=="logout") {
					echo "<a href='". site_url("bandook/users_manage/users_list/".$activity_log['member_username'])."'>".$activity_log['member_username']."</a> Logged-out";
					...
					echo "<a href='". site_url("bandook/users_manage/users_list/".$activity_log['member_username'])."'>".$activity_log['member_username']."</a> ".ucfirst($activity_log['activity'])." (".$activity_log['number_of_contacts'].")";
					...
					if(strpos($activity_log['activity'],"delete")!==False){
						echo ucfirst($activity_log['activity'])."  by "."<a href='". site_url("bandook/users_manage/users_list/".$activity_log['member_username'])."'>".$activity_log['member_username']."</a> ";
					}else{
						$campaign_link= CAMPAIGN_DOMAIN.'c/'.$this->is_authorized->encryptor('encrypt',$activity_log['campaign_id']);
						echo ucfirst($activity_log['activity'])." (<a  href='". $campaign_link."' target='_blank'>".$activity_log['campaign_title']."</a>) by "."<a class='fancybox' href='". site_url("bandook/users_manage/view/".$activity_log['member_id'])."'>".$activity_log['member_username']."</a> ";
					}
				}else if(strpos($activity_log['activity'],"autoresponder_")!==False) {
					$autoresponder_link= CAMPAIGN_DOMAIN.'a/'.$activity_log['campaign_id'];
					echo ucfirst($activity_log['activity'])." (<a  href='". $autoresponder_link."' target='_blank'>".$activity_log['campaign_title']."</a>) by "."<a class='fancybox' href='". site_url("bandook/users_manage/view/".$activity_log['member_id'])."'>".$activity_log['member_username']."</a> ";
				}else{
					echo "<a class='fancybox' href='". site_url("bandook/users_manage/users_list/".$activity_log['member_username'])."'>".$activity_log['member_username']."</a> ".ucfirst($activity_log['activity']);
				}
				if($activity_log['contact_list_type']==1){
					...
					if($activity_log['file_name']!=""){
						...
						echo " <a href='". site_url("bandook/activity_log/export_file/".$activity_log['member_id']."/".$activity_log['file_name'])."'>Download File</a>";
						if($activity_log['is_undone'] == 0){
						//echo "<span id='{$importBatchID}'> | <a href='". site_url("bandook/activity_log/undo_import/{$importBatchID}'>Undo Import</a></span>";
