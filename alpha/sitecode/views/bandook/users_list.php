<script type="text/javascript" src="<?php echo $this->config->item('locker');?>js/fancybox/jquery.mousewheel-3.0.4.pack.js"></script>
<script type="text/javascript" src="<?php echo $this->config->item('locker');?>js/fancybox/jquery.fancybox-1.3.4.pack.js"></script>
<link href="<?php echo $this->config->item('locker');?>js/fancybox/jquery.fancybox-1.3.4.css" rel="stylesheet" type="text/css" />
...
<script type="text/javascript">
	...
	jQuery('.is_authentic').live('click',function(){
		...
		jQuery.ajax({
		  url: "<?php echo base_url() ?>bandook/users_manage/update_user/"+id+"/"+status,
		  type:"POST",
		  data: "mode=authentic",
	...
	jQuery('.is_disclaimer').live('click',function(){
		...
		jQuery.ajax({
		  url: "<?php echo base_url() ?>bandook/users_manage/update_user/"+mid+"/"+status,
		  type:"POST",
	...
	jQuery('.update_urc').live('click',function(){
			...
			jQuery.ajax({
			  url: "<?php echo base_url() ?>bandook/users_manage/update_user/"+mid+"/"+urc_val,
			  type:"POST",
	...
	jQuery('.update_user_note').live('click',function(){
			...
			jQuery.ajax({
			  url: "<?php echo base_url() ?>bandook/users_manage/update_user/"+mid+"/",
			  type:"POST",
	...
	jQuery('.update_segment_size').live('click',function(){
			...
			jQuery.ajax({
			  url: "<?php echo base_url() ?>bandook/users_manage/update_user/"+mid+"/"+segment_size_val,
			  type:"POST",
	...
	jQuery('.attach_seedlist').live('click',function(){
		...
		if ($(this).attr('checked')) status = 1;			
		jQuery.ajax({
		  url: "<?php echo base_url() ?>bandook/users_manage/attach_seedlist/"+id+"/"+status ,
		  type:"POST",
		  success: function(data) {
	...
	jQuery('.is_risky').live('click',function(){
			...
			if ($(this).attr('checked')) status=1;			
			jQuery.ajax({
			  url: "<?php echo base_url() ?>bandook/users_manage/update_risky/"+id+"/"+status,
			  type:"POST",
			  success: function(data) {
	...
	function updateVmta(u, v){
		var vmta =v.value;
		jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/users_manage/update_vmta/"+u+"/"+vmta,
			type:"POST",
			data: "mode=authentic",
			success: function(data) {
			$('#vmta-'+u).text(data);
	...
	jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/users_manage/reanalysis/"+mid,
			type:"POST",
			data: "mode=authentic",
			success: function(data) {			
				$.fancybox(data);
	...
	jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/users_manage/analyseNewContacts/"+mid,
			type:"POST",
			data: "mode=authentic",
			success: function(data) {			
				$.fancybox(data);
	...
	jQuery('a.exp').live('click',function(){	
		...
		if(confirm(" Filtered contacts will be exported to csv. Are you sure to export contacts?")){		
			$.get("<?php echo base_url() ?>bandook/users_manage/export_analysed_contacts/"+thisID,function(filedata){ // AJAX call returns with CSV file data
	...
	jQuery('a.del').live('click',function(){	
		...
		if(confirm(" Filtered contacts will be deleted. Are you sure to delete contacts?")){		
			$.get("<?php echo base_url() ?>bandook/users_manage/delete_analysed_contacts/"+thisID,function(filedata){ // AJAX call returns with CSV file data
				 alert(filedata);       
			});					 
		}
	});	
	jQuery('a.sup').live('click',function(){	
		...
		if(confirm(" Filtered contacts will be suppressed. Are you sure to suppress contacts?")){		
			$.get("<?php echo base_url() ?>bandook/users_manage/suppress_analysed_contacts/"+thisID,function(filedata){ // AJAX call returns with CSV file data
				 alert(filedata);       
			});					 
		}
	});	
	jQuery('.clAttachMsg').live('click',function(){
		...
		$("form#hiddenform").attr("action", "/bandook/campaign/attachMessage/0/"+member_id+'/'+message_id);
...
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo ($mode == "upgrade_users") ?  base_url().'bandook/users_manage/upgrade_users' : base_url().'bandook/users_manage/users_list';?>">
...
?>
 ...
 <div style="text-align: right;"><a href="<?php echo base_url() ?>bandook/users_manage/user_create"><strong>Create New User</strong></a></div>
...
<tr>
	...
	<?php 
		...
		?>
		<br/> Via: <?php echo ($user['site_id'] == 5)?'<font color="green"><b>GoWebby</b></font>':'BIB';?>
		<hr/>
	...
	<td>		
		<a class="fancybox" href="<?php echo  site_url("bandook/users_manage/contact_details/".$user['member_id']); ?>"><?php echo $user['contacts']; ?></a>
		...
		<?php if($user['is_analyzed'])echo "<br/><a class='fancybox' href='".site_url("bandook/users_manage/view_contact_analysis/".$user['member_id'])."'>View analysis</a>";?>
	</td>
...
<tr><td colspan="2"><textarea style="width:450px;color:#ff0000;" name="user_note" id="user_note_<?php echo $user['member_id'];?>"><?php echo $user['campaign_approval_notes'];	?></textarea>
	...
	<td>
	<ul style="line-height:20px;margin:0 0 0 10px;padding:0;">
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url("bandook/users_manage/check_user_for_admin/".$user['member_id']); ?>" target="_blank">View Dashboard</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/users_manage/user_add_package/'.$user['member_id']);?>" >Add invoice</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/users_manage/invoice_list/'.$user['member_id']);?>" >View invoice</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/contacts_segmentation/index/'.$user['member_id']);?>" >Segmentation</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/contacts_segmentation/filter/'.$user['member_id']);?>" >Filter</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/contacts_segmentation/importContacts/'.$user['member_id']);?>" >Add contact</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/contacts_segmentation/addDNM/'.$user['member_id']);?>" >Add DNM</a></li>
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/users_manage/user_edit/'.$user['member_id']);?>" style="color:red;">Edit</a></li>	
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" href="<?php echo  site_url('bandook/users_manage/user_batch/'.$user['member_id']);?>" style="color:red;">Batch Scheduled</a></li>	
		<li style="float:left;width:100px;margin-top:1px;"><a style="border:1px solid #ccc;background:#ebebeb; color:#333;padding:2px 5px;" class="data_validation" user_id="<?php echo $user['member_id'];?>" href="javascript:void(0)" style="color:red;">Validate Contacts</a></li>
	</ul></td>	
</tr>
<?php } } else { ?>
...
<form id="hiddenform" method="POST" action="<?php echo base_url() ?>bandook/users_manage/download">
 ...
 $(".data_validation").click(function(){
    ...
    $.fancybox({'content' : "<div style=\"width:400px;\"><h5 style=\"margin-top:0px;\">Validate Contacts</h5><div class=\"subscription_msg info\" style=\"padding-left:10px\">Are you sure to validate contacts/list?</div><div class=\"btn-group\"><input type=\"submit\" class=\"btn confirm\" content=\"Submit\"  value=\"Save\" userid="+user_id+" id=\"email_validation\"  onclick=\"email_validation("+user_id+");\"><button class=\"btn cancel\" onclick=\"$.fancybox.close();\" value=\"Cancel\" type=\"button\">Cancel</button></div></div>"});
 });
 
 
 function email_validation(user_id){
    var block_data="user_id="+user_id;
    $.ajax({
        url: "<?php echo base_url() ?>bandook/users_manage/email_validation/",
        type:"POST",
        data:block_data,
        success: function(data){
