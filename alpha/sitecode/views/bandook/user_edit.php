...
<?php
	echo "<a  onclick=\"return confirm('Are you sure to delete user ')\" href='".site_url('bandook/users_manage/user_delete/'.$user['member_id'])."' style='padding:0 50px;float:right'>Delete User</a>";
	if(($transaction_count>0)&&(is_null($user['cancel_subscription_date']))) {
		echo "<a  onclick=\"return confirm('Are you sure to cancel subscription ')\" href='".site_url('bandook/users_manage/cancel_subscription/'.$user['member_id'])."' style='float:right'>Cancel Subscription</a>";
	}
?>
</div>
<div id="messages">
...
<?php 
echo form_open('bandook/users_manage/user_edit/'.$user['member_id'], array('id' => 'frmUserEdit'));
