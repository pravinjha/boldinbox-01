...
function saveIt(cid){
	var cval = $('#'+cid).val();
	$.post( 
                  "/bandook/sitesetting_manage/ajaxSaveSetting",
                  { "cname": cid, "cval" : cval },
                  function(data) {
                     //$('#stage').html(data);
                     alert(data);
...
function showQueue(){	
	$.get( "/bandook/sitesetting_manage/ajaxShowQueue",'',
                  function(data) {
                     $('#div_queue').html(data);                     
                  }
               );	
...
function showRunningCron(){	
	$.get( "/bandook/sitesetting_manage/showRunningPS",'',
                  function(data) {
                     $('#div_cron').html(data);                     
                  }
               );	
...
<?php
echo form_open('bandook/sitesetting_manage/cron_setting/', array('id' => 'frmCronSetting'));
...
<fieldset style='width:60%'><legend>Send Campaign Cron:</legend>
	...
	<label style='display:inline-block;width:40%; border:0px solid #ccc;'>Preprocessing:</label> ".form_dropdown('preprocessing_cron',array('1'=>'Working', '0'=>'Completed'),$preprocessing_cron, "id='preprocessing_cron'")." &nbsp; <a href='javascript:void(0);' onclick='javascript:saveIt(\"preprocessing_cron\");' >Update</a><br/><span style='color:red; font-size:9px;'>Campaign-id: ".$preprocessing_cron .'; '.date('F j, Y \a\t g:i a', strtotime( getGMTToLocalTime($preprocessing_start_date, date_default_timezone_get() )))."</span><br/><span style='color:#999; font-size:10px; text-decoration:italics;'>(Default is Active (working). To stop change to \"Stop\")</span>
