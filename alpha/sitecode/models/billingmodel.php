	...
	**/
	function createCustomerProfileRequest(){
		...
		if ($this->billingcim->isSuccessful()){
			...
			$ci->UserModel->update_member_package(array('customer_profile_id'=>$customerProfileId),array('member_id'=>$this->session->userdata('member_id')));
	...
	function createCustomerPaypalProfileTransactionRequest($amount,$package_id, $user_profile_arr, $first_payment = false){
		...
		*/
		...
		if($amount > 5000){
                    ...
                    @mail('pravinjha@gmail.com','ERROR: over charge','BoldInbox tried to charge amount:$'.$amount.' user-id is='.$user_profile_arr['member_id'].' package selected='.$package_id);
                    return false;
                    exit;
		}
                */
	...
	function createCustomerProfileTransactionRequest($amount,$package_id, $user_profile_arr, $first_payment = false){
		...
		*/
		...
		if ($this->billingcim->isSuccessful()){
			...
			@mail('pravinjha@gmail.com','ERROR: Fail-Payment',$str_input_data);
			#################################################################################
