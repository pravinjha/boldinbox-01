...
class Campaign_email_setting extends CI_Controller
{
	...
	**/
	function index($campaign_id=0){		 
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Send Campaigns','contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_email_setting',array('campaign_data'=>$campaign_data,'subscription_data'=>$subscription_data,'messages'=>$messages,'shorten_url'=>$shorten_url,'quota_remaining'=>$quota_remaining,'queued_campaign_count'=>$queued_campaign_count));
