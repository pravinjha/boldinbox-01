...
class User extends CI_Controller
{
	...
	*/
	function register(){
		...
		if($this->input->post('is_submitted')=='save' && $this->form_validation->run()){	
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',$data);
		$this->load->view($this->config->item('theme_folder').'user_register',$data);
		$this->load->view($this->config->item('theme_folder').'footer_outer',$data);
	}
	/**
	...
	**/
	function registration_notification(){		
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',$data);
		$this->load->view($this->config->item('theme_folder').'user_confirmation');
	...
	*/
	...
	function login(){		
		...
		if(!(isset($_POST['action']))){
			$this->load->view($this->config->item('theme_folder').'header_outer',$data);  
			$this->load->view($this->config->item('theme_folder').'user_login',array('user'=>$user_data_array,'messages' =>$messages,'title'=>'Login - Email Marketing'));
			$this->load->view($this->config->item('theme_folder').'footer_outer');  
		}
	...
	*/
	function change_password(){
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Change Password','previous_page_url'=>$previous_page_url,'contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'user/user_change_password',array('messages' =>$messages));
	...
	**/
	function reset_password($random_token=""){
		if($random_token){
			...
			if(is_numeric($user_id)){
				...
				$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'Reset Password','previous_page_url'=>$previous_page_url));
				$this->load->view($this->config->item('theme_folder').'user/user_reset_password',array('token'=>$random_token));
				$this->load->view($this->config->item('theme_folder').'footer_outer');
			}else{
				redirect('/');
			}
	...
	function thanks_msg($invalid_url=""){
		...
		$this->load->view($this->config->item('theme_folder').'promotions/thanks_msg',array('msg'=>$msg));
	}
	/**
	...
	**/
	...
	function user_account_inactive_message(){
		$msg= '<h2><b>Oops... Your account is deactivated. Please contact us at <a href="mailto:'.SYSTEM_EMAIL_FROM.'" style="color:blue">'.SYSTEM_EMAIL_FROM.'</a>.</b></h2>';
		
		$this->load->view($this->config->item('theme_folder').'header_outer');
		$this->load->view($this->config->item('theme_folder').'user/user_message',array('msg'=>$msg));
		$this->load->view($this->config->item('theme_folder').'footer_outer');
	}
	...
	*/
	...
	function verify($key=''){	
		...
		if($rsEml->num_rows() > 0){
			...
			$this->load->view($this->config->item('theme_folder').'header_outer');
			$this->load->view($this->config->item('theme_folder').'user/verify',array('str'=>$key,'new_email'=>$eml));
			$this->load->view($this->config->item('theme_folder').'footer_outer');
		}else{
	...
	function verified(){
		$this->load->view($this->config->item('theme_folder').'header_outer');
		$this->load->view($this->config->item('theme_folder').'user/verified');
		$this->load->view($this->config->item('theme_folder').'footer_outer');
	}
	function confirmed(){
		$this->load->view($this->config->item('theme_folder').'newsletter/accountEmlVerified');
	}
