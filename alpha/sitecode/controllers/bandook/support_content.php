...
class Support_content extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/SupportContent_Model');
		# HTTPS/SSL enabled
		force_ssl();
	...
	function support_content_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/support_content/support_content_list';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Support Content','logo_link'=>$logo_link));
		$this->load->view('bandook/support_content_list',array('contents'=>$content_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function support_content_edit($id=0){
		...
		{	
			...
			if($this->form_validation->run()) {
				...
				redirect('bandook/support_content/support_content_list');
			}
		...
		$this->load->model('bandook/SupportCategory_Model');
		...
		$this->load->view('bandook/header',array('title'=>'Update Support Content','logo_link'=>$logo_link));
		$this->load->view('bandook/support_content_edit',array('content'=>$content_data_array,'categories'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	{		
		...
		$this->load->view('bandook/support_content_create',array('content'=>$content_data_array,'categories'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
