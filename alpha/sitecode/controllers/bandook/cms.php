...
class Cms extends CI_Controller
{
	...
	function page_list($start=0){
		...
		$config['base_url']=base_url().'bandook/cms/page_list';
		...
		$this->load->view('bandook/page_list',array('pages_array'=>$pages_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function page_create(){
		...
		if($this->input->post('action')=='submit')
		{
			...
			$content=str_replace("../../../asset/images", base_url()."locker/images", $this->input->post('content',true)); 
			$input_array = array(
			...
			if($this->form_validation->run()) {
				...
				{	
					...
					redirect('bandook/cms/page_list');
				}
			}
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, template header  and footer view.
		$this->load->view('bandook/header',array('title'=>'Create cms pages','logo_link'=>$logo_link));
		$this->load->view('bandook/page_create',array('page'=>$pages_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	function page_edit($id=0){
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, template header  and footer view.
		$this->load->view('bandook/header',array('title'=>'Edit cms pages','logo_link'=>$logo_link));
		$this->load->view('bandook/page_edit',array('page'=>$page_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
}
/* End of file */
