...
class Users_Manage extends CI_Controller{
	function __construct(){
		...
		$this->load->model('bandook/Croncb_Model');
	...
	function users_list($start=0){
		if(!is_numeric($start)){ 
			...
			$_POST['field_value']= urldecode(trim($start));	
		}
		...
		$config['base_url']=base_url().'bandook/users_manage/users_list';
		...
		$this->load->view('bandook/users_list',array('users'=>$user_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'search'=>$search_array,'packages'=>$packages));
	...
	function user_edit($id){
		...
		$this->UserModel->update_user(array('is_authentic'=>$is_authentic,'authenticated_on' => "$thisDate",'unauthentic_contacts'=>0),array('member_id'=>$member_id));	
			...
			if($this->form_validation->run()) {
				...
				//check email exists
					...
					if($member_package_data['next_payement_date'] != '')	
					$arrUpdatePackage['next_payement_date']=$member_package_data['next_payement_date'];
		...
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>$logo_link));
		$this->load->view('bandook/user_edit',array('user'=>$user_data_array,'messages' =>$messages,'transaction_count'=>$transaction_count,'user_package'=>$user_package, 'referrer'=>$arrReferrerName));
		$this->load->view('bandook/footer');
	}
    ...
    function email_validation() {
        ...
        if (count($fetchData) > 0) {            
            ...
            $inputArray = array('rc_member_id' => $id, 'rc_createddate' => date('Y-m-d h:i:s'), 'rc_updateddate' => date('Y-m-d h:i:s'), 'rc_status' => 0 );
    ...
    function user_batch($id) {
       ...
       $new_filter_query = "SELECT COUNT(subscriber_created_by) AS total_member ,dv_createddate,dv_csv_count,dv_grade,dv_singlecsv_run,dv_csv_name,rc_member_id,dv_id,batch.dv_batch_grade,dv_scheduled FROM red_dv_cron_setup AS csv
        ...
        //Loads header, users listing  and footer view.
        $this->load->view('bandook/header', array('title' => 'Manage Users', 'logo_link' => $logo_link));
        $this->load->view('bandook/user_batch', array('subscriber_data' => $subscriber_data, 'paging_links' => $paging_links, 'messages' => $messages, 'search' => $search_array, 'packages' => $packages));
        $this->load->view('bandook/footer');
    }
    ...
    function user_cron_batch($id,$memberid) {
        ...
        if ($rsContacts->num_rows() > 0) {
            ...
            redirect('bandook/users_manage/user_batch/' . $memberid);
        }else{
            ?>
                <script>
                 alert('Cron already scheduled');
                window.location.href = "<?php echo $base_url(); ?>bandook/users_manage/user_batch/<?php echo $memberid; ?>";
               </script>
		...
		if($this->input->post('action')=='submit'){
			...
			if($this->form_validation->run()) {
				$member_data = array(
					...
					'sign_up_form_status' => 0, 'contact_import' => 0,   'is_authentic' => 0, 'authenticated_on' =>NULL,
