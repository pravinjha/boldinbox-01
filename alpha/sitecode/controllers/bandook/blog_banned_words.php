...
class Blog_banned_words extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/BlogBannedWords_Model');
	...
	function banned_words_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/blog_banned_words/banned_words_list';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Banned Words'));
		$this->load->view('bandook/blog_banned_words_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function banned_edit($ban_id)
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				else
				{
					...
					redirect('bandook/blog_banned_words/banned_words_list');
				}
			}
		...
		$this->load->view('bandook/header',array('title'=>'Update Category'));
		$this->load->view('bandook/blog_bannedword_edit',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function banned_create($ban_id=0)
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				else
				{
					...
					redirect('bandook/blog_banned_words');
				}
			}
		...
		$this->load->view('bandook/header',array('title'=>'Create Banned Words'));
		$this->load->view('bandook/blog_bannedword_create',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
