...
class Language_manage extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/Language_Model');
	...
	function language_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/language_manage/language_list';
		...
		$this->load->view('bandook/header',array('title'=>'Manage template colors','logo_link'=>$logo_link));
		$this->load->view('bandook/language_list',array('languages'=>$language_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function color_edit($id=0)
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				else
				{
					...
					redirect('bandook/template_color/template_color_list');
		...
		$this->load->view('bandook/header',array('title'=>'Update Color','logo_link'=>$logo_link));
		$this->load->view('bandook/template_color_edit',array('color'=>$color_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function language_create()
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				else
				{
					...
					redirect('bandook/language_manage/language_list');
		...
		$this->load->view('bandook/header',array('title'=>'Create Language','logo_link'=>$logo_link));
		$this->load->view('bandook/language_create',array('langauge'=>$language_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
}
?>