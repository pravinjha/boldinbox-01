...
class Dashboard_Stat extends CI_Controller{
	...
	$this->output->enable_profiler(false);
		...
		//==============
		$rsPaidUsers = $this->db->query("select m.member_id,m.member_username,mp.package_id,mp.amount,date_format(mp.next_payement_date, '%Y-%m-%d') pt_date from red_members m inner join red_member_packages mp on m.member_id=mp.member_id where mp.package_id >0 and mp.next_payement_date > '$yesterday' order by package_id");
		...
		$totalAmountAlreadyPaidToday = $this->db->query("select sum(amount_paid)rs from red_member_transactions where gateway='AUTHORIZE' and status='SUCCESS' and date_format(transaction_date,'%Y-%m-%d') = '$today'")->row()->rs;
		 $totalAmountAlreadyPaidWeek = $this->db->query("select sum(amount_paid)rs from red_member_transactions where gateway='AUTHORIZE' and status='SUCCESS' and date_format(transaction_date,'%Y-%m-%d') >= '$week_start' and date_format(transaction_date,'%Y-%m-%d') <= '$week_end'")->row()->rs;
		...
		$stats['mrr_expected_week_sun_sat'] = $totalAmountAlreadyPaidWeek + $totalAmountExpectedWeek;
		...
		// $customerChurnMonth				= date('Y-m', strtotime('first day of last month'));
		$customerChurnMonth				= date('Y-m', strtotime(date('Y-m')." -1 month"));
		$stats['customerChurnMonthStr']	= date('F Y', strtotime(date('Y-m')." -1 month"));
		$customerChurnMonthBefore		= date('Y-m', strtotime(date('Y-m')." -2 month"));
		$totalMemberInMonthStart 		= $this->db->query("select count(distinct user_id) m from red_member_transactions where date_format(transaction_date,'%Y-%m') = '$customerChurnMonthBefore' and status='SUCCESS' and gateway='AUTHORIZE'")->row()->m;
		 
		$totalMemberInMonthEnd 			= $this->db->query("select count(distinct user_id) m from red_member_transactions where date_format(transaction_date,'%Y-%m') = '$customerChurnMonth' and payment_type=2  and status='SUCCESS' and gateway='AUTHORIZE'")->row()->m;
		
		$stats['customerChurn'] = ($totalMemberInMonthStart > 0)?round(($totalMemberInMonthStart - $totalMemberInMonthEnd)/$totalMemberInMonthStart, 5) : 1;
		
		$stats['signupform_unverified'] = $this->db->query("select count(id) c from red_signup_form where is_verified=0 and is_deleted=0 and is_hidden=0")->row()->c;
		
		$stats['autoresponders_unverified'] = $this->db->query("select count(campaign_id) c from red_email_autoresponders where is_verified=0 and is_deleted=0")->row()->c;
		
		
		$stats['free_to_paid_today'] = $this->db->query("select count(m.member_id) c from red_members m inner join red_member_transactions t on m.member_id=t.user_id where t.transaction_date > '$yesterday' and t.payment_type=1 ")->row()->c;
		
		$stats['free_to_paid_last7days'] = $this->db->query("select count(m.member_id) c from red_members m inner join red_member_transactions t on m.member_id=t.user_id where t.transaction_date > '$last7days' and t.payment_type=1 ")->row()->c;
		
		$stats['free_to_paid_last30days'] = $this->db->query("select count(m.member_id) c from red_members m inner join red_member_transactions t on m.member_id=t.user_id where t.transaction_date > '$last30days' and t.payment_type=1 ")->row()->c;
...
$this->load->view('bandook/dashboard_stats',array('stats'=>$stats));		//$this->load->view('bandook/dashboard_stats',array('total_users_count'=>$total_users_count, 'free_users_count'=>$free_users_count,'paid_users_count'=>$paid_users_count,'fail_cc_users_count'=>$fail_cc_users_count, 'admin_comped_users_count'=>$admin_comped_users_count, 'campaign_count'=>$campaign_count,'campaign_approval_count'=>$campaign_approval_count,'user_upgrade_count'=>$user_upgrade_count,'registered_user_count'=>$registered_user_count, 'this_month_users'=>$this_month_users, 'last_month_users'=>$last_month_users, 'mrr_expected'=> '$'.$mrr_expected, 'monthly_avg_revenue_per_customer'=>$monthly_avg_revenue_per_customer, 'lifetime_value'=> $lifetime_value, 'sent_campaign_count'=>$sent_campaign_count, 'paid_users_count_from_beginning'=>$paid_users_count_from_beginning, 'avg_subscription_lifetime'=>$avg_subscription_lifetime , 'customerChurn' => $customerChurn , 'customerChurnMonthStr' => $customerChurnMonthStr));
	...
	function sent_campaign($start=0){		
		...
		$config['base_url']=base_url().'bandook/dashboard_stat/sent_campaign';
		...
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/sent_campaign_stat',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
	function sent_campaign_daily($start=0){	 
		
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/dashboard_stat/sent_campaign_daily';
		$config['total_rows']=$this->Campaigns_Model->get_campaign_count(array(	'campaign_status'=>'active'	),true);
		$config['per_page']=1;
		$config['uri_segment']=4;
		...
		$campaign_data=$this->Campaigns_Model->get_campaign_data_for_sentmails_new(array('campaign_status'=>'active','DATEDIFF(now(), email_send_date)='=>$start),$sort_by,$config['per_page'],$start);
		...
		foreach($campaign_data as $campaign){
			...
			$upvote = $this->db->query("select ifnull(required_open,0) as upvote from red_email_campaigns_detail where campaign_id='".$campaign['campaign_id']."'")->row()->upvote;
			...
			$emailreport_data[$campaign['campaign_id']]['upvote']=$upvote ; // total upvote	
			$emailreport_data[$campaign['campaign_id']]['total_unread_emails']=$total_sent_emails - $campaign['email_track_read'] - $campaign['email_track_bounce']; // total unread emails			
			...
			$emailreport_data[$campaign['campaign_id']]['email_subject']=$campaign['email_subject']; // Campaign title	
			...
			$emailreport_data[$campaign['campaign_id']]['reply_to_email']=$campaign['reply_to_email']; // Campaign title
			$emailreport_data[$campaign['campaign_id']]['is_dmarc']=$campaign['is_dmarc']; // Campaign title
			$emailreport_data[$campaign['campaign_id']]['dmarc_from_email']=$campaign['dmarc_from_email']; // Campaign title
			...
			$emailreport_data[$campaign['campaign_id']]['member_username']=$user_data_array[0]['member_username'];
			$emailreport_data[$campaign['campaign_id']]['member_id']=$user_data_array[0]['member_id'];
			$emailreport_data[$campaign['campaign_id']]['lists']=$campaign['list_names'];			
		}		 
		...
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/sent_campaign_stat_new',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
	function sent_campaign_new($start=0){	 
		
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/dashboard_stat/sent_campaign_new';
		$config['total_rows']=$this->Campaigns_Model->get_campaign_count(array(	"campaign_status in('active','archived')"=>NULL),true);
		//echo $this->db->last_query();
		$config['per_page']=40;
		$config['uri_segment']=4;
		...
		$campaign_data=$this->Campaigns_Model->get_campaign_data_for_sentmails_new(array("campaign_status in('active','archived')"=>NULL),$sort_by,$config['per_page'],$start);
		
		//echo $this->db->last_query();
		foreach($campaign_data as $campaign){
		
			 $total_sent_emails = $this->Emailreport_Model->get_emailreport_count(array('campaign_id'=>$campaign['campaign_id']));
			...
			$emailreport_data[$campaign['campaign_id']]['total_delivered_emails']=$campaign['email_track_delivered']; // total delivered email			
			$emailreport_data[$campaign['campaign_id']]['total_unsubscribes']=$campaign['email_track_unsubscribes']; // total unsubscribes email			
			$emailreport_data[$campaign['campaign_id']]['total_sent_emails']= ($total_sent_emails > 0)?$total_sent_emails: $campaign['campaign_contacts']; // total sent emails			
			$emailreport_data[$campaign['campaign_id']]['total_read_emails']=$campaign['email_track_read']; // total read emails			
			$emailreport_data[$campaign['campaign_id']]['upvote']=$upvote ; // total upvote				
			$emailreport_data[$campaign['campaign_id']]['total_unread_emails']=$campaign['campaign_contacts'] - $campaign['email_track_read'] - $campaign['email_track_bounce']; // total unread emails			
			$emailreport_data[$campaign['campaign_id']]['total_click_emails']=$campaign['email_track_click']; // total click emails			
			$emailreport_data[$campaign['campaign_id']]['campaign_title']=$campaign['campaign_title']; // Campaign title	
			$emailreport_data[$campaign['campaign_id']]['email_subject']=$campaign['email_subject']; // Campaign title	
			$emailreport_data[$campaign['campaign_id']]['sender_name']=$campaign['sender_name']; // Campaign title			
	...
	function sent_campaign_beta($start=0){	 
		...
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/dashboard_stat/sent_campaign_beta';
		$config['total_rows']=$this->Campaigns_Model->get_campaign_count(array(	'campaign_status'=>'active'	),true);
		$config['per_page']=25;
		$config['uri_segment']=4;
		...
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/sent_campaign_stat_beta',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
	function bad_campaign_report($typ='spam'){
		...
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/bad_campaign_report',array('emailreport_data'=>$emailreport_data,'typ'=>$typ,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
...
group by vmta,dlvSourceIp,envId,webmail order by envId");
		foreach($rsBlockedStats->result() as $row){
			$cid = $row->envId;
			...
			$strTable .= "<tr><td>$mname</td><td>".$row->vmta." [$dlvSourceIp] </td><td>$cid</td><td>$webmail</td>
							<td><a class=\"fancybox\" href='". site_url('bandook/dashboard_stat/pmta_diag/'.$dlvSourceIpLong.'/'.$cid.'/'.$webmail)."'>".$row->x."</a></td></tr>";
		}
		...
		$this->load->view('bandook/header',array('title'=>'Blocked Contacts','logo_link'=>$logo_link));		
		$this->load->view('bandook/blocked_contacts',array('ReportTable'=>$strTable));
		$this->load->view('bandook/footer',null);
	...
	function user_campaign_stats($start=0){	 
		...
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/dashboard_stat/user_campaign_stats';
		$config['total_rows']=$this->Campaigns_Model->get_campaign_count(array(	'campaign_status'=>'active'	),true);
		$config['per_page']=25;
		$config['uri_segment']=4;
		...
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/user_campaign_stats',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
	
	//Function to show USER specifci stats for user with sent-dt,total-contacts,open,bounce,unsubscribe/complaint
	function sent_stats_for_user($usr=0,$start=0,$pg=30){	 
		if($usr== 0){echo "User not found."; exit;}
		$user_data_array=$this->UserModel->get_user_data(array('member_id'=>$usr));
		$member_username=$user_data_array[0]['member_username'];
		
		
		// Define config parameters for paging like base url, total rows and record per page.
		
		$sort_by = 'email_send_date';
		
		$campaign_data=$this->Campaigns_Model->get_campaign_data_for_sentmails_new(array("campaign_created_by"=>$usr, "campaign_status in('active','archived')"=>NULL),$sort_by,$pg,$start);
		//echo $this->db->last_query();
		$tblStats = '<div style="padding:20px;"><b>'.$member_username.'</b><br/><br/><table cellspacing="0" cellpadding="4" border="1"><tr><th width="180">Date</th><th>Campaign</th><th>Sent</th><th>Opens</th><th>Bounces</th><th>Unsubscribes</th><th>Complaints</th></tr>';
		
		$bouncesAll = $spamAll = $unsubAll =0;
		foreach($campaign_data as $campaign){
			...
			$total_sent_emails = ($total_sent_emails > 0)?$total_sent_emails: $campaign['campaign_contacts']; // total sent emails			
			
			$bouncesAll += $campaign['email_track_bounce'];
			$unsubAll += $campaign['email_track_unsubscribes'];
			$spamAll += $campaign['email_track_spam'];
			$tblStats .= "<tr><td>".date('D M j, Y g:ia', strtotime( getGMTToLocalTime($campaign['email_send_date'], date_default_timezone_get()))).'</td><td>'.$campaign['email_subject'].'</td><td>'.$total_sent_emails.'</td><td>'.($campaign['email_track_read'] + $upvote).'</td><td>'.$campaign['email_track_bounce'].'</td><td>'.$campaign['email_track_unsubscribes'].'</td><td>'.$campaign['email_track_spam'].'</td></td></tr>';
		}		 
 		$tblStats .= "<tr><th colspan='4' align='right'>Total:</th><th>$bouncesAll</th><th>$unsubAll</th><th>$spamAll</th></tr>";
		$tblStats .= '</table></div>';
		echo $tblStats;
	...
	function sent_campaign_user_stats($mid=0){			
		...
		#Loads header, users edit  and footer view.
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/sent_campaign_user_stats',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>get_shorten_url()));
		$this->load->view('bandook/footer');
	}
	...
	function sumit($dt=0, $oprate_morethan=0, $oprate_lessthan=3 , $vmta='all'){
		...
		//echo $this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link),true);
		echo $tblStats;
	...
	function sent_autoresponders($start=0){	 
		...
		# Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/dashboard_stat/sent_autoresponders';
		...
		#Loads header, users edit  and footer view.
		$this->load->view('bandook/header',array('title'=>'Sent Campaign List','logo_link'=>$logo_link));
		$this->load->view('bandook/sent_autoresponder_stats',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'contacts_array'=>$contacts_array,'shorten_url'=>$shorten_url));
		$this->load->view('bandook/footer');
	}
