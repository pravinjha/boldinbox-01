...
class Fromemail extends CI_Controller
{
	...
	*/
	function index( $start=0){		
		...
		$config['base_url']=base_url().'bandook/fromemail/index/';
		...
		$this->load->view('bandook/header',array('title'=>'Verify From Email','logo_link'=>$logo_link));
		$this->load->view('bandook/fromemail_list',array('fromemails'=>$femail_data_array,'contacts_array'=>$contacts_array,'paging_links'=>$paging_links,'messages' =>$messages,'mode'=>$mode));
	...
	*/
	function verifyit( $aid=''){
		...
		if($rsGetMember->num_rows() > 0){
			...
			$message .="Now you can use this address in your BoldInbox account and send emails from this address.<br/><br/>";
			$message .="Thanks,<br/>";
			$message .="The BoldInbox Team<br/><br/>";

			// send email using transactional_notification_helper
			send_tmail($email_to, SYSTEM_EMAIL_FROM, 'BoldInbox', 'New sending email verified!',$message,$textMessage,'boldinbox.com');
			// END
		}
		$rsGetMember->free_result();
		
		redirect('bandook/fromemail/index/');
	}
	 /**
	...
	*/
	function disallow( $aid=''){
		...
		if($rsGetMember->num_rows() > 0){
			...
			$textMessage .= "Please contacts us at \"support@boldinbox.com\" for more details. \r\n\r\n\r\n";
			$textMessage .="Thanks,\r\n\r\n";
			$textMessage .="The BoldInbox Team\r\n";
			
			$message ="Hello ".$name.", <br/><br/>";
			$message .="Your request to add a new from-email \"{$new_email}\" for your campaigns, is not approved and disallowed.<br/>";
			$message .="Please contacts us at \"support@boldinbox.com\" for more details.<br/><br/>";			
			$message .="Thanks,<br/>";
			$message .="The BoldInbox Team<br/><br/>";

			// send email using transactional_notification_helper
			send_tmail($email_to, SYSTEM_EMAIL_FROM, 'BoldInbox', 'New sending email disallowed!',$message,$textMessage,'boldinbox.com');
			// END
		}
		$rsGetMember->free_result();
