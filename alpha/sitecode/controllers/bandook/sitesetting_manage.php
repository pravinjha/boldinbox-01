...
class Sitesetting_Manage extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/Account_Model');
	...
	function change_password(){
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()==true)
			{
				...
				if($user_data_array[0]['webmaster_password']==$webmaster_password)
				{
					...
					redirect('bandook/sitesetting_manage/change_password');
				}
				else
				{
		...
		$this->load->view('bandook/header',array('title'=>'Change Password','logo_link'=>$logo_link));
		$this->load->view('bandook/site_change_password',array('messages' =>$messages));
	...
	function general_setting(){
		...
		if($this->input->post('action')=='submit'){
			...
			if($this->form_validation->run()==true){
				$input_array=array(
					'emergency_sending'=> implode(',', $this->input->post('emergency_sending', TRUE)),
					'p1p1_dnm'=>$this->input->post('p1p1_dnm', TRUE),
					'p1p2_dnm'=>$this->input->post('p1p2_dnm', TRUE),
					'p2p1_dnm'=>$this->input->post('p2p1_dnm', TRUE),
					'p2p2_dnm'=>$this->input->post('p2p2_dnm', TRUE),
			...
			$emergency_sending=$this->input->post('emergency_sending', TRUE);
	...
	function cron_setting(){
		...
		if($this->input->post('action')=='submit'){			
			$input_array=array(
				...
				'preprocessing_cron'=>$this->input->post('preprocessing_cron', TRUE),
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, campaign and footer view.
		$this->load->view('bandook/header',array('title'=>'Cronjob Setting','logo_link'=>$logo_link));
		$this->load->view('bandook/site_cron_setting',array('messages' =>$messages,
		...
		'preprocessing_cron'=>$preprocessing_cron,
		'preprocessing_start_date'=>$preprocessing_start_date,
	...
	function showRunningPS(){	
		...
		//$cmd = 'ps auxwww|grep "/public/index.php"|grep -v grep';
		$cmd = 'ps auxwww|grep "/bin/sh -c"|grep -v grep';
		exec($cmd,$op);
		$i = 1;
		foreach( $op as $p){	
			echo "<br/>".$i++ .". ";	
			...
			elseif(strstr($p, 'sendBomb') !== FALSE)
