...
class Feature extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/TemplateHeader_Model');
	...
	function feature_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/feature/feature_list';
		...
		foreach($header_data_array as $header){
			...
			// Load the template category model which interact with database
			$this->load->model('bandook/TemplateCategory_Model');
		...
		$this->load->view('bandook/header',array('title'=>'Manage template headers','logo_link'=>$logo_link));
		$this->load->view('bandook/template_header_list',array('headers'=>$header_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function header_edit($id=0)
	{
			...
			if($this->form_validation->run()) {
				...
				$template_id=$this->input->post('template_id');
					...
					if($_FILES['screenshot']['name']!=""){
						...
						$upload_path= $upload_path.'/locker/header-images/';						
						...
						else
						{
							...
							redirect('bandook/template_header/template_header_list');
						}
					}else{
						// Update Listing Category in database
						$this->TemplateHeader_Model->update_header($header_data,array('template_id'=>$template_id));
		...
		$this->load->view('bandook/header',array('title'=>'Update Header','logo_link'=>$logo_link));
		$this->load->view('bandook/template_header_edit',array('header'=>$header_data_array,'categories'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function header_create()
	{
		...
		$this->load->view('bandook/header',array('title'=>'Create Template Header','logo_link'=>$logo_link));
		$this->load->view('bandook/template_header_create',array('header'=>$header_data_array,'categories'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	/**
