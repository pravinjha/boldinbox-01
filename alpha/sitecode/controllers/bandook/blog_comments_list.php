...
class Blog_comments_list extends CI_Controller
{
	...
	function display($entry_id=0,$start=0)
	{
		...
		$config['base_url']=base_url().'index.php/bandook/blog_comments_list/display/'.$entry_id;
		...
		$this->load->view('bandook/header',array('title'=>'Manage Commnets ','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_comment_list',array('comments'=>$comment_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'entry_id'=>$entry_id,'page'=>$start));
	...
	function delete($entry_id=0,$id=0)
	{
		...
		redirect('bandook/blog_comments_list/display/'.$entry_id);
	}
