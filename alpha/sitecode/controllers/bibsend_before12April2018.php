...
class Bibsend extends CI_Controller
{
	...
	*/
	function is_soft_bounced($arr_contact){
		if($arr_contact['subscriber_email_domain'] == 'hotmail.com' and $arr_contact['subscriber_created_by']==351)return false;
	...
	*/
	...
	function addToQueueCron_old(){
		...
		if(trim($this->confg_arr['queueing_cron']) == '1'){
			...
			$queueing_campaigns = $this->confg_arr['queueing_campaigns'];
			$rsQueueingCampaigns = $this->db->query("Select campaign_id, campaign_created_by, subscription_list, sent_counter, campaign_contacts,(campaign_contacts - sent_counter)ordby, tobe_campaign_status from red_email_campaigns where campaign_status='queueing' and is_deleted=0 and campaign_id not in($queueing_campaigns)  order by ordby limit 1");
	...
	function addToQueueCronTwo($cid=0,$cron='n'){
		set_time_limit(0); 				 
			...
			$strQueueingCampaigns = @implode(',',$arrQueueingCampaigns);
...
//		}	
			...
			if(count($arrSeedlistMails) > 0){
				...
				$message= $CIDHeader."<table width='100%' style='#ffffff'><tr><td align='center'>
							...
							<tr><td>&nbsp;</td></tr>
							<tr><td  align='center'><a href='".CAMPAIGN_DOMAIN."markit/powered_by_bib/-1/-99'>
	...
	*/
	function improveLog($mailid=0,$count=0){
		...
		$rsCampaignToImproveLog = $this->db->query("select t1.campaign_id, t1.add_open, t1.required_open, t2.campaign_created_by from `red_email_campaigns_detail` t1 inner join red_email_campaigns t2 on t1.campaign_id=t2.campaign_id where t1.`add_open` > t1.`required_open` and t2.campaign_status='active' and t2.is_deleted=0");
