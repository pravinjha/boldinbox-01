...
*
* This class is for taking campaign email stat freezing
*
* @version 1.0
...
class Campaign_email_track_restorage extends CI_Controller
{
	...
	*/
	function email_count($mode="",$campaign_id=0){
		$retval = 0;
		if($mode=="send"){		
			$retval = $this->Campaign_email_track_restorage_Model->email_sent_count(array('campaign_id'=>$campaign_id));			
			...
			$retval = $this->Campaign_email_track_restorage_Model->email_read_count(array('campaign_id'=>$campaign_id, 'email_track_read >'=>0));			
...
//					echo strip_tags($this->db->_error_message());
