...
class Change_package extends CI_Controller{
	...
	*/
	function index(){		
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Change Package','previous_page_url'=>$previous_page_url,'contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'user/change_plan',array('packages'=>$packages, 'eligiblePackage'=>$eligiblePackage, 'user_package'=>$user_packages_array[0],'selected_package'=>$current_package_id,'messages'=>$messages,'posted'=>$posted, 'arrUserProfile'=>$arrUserProfile, 'country_info'=>$country_info, 'mode'=>$mode));
	...
	function getPromoDetail(){
		...
		$rsPcode = $this->db->query("select * from red_promotion_code where `promotion_code`='$promocode' and `payment_months`='$months' and is_deleted=0");	
