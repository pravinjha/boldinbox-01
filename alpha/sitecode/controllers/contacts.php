...
class Contacts extends CI_Controller
{
	...
	function index($subscription_id=0,$scroll=0,$action=""){		
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'List subscriptions','contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'contacts/contacts',$subscription_data);
	...
	*/
	 ...
	 *
	 * 'add_contacts' controller function add contacts.
	 *
	 * @param (int) (subscription_id)  for displaying subscription selectable(blue color)  in view of subscription list
	 */
	
	function add_contacts($subscription_id=0){
		$subscription_id = (is_numeric($subscription_id))? $subscription_id : 0;
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id'));
		//Loads header, subscription and footer view.
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Add/Import Contacts','contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'contacts/bib_add_contacts',$subscription_data);
		$this->load->view($this->config->item('theme_folder').'footer');
	}
	/**
