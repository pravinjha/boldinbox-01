 ...
 */
class Biblogs extends CI_Controller
{
	//Define hard bounce
	...
	*/
	function __construct()
    {
		...
		$this->load->model('userboard/Pmta_Model');		
				...
				foreach( $arrFiles as $f){
					...
					if(is_dir($file)){
						...
						if(($intFileCount % 2 == 0)&& ($newStatus == 'even')){				// For even files							 	
								...
								if (file_exists($source_dir.$file)  && copy($source_dir.$file, $destination_dir.$file)) {
			...
			foreach( $arrFiles as $f){	
				...
				if(is_dir($source_dir.$f)){
					...
					if($intFilePrefixNo % 2 == 1){				// For odd files							 	
						...
						$this->importCsv($f);					
						// If we copied this successfully, delete it
						if (file_exists($source_dir.$f)  &&  copy($source_dir.$f, $destination_dir.$f)) {
							@unlink($source_dir.$f);
	...
	*/
	...
	function importCsv($file=""){
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE){			
				 ...
				 $envId = $extra_header_arr[1];
					...
					if(count($pmta_log_array)>0){						
						...
						if(count($hard_bounce_match) >= 1){
							...
							$subscrber_bounce=1;
							$email_track_bounce=0; // 2 changed to 0 as soft bounces are now ignored only
							$subscriber_status=1; 
							$dnm_type ='softbounce';
						...
						if($subscrber_bounce == 1 and strtolower(trim($subscriber_array[0]['subscriber_email_domain'])) == 'hotmail.com'){
							$ignr = 0;
						}else{
							$ignr = 1;
						}
						
						// IGNORE for soft bounce disabled. Sending will continue from Dt: 7 Sep, 2019
						// Update subscriber
						...
						//$this->Subscriber_Model->update_subscriber(array('ignore'=>$ignr,'subscrber_bounce'=>$subscrber_bounce,'subscriber_status'=>$subscriber_status,'soft_bounce'=>$soft_bounce, 'bounce_status'=>$bounce_status, 'bounce_detail'=>$bounce_detail, 'status_change_date'=>$utc_str,'bounced'=>$soft_bounce,'last_bounced_date'=>$utc_str),array('subscriber_id'=>$jobId));
						// *****************************************************
						
						$this->incrementDeliveredCounter($envId, $jobId);
												
						if($dnm_type == 'hardbounce'){
						//update  email report	
			...
			if($data_array['type']=='f'){ // FBL report
			$strDebugText = implode(':==:', $data_array);
			admin_notification_send_email('pravinjha@gmail.com, peejha@yahoo.com', SYSTEM_EMAIL_FROM,'BoldInbox', "BIB- FBL",$strDebugText,$strDebugText);				
			...
			}// FBL ends, IF CLOSES FOR CHECKING NOT HEADER ROW
			
			
			if($data_array['type']=='d'){// Star Delivered marked, ROW IS NOT HEADER
				...
				if(trim($data_array['rcpt']) == 'unsubscribe@boldinbox.net' or trim($data_array['rcpt']) == 'unsubscribe@rcmailsv.com' or trim($data_array['rcpt']) == 'unsubscribe@mailsvrc.com' or trim($data_array['rcpt']) == 'unsubscribe@rcmailcorp.com'){
					$x = implode(',',$data_array);
					@mail('pravinjha@gmail.com','BIB-unsubscribe',$x);
			...
			}// Ends Delivered marked, ROW IS NOT HEADER
	...
	function ifCriticalPause($cid, $typ='b'){
		if(is_numeric($cid)){
			...
			$rsIsPausable = $this->db->query("select is_pausable, member_username from red_members where member_id='$mid'");
			...
			if($is_pausable){
				...
				if($is_ongoing){
					...
					$rsCountDNM = $this->db->query("select count(subscriber_id) c from $statsTable where campaign_id='$cid' and $dnmType"); 
					$countDNM = $rsCountDNM->row()->c;
					$rsCountDNM->free_result();
				
					$rsCountAll = $this->db->query("select count(subscriber_id) c from $statsTable where campaign_id='$cid' "); 
					...
					if( $clevel < (($countDNM / $countAll) * 100)  ){
						...
						if( ($cstatus == 'archived') or($is_ongoing > 0) ){
							...
							admin_notification_send_email($to, SYSTEM_EMAIL_FROM,'BoldInbox', "Campaign [$cid] for $member_pausable paused due to $reason",$message,$text_message);				
	...
	function incrementDeliveredCounter($campaign_id, $contact_id){
		...
		$sqlCheckContactMarkedDelivered = "select `email_delivered`,`email_track_bounce` from `$statsTable` where `campaign_id`='$campaign_id' and `subscriber_id`='$contact_id'";
	...
	function incrementBouncedCounter($campaign_id, $contact_id){
		$statsTable = $this->is_authorized->getStatsTable($campaign_id); // get stats table
		$sqlCheckContactMarkedBounced = "select `email_track_bounce` from `$statsTable` where `campaign_id`='$campaign_id' and `subscriber_id`='$contact_id'";
	...
	function incrementSpamCounter($campaign_id, $contact_id){
		$statsTable = $this->is_authorized->getStatsTable($campaign_id); // get stats table
		$sqlCheckContactMarkedSpam = "select `email_track_complaint` from `$statsTable` where `campaign_id`='$campaign_id' and `subscriber_id`='$contact_id'";
	...
	/**
	* Send notification to admin for Payment-failure and completion of subscription-cycle.
	*/
	function notifyPaymentAlert(){	
		$sent_counter_reset = $this->resetSentCounterEveryMonth();
		$strTable = "<table width='100%' cellspacing='2' cellpadding='2' border='1' style='border-collapse: collapse;'>";
		$strT1 = "<tr><th colspan='2'>Users whose payment failed</th></tr>";
		$strT2 = "<tr><th colspan='2'>Users whose payment will fail soon</th></tr>";
		$rsPaidUsers = $this->db->query("select m.`member_id`, `member_username`, m.`email_address`, date_format(`next_payement_date`, '%Y-%m-%d') pdt, mp.`package_id` from `red_members` m INNER JOIN `red_member_packages` mp ON m.`member_id`=mp.`member_id` where mp.`package_id` > 0 order by pdt");
		...
		foreach($rsPaidUsers->result() as $row){
			...
			$pid	 	= $row->package_id;
			$paydt	 	= $row->pdt;
			$date_diff =  (strtotime($paydt) - strtotime(gmdate("Y-m-d")))/(60*60*24);
			if($date_diff < 0){				
				$strT1 .= "<tr><td>(".$i++.") [$mid] $mname < $memail > </td><td>$paydt   $date_diff</td></tr>";
			}elseif($date_diff <= 3){
				$strT2 .= "<tr><td>(".$j++.") [$mid] $mname < $memail > </td><td>$paydt   $date_diff</td></tr>";
			}
			if($date_diff <= 3){
			$this->notifyMembers($paydt, $date_diff, $mname, $memail);			
			}
		}
		$rsPaidUsers->free_result();
		
		$strTable = $strTable . $strT2 . $strT1 . "</table>";
		
		$to = $this->confg_arr['admin_notification_email'];						
		admin_notification_send_email($to, SYSTEM_EMAIL_FROM,'BoldInbox', "Member Payment Detail",$strTable."<br/>".$sent_counter_reset,strip_tags($sent_counter_reset."<br/>".$strTable));	
	}
	
	function notifyMembers($nextPdt, $datediff, $uname, $uemail){	
		
		if($datediff < 0){ //subscription expired
			$content = "<p>Hi $uname, <br/><br/>This is to remind you that your www.boldinbox.com subscription has expired on:  $nextPdt.";
		}else{
			$content = "<p>Hi $uname, <br/><br/>This is to remind you that your www.boldinbox.com subscription is expiring on:  $nextPdt.";
		}
		$content .= "<br/>To avail uninterrupted service, please pay for your next payment-cycle.
					<br/>For any query, reply to this email.<br/>
					<br/>Best,<br/>Team BoldInbox
					<br/> https://www.boldinbox.com";		
		$content = getHeader() . $content . getFooter();
		admin_notification_send_email($uemail, SYSTEM_EMAIL_FROM,'BoldInbox Support',  "Payment Reminder", $content, strip_tags($content)) ;
	}	
	function resetSentCounterEveryMonth(){
		$str = '';
		$sql ="SELECT member_id,next_payement_date, tdt, DATEDIFF(next_payement_date, tdt)days, (DATEDIFF(curdate(), tdt)/30)month_spent, campaign_sent_counter from red_member_packages t1 inner join (SELECT user_id, MAX(transaction_date)tdt FROM `red_member_transactions`  where package_id > 0  group by user_id)t2 on t1.member_id = t2.user_id where package_id > 0 and next_payement_date > now() ";
	$rsGetMonthCount = $this->db->query($sql);
	if($rsGetMonthCount->num_rows() > 0){
		foreach($rsGetMonthCount->result() as $row){
			$mid = $row->member_id;
			$monthCount = $row->month_spent;
			$next_pdt = $row->next_payement_date;
			if($monthCount == intval($monthCount) ){
				$str .= "<br/>YES: Mid=".$mid.'==NextPayDt:'.$next_pdt.'=monthsFloat='.$monthCount.'=====monthsINT'.intval($monthCount);	
				$this->db->query("update red_member_packages set campaign_sent_counter=0 where member_id='$mid'");
			}else{
				//$str .= "<br/>NO: Mid=".$mid.'==NextPayDt:'.$next_pdt.'=monthsFloat='.$monthCount.'=====monthsINT'.intval($monthCount);
			}
	
		}
	}
	$rsGetMonthCount->free_result();
	...
	*/
	...
	function updateDailyGlobalIPR($emlid,$vmta,$recType, $campaign_id=0){
		...
		$this->db->query("insert into red_global_ipr_daily set `mail_domain` = '$IPR_Domain' ,  `log_date`='$email_send_date' ,  `pipeline`='$vmta', `user_id`='$user_id', $increment_counter ON DUPLICATE  KEY UPDATE  $increment_counter ");		
	}
	...
	function removeOldLogFiles(){
		...
		// Remove campaing send log
		$faltuCampaignLog = $this->config->item('campaign_files');
		if($logs = opendir($faltuCampaignLog)){
			while (($log = readdir($logs)) !== false){
				if ($log == '.' || $log == '..')
					continue;
				// echo "<br/>".date('Y-m-d h:i:s',filectime($faltuCampaignLog.$log)).'-------'.$log;
				if (filectime($faltuCampaignLog.$log) <= time() - 2 * 24 * 60 * 60){
					unlink($faltuCampaignLog.$log);
				}
			}
			closedir($logs);		
		}
