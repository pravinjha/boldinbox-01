...
class Subscriber extends CI_Controller
{
	...
	function create($subscription_id=0){	
		...
		if($this->input->post('action')=='copy_pase_contacts'){
			// Validation rules are applied
			$this->form_validation->set_rules('copy_csv', 'Paste contacts', 'required');
			//$this->form_validation->set_rules('terms', 'Terms & Conditions', 'required');
			// To check form is validated
			if($this->form_validation->run()==true){
				if($this->input->post('terms') != 0){
					...
					$command = config_item('php_path')." ".FCFOLDER."/index.php  bibsend bgprocess $memberid $sublistid $file_name $list_type";
		...
		if($this->input->post('action')=='ony_by_one'){
			...
			if($this->form_validation->run()==true)
			{
				...
				for($i=0;$i<count($_POST[subscriber_email_address]);$i++){
					...
					if($_POST[subscriber_email_address][$i]!=''){
						...
						if($this->email_check($_POST[subscriber_email_address][$i])){
							...
							$input_array['subscriber_email_address'] = mysqli_real_escape_string($this->is_authorized->get_mysqli(), $_POST[subscriber_email_address][$i]);
							...
							$input_array['subscriber_first_name'] = mysqli_real_escape_string($this->is_authorized->get_mysqli(), $_POST[subscriber_first_name][$i]);

							if(trim($_POST['subscriber_last_name'][$i]) !='')
							$input_array['subscriber_last_name'] = mysqli_real_escape_string($this->is_authorized->get_mysqli(), $_POST[subscriber_last_name][$i]);
							...
							$flds = '';
							foreach($input_array as $key=>$val)  $flds .= $key . ' = \'' . mysqli_real_escape_string($this->is_authorized->get_mysqli(), $this->fixEncoding(trim($val))) . '\', ';
							...
							$arrListId = explode(',', $subscription_id);
				...
				if( ($total_unauthentic_contacts) > $max_contacts_to_unauthenticate){
					...
					$message = "<p>Hello admin,</p><p>RC Member :$mname [$mid] is unauthenticated after contacts import</p><p>Regards,<br />BoldInbox Team</p>";		
	...
	function subscriber_create($subscription_id=0,$scroll=0,$selected_subscription_id=0,$page_link=0,$action="",$action_notmail="",$search=""){
		...
		if(is_numeric($subscription_id)){
			...
			$subscription_id=0;
			echo "error:List-id does not exist";
			exit;
		}
		...
		if($this->input->post('action')=='save'){
			...
			$previous_page_url=base_url()."contacts/index/".$selected_subscription_id."/".$scroll."/".$action_notmail."/".$action;
			if($search!="")	$previous_page_url .= "/".$search;
			if($page_link!=0)$previous_page_url .= "#".$page_link;
	...
	*/
	function detail($subscriber_id,$scroll=0,$selected_subscription_id=0,$page_link=0,$action="",$action_notmail="",$search=""){
		//Check if user is not login then redirect to index page
		if($this->session->userdata('member_id')=='')
			redirect('user/index');
		...
		if(count($subscriptions)>0){
			if($subscriptions[0]['subscriber_name']!=""){
				...
				$max_el = count($name_arr)-1;
				for($i=0;$i<$max_el;$i++){
					$fname .=  ' ' .$name_arr[$i];
				}
				$fname = trim($fname);
				$subscriptions[0]['subscriber_first_name'] = ($subscriptions[0]['subscriber_first_name']!="")?$subscriptions[0]['subscriber_first_name'] : $fname;
				$subscriptions[0]['subscriber_last_name'] = ($subscriptions[0]['subscriber_last_name']!="")?$subscriptions[0]['subscriber_last_name'] : $name_arr[$max_el];
			...
			// Commented by pravinjha@gmail.com on 14 September, 2017
			//$strHistory = $this->ajaxHistory($subscriptions[0]['subscriber_id'], $subscriptions[0]['soft_bounce'], $subscriptions[0]['subscrber_bounce']);


			
		}else{
			$previous_page_url=base_url()."contacts/index/".$selected_subscription_id."/".$scroll."/".$action_notmail."/";
			if($page_link!=0)$previous_page_url .= $action;
			if($search!="")$previous_page_url .= "/".$search;
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Subscriber View','previous_page_url'=>$previous_page_url,'contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'contacts/subscriber_view',array('subscriptions'=>$subscriptions,'contact_soft_bounce'=>$subscriptions[0]['soft_bounce'],'contact_bounce_status'=>$subscriptions[0]['subscrber_bounce'],'subscription_title'=>$subscription_title,'shorten_url'=>$shorten_url,'max_soft_bounce'=>$max_soft_bounce,'contact_history'=>$strHistory));
	...
	function subscriber_list($subscription_id=0,$start=0,$type=""){
		...
		$config['base_url']=base_url().'subscriber/subscriber_list/'.$id;	// The page we are linking to
		...
		if($id>0){
			...
			if($this->input->get_post('bounce',true)==1){
				...
				if(trim($this->input->post('srch_email',true))!='')$unsubscribe_where .= " and subscriber_email_address like'%".trim($this->input->post('srch_email',true))."%'";
		...
		*/
		...
		if(trim($this->input->post('srch_email',true))!='')$removed_where .= " and subscriber_email_address like'%".trim($this->input->post('srch_email',true))."%'";
		...
		*/
		...
		if(count($subscriber_data['subscribers'])){
			...
			foreach ($subscriber_data['subscribers'] as $subscriber){
				...
				if($subscriber['subscriber_name']!=""){
					...
					$subscriber['subscriber_first_name']=($subscriber['subscriber_first_name']=="")?$fname:$subscriber['subscriber_first_name'];
					$subscriber['subscriber_last_name']=($subscriber['subscriber_last_name']=="")?$name_arr[$max_el]:$subscriber['subscriber_last_name'];
					$fname = '';
				}
				if(!$subscriber['subscription_id']){
					$subscriber['subscription_id']= 0 - intval($this->session->userdata('member_id'));
				}
				//Check status of subscriber
				$status=($subscriber['subscriber_status']==1)?"Subscribed" :  "Unsubscribed";
				if($start==0)$start=1;				
				$checked=(($_POST['action']=="page")||($_POST['action']=="list"))?"checked" : '';
				if($_POST['action']=="page")$disabeld=" disabled true";
				$alternate_class = ($i%2 == 1)?'contacts_list_row_b':'contacts_list_row_w';
				$contact_list.="<div class='row row-striped $alternate_class'>
						<div class='col-1 contacts-cols'><input  class='check-boxalign' type='checkbox'  value='".$subscriber['subscriber_id']."' name='subscriber_id[]' id='subscriber_id' ".$checked.$disabeld."></div>
						<div class='col-6 contacts-cols'><a title='".$subscriber['subscriber_email_address']."' href='".site_url('subscriber/detail/'.$subscriber['subscriber_id'])."'>".substr($subscriber['subscriber_email_address'],0,50)."</a></div>
						<div class='col-4 contacts-cols col-name'>".substr(stripslashes($subscriber['subscriber_first_name']),0,25)." &nbsp;</div>
						<div class='col-1 contacts-cols'>
							<ul class='contacts-ops-links'>
								<li class='contacts-edit'><a href='".site_url('subscriber/detail/'.$subscriber['subscriber_id'])."' class = 'pink round round32'><i class='fa fa-edit'></i></a></li>
								<li class='contacts-trash'><a href='".site_url('subscriber/subscriber_delete/'.$subscriber['subscription_id'].'/'.$subscriber['subscriber_id'])."' class='fancybox_delete pink round round32' name='".$subscriber['subscriber_id']."'><i class='fa fa-trash'></i></a></li>
							</ul>
						</div>
					</div>";					
			}
			...
			$records_pagination="<div class='view-pagesize'>
								<strong>Show</strong>
								<select id='psize' style = 'vertical-align:middle;' onchange=\"javascript:newPageSize(this);\">
									{$opt}
								</select>
			...
			<td colspan="4" valign = "top" style = "padding:10px;font-size:14px;color:#666;text-align:center;"><div style="height: 400px">
            No Records Found. <br /><br /><br /><a class="buttonLg" href="'. site_url("bib_add_contacts").'"> + Add New Contacts</a>
          </div></td></tr>'; 
			}else{ 
			$contact_list.='<tr class="contacts_change">
			<td colspan="4" valign = "top" style = "padding:10px;font-size:14px;color:#666;text-align:center;"><div style="height: 400px">
		...
		echo $subscriber_data['links']."|-|".$contact_list.'|-|'.$unsubscriber_count."|-|".$bounce_count."|-|".$complaint_count."|-|".$config['total_rows']."|-|".$removed_count."|-|".$records_pagination;
	}
	...
	function subscriber_delete($subscription_id=0,$subscriber_id=0){
		if($this->input->post('submit_action')=='submit'){
			...
			if($this->form_validation->run()){
				...
				if($subscription_id == 0)die("error: List is not selected");
					...
					if((count($_POST['subscriber_id'])>0)||($this->input->get_post('action',true)=="page")){
						...
						// Delete All/Searched contacts from member's account or from "ALL Lists"
							...
							if(trim($contactStatus) != '1' and trim($contactEmail) !=''){
								...
								$mid = $this->session->userdata('member_id');
								$mname = $this->session->userdata('member_username');
								$message = "<p>Hello admin,</p><p>RC Member :$mname [$mid] has deleted contact [ $contactEmail ] from DNM.</p><p>Regards,<br />BoldInbox Team</p>";		
								$text_message= "RC Member :$mname [$mid] has deleted contact [ $contactEmail ] from DNM.";								
								$to = $this->db->query('SELECT config_value FROM `red_site_configurations` where `config_name` = "admin_notification_email"')->row()->config_value;	
								admin_notification_send_email($to,  SYSTEM_EMAIL_FROM,'BoldInbox', "DNM deleted by $mname [$mid]",$message,$text_message);
							}
							$delete_contacts=count($this->input->get_post('subscriber_id',true));
						}
	...
	function importcsv($subscription_id,$terms_condition=0){
		...
		if($terms_condition != 0){
			...
			$this->validate_csv_upload($subscription_id);
		}else{
	...
	function validate_csv_upload($subscription_id){
		...
		if(!$this->upload->do_upload('subscriber_csv_file')){			
			...
			$command = config_item('php_path')." ".FCFOLDER."/index.php  bibsend bgprocess $memberid $sublistid $file_name $list_type $do_notify";
			exec( "$command >> $contact_import_log  2>&1 &", $arrOutput );
 ...
 /**
	...
	function exportcsv($subscription_id=0, $search_key=''){	
		...
		$id = (is_numeric($subscription_id))?$subscription_id : 0;
		$mid = $this->session->userdata('member_id');
		$isManageContacts = $this->db->query("select manage_contacts from red_members where member_id='$mid'")->row()->manage_contacts;	
		if($id ==0  or  $isManageContacts < 1){
			redirect('contacts');exit;
		}
		
		$condiotions_string= " `res`.`subscriber_created_by` = '$mid' and `res`.`is_deleted` = 0 and `res`.`subscriber_status` = 1";
		...
		if(trim($search_key) !='')$condiotions_string .= " and res.subscriber_email_address like '%".trim($search_key)."%'";
		//Create output string  with heading
		$csv_output_header="Email, First Name,Last Name,Address,Birthday,City,Company,Country,Phone,State,Zip Code,Optin Time,Optin IP";
		//$csv_output_header.="\n";
		$csv_output="\n";
		...
		$filename = "BoldInbox_Contacts_".date("Y-m-d_H-i",time());
	...
	**/
	...
	function subscriber_action(){
		...
		if($_POST['contact_list_action']=="delete"){
			...
			$todayDt = date('Y-m-d');
			if($action=="page"){
				$msg="Multiple Records Can not be moved to Do-Not-Mail-List!";
				// *********** REMOVED CODE************************
				//if(isset($_POST['email_search']) and trim($_POST['email_search']) !=''){
				//	$srch=mysqli_real_escape_string($this->is_authorized->get_mysqli(), $this->input->post('email_search'));
				...
				$i=0;
				if(isset($_POST['subscriber_id']) and count($_POST['subscriber_id']) > 1){
					$msg="Multiple Records Can not be moved to Do-Not-Mail-List!";
				}else if(isset($_POST['subscriber_id']) and count($_POST['subscriber_id']) == 1){
					foreach($_POST['subscriber_id'] AS $id){
						$this->suppress($id);
						$i++;
					}
			...
			$msg= ($msg !='')?$msg:"Unsubscribed Successfully";
			$_POST['uncheck_list']="";
			$this->subscriber_list($_POST['subscription_selected_id']);
			echo "|-|".$msg;
