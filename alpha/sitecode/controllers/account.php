...
class Account extends CI_Controller
{
	...
	function account(){
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'My Account','contactDetail'=>$contactDetail));
		$this->load->view($this->config->item('theme_folder').'user/account',array('package_detail'=>$package_detail[0],'user_info'=>$user_data,'user_transactions'=>$user_transactions,'user_credit_card_info'=>$user_credit_card_info[0],'messages'=>$messages,'country_info'=>$country_info));
	...
	function invoices_view_all(){
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'My Invoices','previous_page_url'=>$previous_page_url));
		$this->load->view($this->config->item('theme_folder').'user/invoices_list',array('user_transactions'=>$user_transactions));
		$this->load->view($this->config->item('theme_folder').'footer-inner-red');
	}
	/**
	...
	function billing_detail($id=0){
		...
		$this->load->view($this->config->item('theme_folder').'user/billing_detail',array('user_transactions'=>$user_transactions[0]));
	}
