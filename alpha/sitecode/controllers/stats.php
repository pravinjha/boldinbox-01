	...
	*/
	function display($id=0,$start=0){
		...
		if($id=='page' or $int_cid<=0){			
			...
			$config['base_url']=base_url().'stats/display/page/';
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Campaign Stats','contactDetail'=>$contactDetail));

		$this->load->view($this->config->item('theme_folder').'stats/emailreport_view',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'extra'=>$extra));

		$this->load->view($this->config->item('theme_folder').'footer');

	}
	
	function showStats($id=0,$start=0){
		...
		$this->load->view('stats/show_stats',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'extra'=>$extra));

		//$this->load->view('footer');

	}
// function to show comma separated stats for any user
	function showStatsXLS($id=0,$start=0){
		...
		$this->load->view('stats/show_stats_xls',array('emailreport_data'=>$emailreport_data,'paging_links'=>$paging_links,'extra'=>$extra));
	...
	function detail($action="",$enc_cid=0,$start=0,$url=""){
		...
		$this->session->set_userdata('manage_contacts', $isManageContacts);
		
		
		$fetch_condiotions_array=array(	'campaign_created_by'=>$mid,'is_deleted'=>0,'campaign_status'=>'active','campaign_id'=>$id	);
	...
	function view_subscriber_click($enc_cid=0,$url="",$start=0){
		...
		$fetch_condiotions_array=array('ret.campaign_id'=>$id, 'counter >'=>0, 'tiny_url'=>$url, 'is_autoresponder'=>0);
			...
			$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id'));
			$this->load->view('header',array('title'=>'Sent Mail Report','previous_page_url'=>$previous_page_url, 'contactDetail'=>$contactDetail));
			$this->load->view('stats/emailreport_subscriber',array('emailreport_data'=>$emailreport_subsciber_data,'current_tab'=>'click','campaign_id'=>$id,'tiny_url'=>$url, 'campaign_data'=>$campaign_data[0],'paging_links'=>$paging_links, 'sent_total_count'=>$sent_total_count, 'read_total_count'=>$read_total_count, 'bounced_total_count'=>$bounced_total_count, 'unread_total_count'=>$unread_total_count, 'unsubscribes_total_count'=>$unsubscribes_total_count, 'complaints_total_count'=>$complaints_total_count, 'clicks_total_count'=>$clicks_total_count, 'forward_total_count'=>$forward_total_count,'extra'=>$extra));
			$this->load->view('footer');
	}
	/**
	...
	*/
	function display_email_track_from_backup($campaign_id=0){
		$upvote = $this->db->query("select ifnull(required_open,0) as upvote from red_email_campaigns_detail where campaign_id='$campaign_id'")->row()->upvote;
		...
		$emailreport['total_read_emails']=$campaign_stat[0]['read_email_count'] + $upvote;	# total read emails
	...
	*/
	function view_email_detail_from_backup($campaign_array=0,$action=""){
		$upvote = $this->db->query("select ifnull(required_open,0) as upvote from red_email_campaigns_detail where campaign_id='".$campaign_array['campaign_id']."'")->row()->upvote;
		$this->load->model('userboard/Campaign_email_track_restorage_Model');
		...
		$read_total_count = $campaign_stat[0]['read_email_count'] + $upvote;		
	...
	function exportcsv($action, $enc_cid, $url=''){
		...
		$strMid = $this->session->userdata('member_id');
		$isManageContacts = $this->db->query("select manage_contacts from red_members where member_id='$strMid'")->row()->manage_contacts;	
