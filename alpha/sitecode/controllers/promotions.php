...
class Promotions extends CI_Controller
{
	...
	function index($start=0){	
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'List Campaigns','contactDetail'=>$contactDetail));
		$this->display($start,false);
		$this->load->view($this->config->item('theme_folder').'footer',array('isFirstTimeUser'=>$isFirstTimeUser));
	}
	...
	function display($start=0,$is_ajax=true){		
		...
		if($is_ajax)
		echo $this->load->view($this->config->item('theme_folder').'promotions/campaign_list_ajax',array('paging_links'=>$paging_links,'subscriber_data'=>$subscriber_data,'campaign_data'=>$campaign_data,'shorten_url'=>$shorten_url), true);
		else
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_list',array('paging_links'=>$paging_links,'subscriber_data'=>$subscriber_data,'campaign_data'=>$campaign_data,'shorten_url'=>$shorten_url));
	...
	*/
	function layouts(){			
		...
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'List Campaigns','contactDetail'=>$contactDetail));		
		$this->load->view($this->config->item('theme_folder').'promotions/layouts', array('layout_list'=>$arrLayout, 'customLayout'=>$customLayout));
	...
	function plain_text($enc_cid=0){		
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id')); 
		// Loads header, campaign and footer view.
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Promotions: Plain-text', 'contactDetail'=>$contactDetail));		
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_plain_text',array('campaign_data'=>$campaign_array));		
		$this->load->view($this->config->item('theme_folder').'footer');
	}
	...
	function html_code($enc_cid=0){		
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id')); 
		// Loads header, campaign and footer view.
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Promotions: Paste-html-code', 'contactDetail'=>$contactDetail));		
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_html_code',array('campaign_data'=>$campaign_array));		
		$this->load->view($this->config->item('theme_folder').'footer');
	}
	...
	function url_import($enc_cid=0){	
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id')); 
		// Loads header, campaign and footer view.
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Promotions: URL-import', 'contactDetail'=>$contactDetail));		
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_import_url',array('campaign_data'=>$campaign_array));		
		$this->load->view($this->config->item('theme_folder').'footer');	
	}
	...
	function zip_import($enc_cid=0){	
		...
		// Loads header, campaign and footer view.
		$this->load->view($this->config->item('theme_folder').'header',array('title'=>'Promotions: URL-import', 'contactDetail'=>$contactDetail));		
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_import_zip',array('campaign_data'=>$campaign_array));		
		$this->load->view($this->config->item('theme_folder').'footer');
	}	
	...
	*/	
	...
	$email_template_color_info=$this->Campaign_Model->get_background_color_blocks_names_and_content_data(array('red_background_color_page_id'=>$email_template_data['pages'][0]['id']),$block_all_data_count);
		...
		//Loads header, campaign and footer view.		
		//$this->load->view($this->config->item('theme_folder').'header');
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_editor',$email_template_data);
		//$this->load->view($this->config->item('theme_folder').'footer');
	}
	...
	function get_socialmedia_ajax(){
		...
		if($rsSMedia->num_rows() > 0){
			foreach($rsSMedia->result_array() as $row){
				...
				$strTableBody .='<tr height="50">
								<td width="50">
									<img src="'. $this->config->item('locker'). 'images/icons/'.$thisSMName.'-share.png"  style = "box-shadow:2px 3px 5px #888;" title="'.$thisSMName.'" />
								</td>
								<td align="center">
									<input type="checkbox" value="1" name="'.$thisSMName.'_link" id="'.$thisSMName.'_link" '.$isChecked.' />
								</td>
								<td class="'.$thisSMName.'_text">
									<input type="text" name="'.$thisSMName.'_url" id="'.$thisSMName.'_url" style="'.$style.'width:220px" value="'.$thisSMurl.'" placeholder="Enter '.$thisSMName.' URL here..." class = "form-control"/>
								</td>
							</tr>';				
			}
		}
		$rsSMedia->free_result();	
		echo '<div class = "overlay-page"><div id="social_media_dialog">
				 <div class="form-group">
					<label for="recipient-name" class="col-form-label">Check the icons and specify the link to your social media page.</label>					
				  </div>
				 <div class="form-group">
		...
		<div class="message_button">
			<button type="button" class="btn btn-primary" onclick="addSM();">Submit</button>				
			</div>
		</div>';		
	}
	...
	function get_theme_data_for_ajax($category_id=0){
		...
		$template_str.="<div class='container'>
<div class = 'overlay-page'><div class='form-group' style = 'border-bottom:solid 1px #CCC;padding-bottom:5px;margin-bottom:5px;'> 
		                <label class='col-form-label'>Select Campaign Header:</label> ";
		$template_str.="<select onchange='changeHeader(\"change\")' id='category_list' class = 'form-control'>";
		...
		foreach($campaign_data['template_data'] as $key=>$theme_data){
			$template_str.='<div  class="div_template"><div class="row"><div class="col-lg-12"><ul class="themes_ul">';
			...
			$template_str.='</ul></div></div></div></div></div>';
		}
		echo  $template_str;
	}
	// Function to display all the image  bank images
	function used_image_bnk_img($imgid){
		$this->db->query("update `red_image_bank` set `last_used_date` = current_timestamp where img_id='$imgid'");
	}
	
	function get_image_bank_for_ajax($section=''){
		//Count number of image bank
		...
		if($section == 'image_bank_cards'){ 
			$style = 'style = "column-count: 2"'; $rel = ''; $heading = '';
			$imgStyle = 'style="width:105px;border-radius:5px; padding:3px; border:solid 1px #EBEBEB;"';
		}else{ 
			$style = ''; $rel = "rel= 'image_gallery'"; $heading = '<div class= "sectionHeading">Click the image from the your library to place it.</div>'; 
			$imgStyle = "";
		}		
		$image_bank_str='<div class="container '.$section.'">'.$heading.'
  <div class="card-columns" '.$style.'>';
		$j=1;
		if(count($image_bank_data)>0){
			for($i=0;$i<count($image_bank_data);$i++){
				$img_id= $image_bank_data[$i]['img_id'];
				...
				if(file_exists(str_replace(base_url().'asset/user_files/'.$this->session->userdata('member_id').'/',$this->upload_path.'/', $img_path))){
					...
					$image_bank_img=' <div class="image_bank_div2"><img class="image_bank draggable1"  src="'.$thumb_image_path.'" rel="'.$img_id.'" name="'.$img_path.','.$width.','.$height.'" '.$imgStyle.'></div>';
					$class="class='card' ";
					$img_remove='<div  class="del_image_link2"><a href="javascript:void(0);" '.$rel.' class="remove-img-link image_bank_unlink" style = "font-size:12px;padding:2px 5px;background:#C00;" id="'.$image_bank_data[$i]['img_id'].'" title="Delete Image"><i class = "fa fa-trash"></i></a></div>';
					$image_bank_str.='<div '.$class.'  title="Click to place"><div  class="img_slide2">'.$image_bank_img.$img_remove.$image_bank_links.'</div></div>';
					$j++;
				}//check for Image file exists or not
			}
		}else{
			$image_bank_str='<div class="load_images card">
								<b>Click "Upload Images" button to upload your images</b>
							</div>';
		}		
		echo $image_bank_str."</div></div>";
	}
	...
	function editorstyle(){
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_editor_style');
	}
	function editorjs(){
		$this->load->view($this->config->item('theme_folder').'promotions/campaign_editor_js');
	}
...
$response = file_get_contents($url, false, stream_context_create($arrContextOptions));
	...
	function user_address(){
		...
		echo $this->load->view($this->config->item('theme_folder').'promotions/user_address', array('campaign_data'=>$campaign_data),FALSE);
	}	
