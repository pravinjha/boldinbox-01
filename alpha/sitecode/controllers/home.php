...
{	
	...
	function index(){
		...
		$time_entered = gmdate('Y-m-d H:i:s', time());	
		...
		//$this->load->view($this->config->item('theme_folder').'header_outer',array('seo_array'=>$seo_array,'title'=>'Home Page', 'logoclass' => 'class="home"', 'wufoo_url'=>$wufoo_url));
		
		
		$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'BoldInbox.com: Simplest Ever Marketing Tool'));
		$this->load->view($this->config->item('theme_folder').'home/index');
		$this->load->view($this->config->item('theme_folder').'footer_outer');	
	}
	...
	function about(){	
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'About Us'));
		$this->load->view($this->config->item('theme_folder').'home/about');
		$this->load->view($this->config->item('theme_folder').'footer_outer');	
	}
	function contact(){
			if(isset($_POST)){
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'Contact Us | BoldInbox','show_bottom_bar'=>true));
		$this->load->view($this->config->item('theme_folder').'home/contactus',$data);
		$this->load->view($this->config->item('theme_folder').'footer_outer');	
	}	
	function features($str_for_seo='Manage-and-Grow-Your-Email-Lists',$category_id=8){
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',array('seo_array'=>$seo_array,'title'=>'Features-'.$product_data[0]['category']));		
		$this->load->view($this->config->item('theme_folder').'home/features',array('support_data'=>$support_data,'product_data'=>$product_data, 'selected_category_id'=>$category_id));
		$this->load->view($this->config->item('theme_folder').'footer_outer');		
	}
	function terms(){
		$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'Terms Condition'));
		$this->load->view($this->config->item('theme_folder').'home/terms-condition');
		$this->load->view($this->config->item('theme_folder').'footer_outer');	
	}
	function pricing(){
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',array('title'=>'Pricing: Email Marketing'));
		$this->load->view($this->config->item('theme_folder').'home/pricing',array('packages'=>$packages,'packages_yearly'=>$packages_yearly));
		$this->load->view($this->config->item('theme_folder').'footer_outer');
	}
	function support_OLD($str_for_seo='General',$category_id=1,$product_id=0){
		...
		$this->load->view($this->config->item('theme_folder').'header_outer',array('seo_array'=>$seo_array,'title'=>'Support-'.$product_data[0]['category']));		
		$this->load->view($this->config->item('theme_folder').'home/support',array('support_data'=>$support_data,'product_data'=>$product_data, 'selected_category_id'=>$category_id, 'selected_product_id'=>$product_id));
		$this->load->view($this->config->item('theme_folder').'footer_outer');		 
	}
	function support($str_for_seo='General',$category_id=1,$product_id=0){
		...
		$this->load->model('supportModel');
		$support_data=$this->supportModel->get_category_data(array('is_delete'=>0,'is_support'=>1));
		$product_data=$this->supportModel->get_category_productdata(array('rsp.category_id'=>$category_id, 'rsp.is_delete'=>0,'rsp.is_active'=>1));	
		$product_id = ($product_id > 0)?$product_id : $product_data[0]['id'];
		$this->load->view($this->config->item('theme_folder').'header_outer',array('seo_array'=>$seo_array,'title'=>'Support-'.$product_data[0]['category']));		
		$this->load->view($this->config->item('theme_folder').'home/support',array('support_data'=>$support_data,'product_data'=>$product_data, 'selected_category_id'=>$category_id, 'selected_product_id'=>$product_id));
		$this->load->view($this->config->item('theme_folder').'footer_outer');		 
	}
	...
	function search_result($start=0){  
	  ...
	  $this->pagination->initialize($config);
		...
		$messages=$this->messages->get();
		$this->load->view($this->config->item('theme_folder').'header_outer',array('seo_array'=>$seo_array,'title'=>'Support'));  
		$this->load->view($this->config->item('theme_folder').'home/support_search_result',array('product_data'=>$product_data,'paging_links'=>$paging_links,'search_text'=>$this->input->post('search_text')));
		$this->load->view($this->config->item('theme_folder').'footer_outer'); 
	}	
	...
	function getURL($uid){
		switch ($uid) {
			case 1:
				...
				break;
			case 2:
				return urlencode(base_url().'home/pricing');
				break;
			case 3:				
				return urlencode(base_url().'home/contact');
				break;
			default:
			   return urlencode(base_url().'user/register');
		}
	}	
	
	function tempTest(){
		$rsIsThisSent = $this->db->query("select email_sent from red_email_queue where campaign_id='15768' and subscriber_id='12345'");
...
The BoldInbox Team';
		...
		bib_temp('pravinjha@gmail.com', 'somya.ratnam@gmail.com', 'BoldInbox Support', 'Confirm your new "From Email"',nl2br($msg),$msg);
	}
