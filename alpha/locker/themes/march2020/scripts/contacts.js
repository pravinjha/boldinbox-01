...
$('.restrictIt').on('click', function(){bibAlert('This contact is inactive. You can not edit or delete it.');});
...
function deletelistConfrmed(lid){
	$('#message').html("<div class = 'alert alert-danger'>Please Wait...</div>");
	
	$.ajax({ url: base_url+"contacts/delete/"+lid, type:"POST", 
		success: function(data) {			
				showList();
				displayAlertMessageAdvance('Delete Contact List',data,'0','');
			}		
	});
}
...
$(document).on('click',".listdelete", function(event){
	var lid = $(this).attr('id').substr(5); 
	displayAlertMessageAdvance('List delete confirm',"Only list will be deleted while contacts can be found in \"All My Contacts\". Are you sure to do this?", "<button type='button' class='btn btn-primary'  onclick = 'deletelistConfrmed("+lid+");'>OK, Proceed</button>");	 
});
$(document).on('click',".listdelete11", function(event){
	var lid = $(this).attr('id').substr(5);
	...
	//displayAlertMessageAdvance('Delete List',$( document ).on('load', "#message", site_url+'contacts/delete/'+lid ),'0','');
	 
});


// ajax request for contacts-listing pg.
...
$("#btnSearch").on('click',function(e){alert('xyz');
e.preventDefault();
...
function createList(){	
	var addlistblk = '<form onsubmit="ajaxSubscriptionFrm(this); return(false);" method="post" class="form-website" id="subscriptionfrm"  name="subscriptionfrm">	<div class = "overlay-page"> <div class="alert alert-danger subscription_msg" role="alert" style = "display:none;"></div> <div class="form-group"><label>Enter List Name:</label> <input type="text" name="subscription_title" value="" id="subscription_title" maxlength="250" size="40" class="form-control"  />	</div> </div> <div class="message_button"><button type="button" class="btn btn-primary" onclick="javascript: $(\'#subscriptionfrm\').submit();">Submit</button></div></form>';
	//	displayAlertMessageAdvance('Create a new contacts list',$("#add-list").html(),'2', '');		 
		displayAlertMessageAdvance('Create a new contacts list', addlistblk, '2', '');		 
}
function ajaxSubscriptionFrm(){	
	$.ajax({
			url: base_url+"contacts/create/",
			type:"POST",
			data:"action=submit&"+'&subscription_title='+escape($('#subscription_title').val()),
			success: function(data) {
				var data_arr=data.split(":", 2);
				if(data_arr[0]=="error"){
					$('.subscription_msg').html(data_arr[1]).show();
				}else if(data_arr[0]=="success"){			
					$('#subscription_title').val('');
					showList();
					$('.subscription_msg').html("List created successfully.").show();			
				}
				setTimeout(function(){closeAlertMessage();} , 1000);			
			}
		});		
		return false;
}
...
function reinit() {
	$(".fancybox_delete").click(function(){				
		...
		$( "#message" ).on('load', $this.attr('href') ); return false; 
		$.modalBox.close();
	});
}
...
*/
$(".fancybox_delete").on('click',function(event){
	$('#action').val('');
	$('.check-boxalign').attr('checked','');
...
*/
$(".subscriber_edit").on('click',function(event){
