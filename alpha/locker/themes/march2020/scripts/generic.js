...
function bibAlert(message) {
	displayAlertMessageAdvance('Alert!',message,'0','');
}
function bibConfirm(message,fn) {
	// var confirmbtn = "<a href = 'javascript:void(0);' onclick='"+fn+"' >OK, Proceed</a>";
	var confirmbtn = "<button type='button' class='btn btn-primary' data-dismiss='modal' onclick = '"+fn+"'>OK, Proceed</button>";	
	//var cancelbtn = "<a href = 'javascript:void(0);' onclick='$.modalBox.close()' >Cancel</a>";
	displayAlertMessageAdvance('Please Confirm!',message,confirmbtn,'');
}
...
function displayAlertMessageAdvance(title,message,button,action) {
	...
	if(button != '0'){
		if(button != '1'){
			if(button == '2'){
				var button = jQuery('#messageBox .message_button').html();
				jQuery('#message_button').html(button);
				jQuery('#messageBox .message_button').html('');
			}else{
				jQuery('#message_button').html(button);
			}
		}
	...
	$('#messageBox').on('hidden.bs.modal', function (event) {
		if(action == ''){			
				//return false;
			}else if(action == 'self'){
...
function displayAlertMessage_COPY(title,message,button,timeOut,width,height,overflow,action) {
...
function modifyAlertMessage_COPY(title,message,button,timeOut,width,height,overflow,action) {	
	jQuery('#message_title').html(title);
	jQuery('#message').html(message);
...
setTimeout(function(){ 
		jQuery('#message_title').html('');
		jQuery('#message').html('');	
		jQuery('#message_button').html('');		
		jQuery('#messageBox').modal('hide'); 
	},
	500);
