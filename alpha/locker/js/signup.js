  ...
  function addCustomRow(type,friendlyName){
    ...
    var row = [
      '<tr id="custom',index,'" name="field_custom',index,'"><td class="add-sign-up-fields custom">',
      ...
      '<strong class="">',friendlyName,' Title:</strong>',
      '<input class="sign-up-field-custom custom_text" style="width:25px;" type="text" name="custom_field_name[]" placeholder="Enter field title" id="custom',index,'_field_name" onkeyup="display_form_field(\'custom'+index+'\',\'Custom\','+index+',\''+type+'\')"/>'];

        if (type !== "text" && type !== "date_dropdown" && type !== "textarea") {
          row.push('<strong class="options">Options: <span class="">(comma separated)</span></strong>');
          row.push('<input class="custom_text" type="text" name="custom_field_options[]" placeholder="Enter options ex. Basketball, Baseball, Soccer, Tennis" id="custom',index,'_field_name_options" onkeyup="display_form_field(\'custom'+index+'\',\'Custom\','+index+',\''+type+'\')"/>');
        }

        row.push('</td><td><a class="delete_custom" style="font-weight:bold;font-size:15px;" href="javascript:void(0);">X</a></td>');
        row.push('<td>Is Required? <br/><input type="checkbox" class="field_required_toggle required custom" /></td>');
  ...
  function addCustomRow(type,friendlyName){
    ...
    var row = [
      '<tr id="custom',index,'" name="field_custom',index,'"><td class="add-sign-up-field custom">',
      '<input type="hidden" name="custom_field[]" value="0" id="field_custom',index,'" />',
      '<input type="hidden" name="custom_field_required[]" value="0" id="field_custom',index,'_required" />',
      '<input type="hidden" name="custom_field_type[]" id="field_custom',index,'_type" value="',type,'" />',
      '<input type="hidden" value="1" id="add_custom',index,'" />',
      ...
      '<input class="sign-up-field-custom custom_text" type="text" name="custom_field_name[]" placeholder="Enter field title ex. Interests" id="custom',index,'_field_name" onkeyup="display_form_field(\'custom'+index+'\',\'Custom\','+index+',\''+type+'\')"/>'];
        ...
        row.push('</td><td><i class="icon icon-trash delete_custom">Del</i></td>');
        row.push('<td><input type="checkbox" class="field_required_toggle required custom" /></td>');

      row.push('</tr>');
    $('.signup-tbl-field-list').append(row.join(""));
  }
  */
  $("#add_custom_fld").live("change",function() {
    var type = $(this).val();
  ...
  /*$(document).delegate(".field_add_toggle","click",function(e) {
    var $this = $(this).parents().siblings(".add-sign-up-field");
...
$(document).delegate(".field_add_toggle","click",function(e) {
   ...
   var l = $(this).attr('data-language');  
	...
	if(undefined == $(this).attr("data-field")){
		...
		$('#'+left_block_chk_id).val(1);
		$('#'+left_block_chk_id).removeAttr("checked");
		// REQUIRED checkbox: uncheck & disable
		$("#field_" + field_name + "_required").removeAttr("checked");
	    $("#field_" + field_name + "_required").addClass("disabled");  
		display_form_field(dtfld,l);
	}else{
		// REQUIRED checkbox: activate
		$("#field_" + field_name + "_required").removeAttr("disabled");
		$("#field_" + field_name + "_required").removeClass("disabled");
	    $("#field_" + field_name + "_required").addClass("active");
		display_form_field($(this).attr("data-field"),l);
	}
  });
  ...
  $(document).delegate("input.field_required_toggle","click",function(e) {
    var $this = $(this);
    display_required_field($this.attr("data-field"),$this.attr("checked"), $this, e);
  });
  ...
  function updateLanguage(target) {
    ...
    jQuery.ajax({
        url: base_url+"subscription/signup_translate",
        type:"POST",
        data:{
          language: target,
...
function save_another_eml(){
  var newEml = encodeURIComponent($('#message').find('#another_emailid').val());
   
  $("#spin").show();
  jQuery.ajax({
    url: base_url+"campaign_email_setting/add_another_emailid/",
    type:"POST",
    data:'newEml='+newEml,
    success: function(data) { 
		$("#spin").hide();
      if(data =='err'){
        $('#message').find('#errInvalid').text('* Invalid Email or Common webmails are not allowed ');
      }else{
		displayAlertMessage('Add a New From Email','','0',true,500,150,false,'');
...
$("#spin").show();
    jQuery.ajax({
    url: base_url+"campaign_email_setting/ajaxFromEmlArray/",
    type:"POST",
    success: function(data) {
...
block_data+="&background_background_tile_image=0";
...
block_data+="&fld_sequence="+$('#form_preview').find('input').serialize();
    jQuery.ajax({
      url: base_url+"subscription/edit/"+sid+"/true/ajax/",
       type:"POST",
      data: block_data,
      success: function(data) { 
    $("#spin").hide();
  ...
  function submit_custom_message() {
    ...
    jQuery.ajax({
      url: base_url+"subscription/signup_custom_frm/"+sid,
       type:"POST",
      data:block_data,
      success: function(data) {
        var data_arr=data.split(":", 2);
  ...
  * Upload BG Images
  */
  jQuery('#btn_file').live('change',function(){ $("#add_btn").removeClass("loading"); });
  jQuery('#add_btn').live('click',function(){
    $("#spin").show();
    jQuery('#btn_file').upload(base_url+'ajax/signup_background_img/btn/', function(res) {
      var image_obj=res.split('|');
      if(image_obj[0] == 'noerr'){
        var img_path=image_obj[1];
        $('#btn_image').val(img_path);
        $('#btnSubmitForm').removeClass('submit_button');
        $('#btnSubmitForm').html('');
        $('#btnSubmitForm').children('img#btnImg').remove();
        $('#btnSubmitForm').append($('<img/>').attr({'src': img_path,'id':'btnImg','style':'width:100%;height:auto;'}));
      }else{
        if(image_obj[1]==1)bibAlert('<div class="signup-tbl-file-error"><i class="icon-warning-sign"></i>The uploaded file was too large, please upload a file that is less than 2 MB.</div>');
        else if(image_obj[1]==2)bibAlert('<div class="signup-tbl-file-error"><i class="icon-warning-sign"></i>Invalid file type. Please upload one of the following supported types: JPG, GIF and PNG.</div>');
        else if(image_obj[1]==3)bibAlert('<div class="signup-tbl-file-error"><i class="icon-warning-sign"></i>The file type you are attempting to upload is not allowed. Please upload one of the following supported types: JPG, GIF and PNG.</div>');
        else if(image_obj[1]==4)bibAlert('<div class="signup-tbl-file-error"><i class="icon-lightbulb"></i>File could not be uploaded, please try again later.</div>');
        else if(image_obj[1]==5)bibAlert('<div class="signup-tbl-file-error"><i class="icon-lightbulb"></i>File could not be uploaded, please try again later.</div>');
		//else bibAlert('<div class="signup-tbl-file-error"><i class="icon-lightbulb"></i>'+image_obj[1]+'</div>');
      }
  ...
  $("#spin").show();
  jQuery.ajax({
      url: base_url+"subscription/getAdvanced/",
      type:"POST",
      success: function(data) {
    
	displayAlertMessage('Advanced Settings','','0',true,900,250,false,'');
	$( "#message" ).html( data );
...
$(document).ready(function() {
  ...
  $('.delete_custom').live('click',function () {
    var $this = $(this);
	var fld = $(this).parent().parent().attr('id');
	$('#'+fld).remove();
	$('#form_preview').find('.'+fld+'_fld').remove();
    //delCustomField($this,$this);
  });

  $(".icon-trash1").live("click",function(e) {
