...
<script type="text/javascript" src="<?php echo $this->config->item('locker');?>js/signup.js?9-16-2014"></script>
...
<tr><td  valign="top"><b>Name: </b> <br/>
	...
	<td><b>Select a List:</b> <br/>
	 <input type="hidden" name="selectedSubscriptionValues" id="selectedSubscriptionValues" value="<?php echo $signup_from['form'][0]['subscription_id']; ?>"/>
              ...
              <?php } ?>
              </select><a href="javascript:void(0);"  onclick="javascript:$('.add_subscription').toggle();" class="btn cancel inline-block" style="padding: 3px 12px 4px;position: relative;bottom: 1px;font-size: 14px;">
                New List
              </a>
...
<tr><td>Fields</td><td>Required</td><td>Show</td></tr>
      ...
      // BASIC FIELDS
        foreach($arrSignupFormFieldLabels as $fldKey => $fldValue){
          if($fldKey != 'email'){
            ...
            $thisFldRecord = "<tr><td class='add-sign-up-field' id='update-language-{$fldKey}-data' data-field='{$fldKey}' data-language='".$this->lang->line($fldKey)."'>"   ;
            $thisFldRecord .= "<span id='{$fldKey}'>$fldValue</span></td><td>";
            ...
            if($thisRequired == 'Y') {
			  ...
			  $thisFldRecord .= "<input type='checkbox' class='field_required_toggle' name='fields[field_{$fldKey}_required]' value='0' id='field_{$fldKey}_required' $isDisabled />";              
            }
			$thisFldRecord .= "</td><td>";
			if($thisKey !== FALSE){$isChecked ='checked'; $val=1;}else{$isChecked =''; $val=0;}           
            $thisFldRecord .= "<input type='checkbox' value='$val' class='field_add_toggle' name='fields[is_{$fldKey}]' id='field_{$fldKey}' $isChecked  />";           
      ...
      // CUSTOME FIELDS PRE-POPULATED FROM DB
        ...
        ?>
          <tr><td id="custom<?php echo $i; ?>" class="add-sign-up-field custom active" name="field_custom<?php echo $i; ?>">
 ...
 <td>
          ...
          <div class="signup-tbl-inline preview">
            <label class="label" style="margin-bottom: 0">Preview Changes <a class="btn add inline-block signup-style-options" href="javascript:void(0);">Style Options <i class="icon-plus"></i></a></label>
