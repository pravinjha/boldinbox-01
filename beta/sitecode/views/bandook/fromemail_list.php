...
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo base_url().'bandook/fromemail/index/';?>">
...
if(count($fromemails)) {
	...
	<tr>
		<td width="20%">
			<a href="<?php echo  site_url("bandook/users_manage/users_list/".$thisFromemail['member_username']); ?>"><?php echo $thisFromemail['member_username'];  ?></a>
		</td>
		<td width="20%"> <?php echo $thisFromemail['email_address'];?> </td>
		<td width="20%"> <?php echo $thisFromemail['domain_reason'];?> </td>
	 
		<td>
		<a onclick="return confirm('Are you sure to verify this email')" href="<?php echo base_url() ?>bandook/fromemail/verifyit/<?php echo $thisFromemail['unique_string'];?>" > Verify	</a>
		&nbsp; | &nbsp;
		<a onclick="return confirm('Are you sure to disallow this email')" href="<?php echo base_url() ?>bandook/fromemail/disallow/<?php echo $thisFromemail['unique_string'];?>" > Disallow	</a>
		&nbsp; | &nbsp;		
		<a onclick="return confirm('Are you sure to remove this email')" href="<?php echo base_url() ?>bandook/fromemail/remove/<?php echo $thisFromemail['unique_string'];?>" > Remove	</a>
		</td>		
	</tr>
<?php }
	}else{
