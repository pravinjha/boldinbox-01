...
//---------------------------------Banner operation Starts
// 	Banner image height:Start	

	$retval = '';
foreach($html_strp->find('img.header_img') as $e){
		 $thisBannerBlockID = $e->lang;
		 $header_img=$e->src;
		...
		list($width_orig, $height_orig,$type_header) = getimagesize(str_replace(base_url().'locker/',config_item('locker_path'),$header_img));
	...
	// Merge Logo image and header image
		foreach($html_strp->find('table#'.$thisBannerBlockID) as $blok){
			foreach($blok->find('.banner_logo') as $e){ 
				...
				$logo_img="";
				foreach($blok->find('#logo_'.$thisBannerBlockID) as $e){
					$logo_img		= $e->src;			
					$logo_img_width	= $e->getAttribute('style') ;
				}
				 
				foreach($blok->find('.header_img') as $e){
					$header_img= $e->src  ;
				...
				*/
				...
				$feedBurnerStatsSource =$upload_path.$user_dir.'/'.$CI->session->userdata('member_id').'/'.$campaign_folder.'/'.$campaign_id.'/'.$thisBannerBlockID.'_logo.'.$logo_ext;
				...
				*/
				...
				$filename= str_replace(base_url().'locker/',config_item('locker_path'),$header_img);
				...
				$backgroundSource = $upload_path.$user_dir.'/'.$CI->session->userdata('member_id').'/'.$campaign_folder.'/'.$campaign_id.'/'.$thisBannerBlockID."_header.".$banner_ext; 
				...
				*/
				...
				if($type_header==1){
					imagegif($outputImage,$upload_path.$user_dir.'/'.$CI->session->userdata('member_id').'/'.$campaign_folder.'/'.$campaign_id.'/'.$thisBannerBlockID.'.gif');
					$header_img=base_url().'asset/user_files/'.$campaign_type_url.'/'.$campaign_id.'/'.$thisBannerBlockID.'.gif';
				}else if($type_header==2){
					imagejpeg($outputImage,$upload_path.$user_dir.'/'.$CI->session->userdata('member_id').'/'.$campaign_folder.'/'.$campaign_id.'/'.$thisBannerBlockID.'.jpg');
					$header_img=base_url().'asset/user_files/'.$campaign_type_url.'/'.$campaign_id.'/'.$thisBannerBlockID.'.jpg';
				}else if($type_header==3){
					imagepng($outputImage,$upload_path.$user_dir.'/'.$CI->session->userdata('member_id').'/'.$campaign_folder.'/'.$campaign_id.'/'.$thisBannerBlockID.'.png');
					$header_img=base_url().'asset/user_files/'.$campaign_type_url.'/'.$campaign_id.'/'.$thisBannerBlockID.'.png';
				}
			
		//$retval .= $header_img;
				foreach($blok->find('.header_img') as $i){
					$i->src=$header_img;
				} 
	...
	foreach($html_strp->find('table#'.$thisBannerBlockID) as $e){
		foreach($e->find('.header_link_show') as $i){
			$header_link=$i->name;
			
			if($header_link!=''){
				foreach($e->find('.banner_div') as $d){
					
					$d->innertext = "<a href='".$header_link."' border='0' target='_blank'>".$d->innertext."</a>"; 
					
				}				
			}
		}
		foreach($e->find('.header_link_show') as $rem){
			$rem->outertext = "";
		}	
	}

}	// 	Banner image height:Ends


// echo $retval;exit;

//---------------------------------Banner operation ENDS
	...
	foreach($html_strp->find('div.text-paragraph-container') as $e){		
		...
		$style.=";border:2px solid transparent;font-family:Arial,Helvetica,sans-serif;color:#333333;font-size:14px; padding-top:10px;padding-bottom:10px; padding-left:15px;padding-right:15px;";
	...
	foreach($html_strp->find('table.youtube_container') as $tbl_html){
		...
		// get video url
				foreach($tbl_html->find('img.image-container') as $e){
					...
					imagecopymerge_alpha($local_img,imagecreatefrompng(config_item('locker_path').'images/play.png'),($video_width-58)/2,($video_height-41)/2,0,0,58,41,100);
	...
	foreach($html_strp->find('.divider_block') as $td_divider){
		$style=$td_divider->getAttribute('style');
		$td_divider->setAttribute("style",$style.'padding-top:5px;padding-bottom:5px;');
	}
	//HEader Link
	...
	foreach($html_strp->find('td#body_main') as $e){
		...
		foreach($style_arr as $style_element){
			$arr_style_element = explode(":",$style_element);
			if( trim($arr_style_element[0]) == 'background-color')$campaign_body_bg_color = trim($arr_style_element[1]);
		}		
	...
	foreach($html_strp->find('.button_block') as $e){		
		$e->setAttribute("href",$e->getAttribute('title')); 
	}
	$i=1;
	foreach($html_strp->find('table.container-div') as $e){
		
		
		$style="  border:none;clear:both; width: 100%;";
		...
		if('container-div preheader'==$e->getAttribute("class")){
			...
			*/
			...
			if('header_banner container-div' != $e->getAttribute("class")){
				$this_block_bg_color = '';
				$thiStyle=$e->getAttribute('style');
				$style_arr=explode(";",$thiStyle);	
				foreach($style_arr as $style_element){
					$arr_style_element = explode(":",$style_element);
					if( trim($arr_style_element[0]) == 'background-color')$this_block_bg_color = trim($arr_style_element[1]);
				}	
				if($this_block_bg_color == '')$this_block_bg_color = $campaign_body_bg_color;
			 
				$style .=" width: 100%; background-color:".$this_block_bg_color."; border-top-color:".$this_block_bg_color.";border-bottom-color:".$this_block_bg_color."; border-top-style: solid;  border-bottom-style: solid;";
				//$style .="border-left-color:".$this_block_bg_color."; border-left-style: solid;border-left-width: 20px;border-right-color:".$this_block_bg_color."; border-right-style: solid;border-right-width: 20px;";		
			} 
			
			if('container-div table_block' == $e->getAttribute("class")){
				$style .="border-left-color:".$this_block_bg_color."; border-left-style: solid;border-left-width: 20px;border-right-color:".$this_block_bg_color."; border-right-style: solid;border-right-width: 20px;";		
			}
			
			// to debug
			//return ($e->getAttribute("class").'====='.$thiStyle);exit;
			if($i==1){
	...
	foreach($html_strp->find('a') as $a) {
		if($a->href) {
			if (strtolower(substr($a->href, 0, 7)) != 'mailto:' && strtolower(substr($a->href, 0, 6)) != 'skype:') {
				...
				if($autoresponder){
					$url=base_url().'clickrate/create_autoresponder/'.$campaign_id.'/[scheduled_id]/[subscriber_id]/'.$url_key;
				}else{
					$url=base_url().'clickrate/create/'.$campaign_id.'/[subscriber_id]/'.$url_key;
				}
				$a->href= trim($url);
			}
		}
