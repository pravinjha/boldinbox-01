...
class Cprocess extends CI_Controller
{
	...
	function abuse($id=''){		 	
		...
		if(intval($subscriber_id) > 0 && intval($cid) > 0){		 
			...
			if(count($email_report_unsubscribe) > 0){
				...
				$this->Subscriber_Model->update_subscriber(array('subscriber_status'=>2, 'status_change_date'=>$utc_str, 'complaint'=>1,'last_complaint_date'=>$utc_str),array('subscriber_id'=>$subscriber_id));
				// To debug auto-unsubscribe				
				if($subscriber_created_by ==174){
					$abuse_query = $this->db->last_query();	
					$mIP = $this->is_authorized->getRealIpAddr();
					$abuse_query = $abuse_query . ' via IP='.$mIP;
					admin_notification_send_email('pravinjha@gmail.com', SYSTEM_EMAIL_FROM,"BoldInbox", 'Debug unsubscribe',"$abuse_query","$abuse_query");
				}
				// Mark email as read				
	...
	*/
	...
	function unsubscribe($enc_cid=0,$subscriber_id=0){
		...
		if($subscriber_id==0){					
			...
			$this->Subscriber_Model->update_subscriber(array('subscriber_status'=>0, 'status_change_date'=>date('Y-m-d H:i:s',now()), 'unsubscribed'=>1,'last_unsubscribed_date'=>date('Y-m-d H:i:s',now())),array('subscriber_id'=>$subscriber_id, 'subscriber_email_address'=>$subscriber_email));
			// To Debug
			if($user_id == 174){
				$abuse_query = $this->db->last_query();	
				$mIP = $this->is_authorized->getRealIpAddr();
	...
	*/
	...
	/**
		...
		@param int subscriber_id contain subscriber id
	*/
	function opened_0($enc_cid=0,$subscriber_id=0){
	// to check
		$this->db->query("insert into atest set is_opened=5");
