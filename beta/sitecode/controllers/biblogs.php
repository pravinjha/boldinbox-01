...
class Biblogs extends CI_Controller
{
				...
				if(count($extra_header_arr) == 3 and $extra_header_arr[2]==CAMPAIGN_HEADER_SUFFIX){
				 ...
				 $envId = $extra_header_arr[1];
					...
					if(count($pmta_log_array)>0){						
						...
						// Update subscriber						
						$this->Subscriber_Model->update_subscriber(array('ignore'=>1,'subscrber_bounce'=>$subscrber_bounce,'subscriber_status'=>$subscriber_status,'soft_bounce'=>$soft_bounce, 'bounce_status'=>$bounce_status, 'bounce_detail'=>$bounce_detail, 'status_change_date'=>$utc_str,'bounced'=>$soft_bounce,'last_bounced_date'=>$utc_str),array('subscriber_id'=>$jobId));
	...
	*/
	function notifyPaymentAlert(){	
		...
		$to = $this->confg_arr['admin_notification_email'];						
		admin_notification_send_email($to, SYSTEM_EMAIL_FROM,'BoldInbox', "Member Payment Detail",$strTable,strip_tags($strTable));	
	}
