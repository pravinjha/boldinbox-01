...
class Manage_messages extends CI_Controller{
	...
	*/	
	function index(){
		...
		$this->load->view('bandook/manage_messages',array('message_list'=>$message_data_array));	 
	...
	function message_edit($message_id){
		...
		if($this->input->post('action')=='submit'){
			...
			if($this->form_validation->run()) {
				...
				redirect('bandook/manage_messages');
		...
		$this->load->view('bandook/header',array('title'=>'Manage Message','logo_link'=>$logo_link));
		$this->load->view('bandook/message_edit',array('message_detail'=>$message_data[0],'messages' =>$messages));
		$this->load->view('bandook/footer');
	} 
	function message_create(){
		...
		$this->load->view('bandook/message_add',array('message_detail'=>$message_data[0],'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function member_message($mid=1){
		...
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>$logo_link));
		$this->load->view('bandook/member_message',array('mid'=>$mid, 'member_message_list'=>$member_message_data, 'message_list'=>$message_data_array));	 
		$this->load->view('bandook/footer');
	}
	...
	function assign_message(){
		...
		if($this->input->post('action')=='Submit'){
			...
			if($this->form_validation->run()) {			
				...
				redirect('bandook/manage_messages/member_message');				 
			}
		}
	}
