...
class Blog_listing_post extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/BlogPost_Model');
	...
	function post_list($cat_id,$start=0)
	{
		...
		$config['base_url']=base_url().'index.php/bandook/blog_listing_post/post_list/'.$cat_id;
		...
		$this->load->view('bandook/header',array('title'=>'Manage Listing Post','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_post_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'cat_id'=>$cat_id,'status'=>1,'page'=>$start));
	...
	function post_list_status($cat_id,$status,$start=0){
		...
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'index.php/bandook/blog_listing_post/post_list_status/'.$cat_id.'/'.$status;
		$config['total_rows']=$this->BlogPost_Model->get_category_count($fetch_conditions_array);
		$config['per_page']=10;
		...
		//Loads header, category listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Manage Listing Post','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_post_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'cat_id'=>$cat_id,'status'=>$status));
		$this->load->view('bandook/footer');
	}
	...
	function post_list_archive($cat_id,$archive,$start=0){
		...
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'index.php/bandook/blog_listing_post/post_list_archive/'.$cat_id.'/'.$archive;
		$config['total_rows']=$this->BlogPost_Model->get_category_count($fetch_conditions_array);
		$config['per_page']=10;
		$config['uri_segment']=6;
		...
		//Loads header, category listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Manage Listing Post','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_post_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'cat_id'=>$cat_id,'archive'=>1));
		$this->load->view('bandook/footer');
	}
	...
	function post_edit($cat_id,$id){
		...
		if($this->input->post('action')=='submit'){
			...
			if($this->form_validation->run()) {
				...
				if(count($category_exists)){
					...
					redirect('bandook/blog_listing_post/post_list/'.$cat_id);
				}
			}
		...
		$this->load->view('bandook/header',array('title'=>'Update Category','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_post_edit',array('category'=>$category_data_array,'messages' =>$messages,'cat_id'=>$cat_id));
		$this->load->view('bandook/footer');
	}
	...
	function mark_restore($cat_id,$id){
		...
		// Redirect to listing of categories
		redirect('bandook/blog_listing_post/post_list_archive/'.$cat_id.'/1');
	}
	...
	function mark_active($cat_id,$id){
		...
		// Redirect to listing of categories
		redirect('bandook/blog_listing_post/post_list_status/'.$cat_id.'/0');
	}
	...
	function post_create($cat_id,$id=0){
		...
		$this->load->view('bandook/header',array('title'=>'Create Listing Category','logo_link'=>$logo_link));
		$this->load->view('bandook/blog_post_create',array('category'=>$category_data_array,'messages' =>$messages,'cat_id'=>$cat_id));
		$this->load->view('bandook/footer');
	}
}
?>