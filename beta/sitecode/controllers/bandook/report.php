	...
	function showIPR($typ=''){
		...
		if($this->input->post('btn_search')=='Search'){
			...
			if($clausePipeline ==''){ 
				...
				$sqlIPR = "select Case WHEN (pipeline='pmta-pool-1' or pipeline='pmta-vmta1' or pipeline='pmta-vmta2' or pipeline='pmta-vmta3') then 'pmta-pool-1' WHEN (pipeline='pmta-pool-2' or pipeline='pmta-vmta4' or pipeline='pmta-vmta5' or pipeline='pmta-vmta6') then 'pmta-pool-2' WHEN (pipeline='pmta2-pool-1' or pipeline='pmta2-vmta1' or pipeline='pmta2-vmta2' or pipeline='pmta2-vmta3' or pipeline='pmta2-vmta6' or pipeline='pmta2-vmta7') then 'pmta2-pool-1' WHEN (pipeline='pmta2-pool-2' or pipeline='pmta2-vmta4' or pipeline='pmta2-vmta5' or pipeline='pmta2-vmta8' or pipeline='pmta2-vmta9' or pipeline='pmta2-vmta10') then 'pmta2-pool-2' END as vmta, mail_domain, Case WHEN(mail_domain='gmail.com')then 1 WHEN(mail_domain='yahoo.com')then 2 WHEN(mail_domain='hotmail.com')then 3 WHEN(mail_domain='aol.com')then 4 WHEN(mail_domain='msn.com')then 5 WHEN(mail_domain='outlook.com')then 6 WHEN(mail_domain='windowslive.com')then 7 WHEN(mail_domain='live.com')then 8 WHEN(mail_domain='mail.ru')then 9 WHEN(mail_domain='me.com')then 10 WHEN(mail_domain='mac.com')then 11 WHEN(mail_domain='comcast.net')then 12 WHEN(mail_domain='cox.net')then 13  WHEN(mail_domain='rediffmail.com')then 14 END as ord, sum(total_sent)total_sent,  sum(total_released)total_released, sum(total_delivered)total_delivered, sum(total_opened)total_opened, sum(total_bounced)total_bounced, sum(total_unsubscribed)total_unsubscribed, sum(total_complaint)total_complaint from red_global_ipr_daily where CONVERT_TZ( `log_date`, '+00:00','+5:30' ) > '$date_from 23:59:59' and  CONVERT_TZ( `log_date`, '+00:00','+5:30' ) < '$date_to 23:59:59' $clausePipeline $clauseUsers  group by mail_domain,vmta order by ord";					
			}
...
//		$sqlPaidUsers ="select m.member_id,m.member_username, m.vmta, date_format(m.created_on,'%Y-%m-%d')created_on, m.is_authentic, date_format(m.authenticated_on,'%Y-%m-%d')authenticated_on, m.is_risky, m.apply_unresponsive_filter, m.unresponsive_release_count, m.member_dnm, m.member_unresponsive, mp.package_id, mp.amount, mp.start_payment_date, mp.next_payement_date, mp.user_quota_multiplier multiplier, mp.max_campaign_quota, mp.campaign_sent_counter, mp.coupon_code_used from red_members m inner join red_member_packages mp on m.member_id=mp.member_id where mp.package_id >0 and mp.next_payement_date > now() order by created_on";
	...
	function remove($id=''){
		if($id != ''){
			echo $sid = @explode('-', base64_decode($id))[0];
			$this->db->query("update red_email_subscribers set subscriber_status='0' where subscriber_id='$sid'");
			echo "<br/>";
			echo ($this->db->affected_rows())?'removed': 'already unsubscribed'; 
		}	
	}
}
?>