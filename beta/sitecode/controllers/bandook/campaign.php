		...
		if($this->input->post('action')=='submit'){	
			...
			if($this->input->post('all')=='All'){
				if($update_campaign_status_show == 4){
					...
					$updateCampaignArr = array('campaign_status_show'=>$update_campaign_status_show,'is_segmentation'=>'0','number_of_contacts'=>0, 'campaign_delay_minute'=> $delay,'email_send_date'=>$date);
				}else{
					$updateCampaignArr = array('campaign_status_show'=>$update_campaign_status_show,'is_segmentation'=>'0','number_of_contacts'=>0, 'campaign_delay_minute'=> $delay);				
				}
				...
				if($this->form_validation->run()==true){
					...
					if($this->input->post('automate')=='automate'){	
						...
						*/
						$this->Campaign_Model->update_campaign(array('campaign_status_show'=>$update_campaign_status_show,'is_segmentation'=>'1','number_of_contacts'=>$segment_size, 'segment_interval'=>$segment_interval, 'campaign_delay_minute'=>$delay),array('campaign_id'=>$campaign_id));
						...
						$this->Campaign_Model->update_campaign(array('campaign_status_show'=>$update_campaign_status_show,'is_segmentation'=>'1','number_of_contacts'=>$segment_size, 'campaign_delay_minute'=> $delay),array('campaign_id'=>$campaign_id));						
	...
	**/
	...
	function global_ipr($interval=0){	
		...
		foreach($arrDomainsToShow as $dmainname){ 
			...
			$deliveredPercent=($thisDomainSent > 0)? number_format($thisDomainDelivered/$thisDomainSent *100 , 2) : 0;
			$openedPercent =($thisDomainDelivered > 0)? number_format($thisDomainOpened/$thisDomainDelivered *100 , 2) : 0;
			$bouncedPercent =($thisDomainDelivered > 0)? number_format($thisDomainBounced/$thisDomainDelivered *100 , 2) : 0;
			$complaintPercent =($thisDomainDelivered > 0)? number_format($thisDomainComplaints/$thisDomainDelivered *100 , 2) : 0;
			$unsubscribePercent =($thisDomainDelivered > 0)? number_format($thisDomainUnsubscribes/$thisDomainDelivered *100 , 2) : 0;
			$strTblBody .= '<tr bgcolor="#ffffff"><td>'.$dmainname.'</td>';
			$strTblBody .= '<td>'.$thisDomainSent.'</td>';
			$strTblBody .= '<td>'.$thisDomainDelivered.'('.$deliveredPercent.'%)</td>';
			$strTblBody .= '<td>'.$thisDomainOpened.'('. $openedPercent.'%)</td>';
			$strTblBody .= '<td>'.$thisDomainBounced.'('. $bouncedPercent.'%)</td>';
			$strTblBody .= '<td>'.$thisDomainComplaints.'('. $complaintPercent.'%)</td>';
			$strTblBody .= '<td>'.$thisDomainUnsubscribes.'('. $unsubscribePercent .'%)</td></tr>';	
		...
		echo "<span style='float:right;font-size:12px;'><a href='/bandook/campaign/global_ipr/".(1+$interval)."'>Show data for ".date( 'Y-m-d', strtotime( date("Y-m-d", gmmktime()) . " " .(-1-$interval) ."  day" ) )."</a></span>";
		echo '</div>';
