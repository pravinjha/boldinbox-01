...
class Subscriber extends CI_Controller
{
		...
		if(count($subscriber_data['subscribers'])){
			...
			foreach ($subscriber_data['subscribers'] as $subscriber){
				...
				if($subscriber['subscriber_name']!=""){
					...
					$subscriber['subscriber_first_name']=($subscriber['subscriber_first_name']=="")?$name_arr[0]:$subscriber['subscriber_first_name'];
					$subscriber['subscriber_last_name']=($subscriber['subscriber_last_name']=="")?$name_arr[0]:$subscriber['subscriber_last_name'];
				}
 ...
 /**
	...
	function exportcsv($subscription_id=0, $search_key=''){	
		...
		$id = (is_numeric($subscription_id))?$subscription_id : 0;
		$mid = $this->session->userdata('member_id');
		if($id ==0  or $this->session->userdata('manage_contacts') < 0){
