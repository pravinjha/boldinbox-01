...
class Subscription extends CI_Controller{
	...
	*/
	function display(){
		...
		$this->load->view('header',array('title'=>'Subscription Forms', 'contactDetail'=>$contactDetail));
		$this->load->view('subscription/signup_list',array('signup_froms'=>$signup_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
...
";
			...
			*/
			...
			if($type!="ajax"){
				...
				#Get shoreten url
				$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id'));
				$this->load->view('header',array('title'=>'Add/Update Subscription Form', 'contactDetail'=>$contactDetail));				
				$this->load->view('subscription/signup_edit',array('signup_from'=>$signup_data_array, 'signup_froms_language'=>$arrLanguage, 'messages' =>$messages,'shorten_url'=>$shorten_url));
				$this->load->view('footer');
			}else{
				echo '1|'.$signup_id;
			}
	...
	function signup_form_stats($form_id=0){
		...
		$str_stats ='<ul>
                      <li>
                        <span>Visits</span>
                        <h5>'.$visit_count.'</h5>
                        <!-- a href="'.site_url("subscription/stats_detail/view/$form_id/").'" class="report-view-list"><i class="icon-search"></i></a -->
                      </li>
                      <li>
                        <span>Signups</span>
                        <h5>'.$confirmation_count.'</h5>';
		if($confirmation_count > 0){				
						$str_stats .='<a href="'.site_url("subscription/stats_detail/$form_id").'" class="report-view-list"><i class="icon-search"></i></a>';
		}				
		$str_stats .='</li>
                      <li>
