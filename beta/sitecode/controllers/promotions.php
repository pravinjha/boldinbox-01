...
class Promotions extends CI_Controller
{
	...
	*/
	...
	function validate_upload(){		
		...
		if(!$this->upload->do_upload('campaign_import_zip_file')){ 
			...
			$this->upload_path.'/extracted_zip_files/'.$new_file_name.'/';
			...
			if($extracted_file!=''){
				...
				//$filtered_html=preg_replace('#(href|src)s?=s?(["\'])([.\/]*)([^:"\']*)(["\'])#',	'$1="'.$path_to_images.'/$4"',$html); // replace path to images and css
	...
	*/	
			...
			if($elem!='id' && $elem!='theme_name'){			
				...
				if($arr_elem[1]=="bg"){
					...
					$style.="<style class='border_style custome_style'>
						#email_template_table{border-color:  $color !important;}
	...
	//Function to show all the email template screenshots
	function get_theme_data_for_ajax($category_id=0,$bid='header'){
		...
		foreach($campaign_data['template_data'] as $key=>$theme_data){
			...
			foreach($theme_data as $theme){
				...
				$template_img='<img onclick="saveHeader(\''.$template_id.'\',\''.$bid.'\')" src="'.$template_img_path.'" border="0" />';
				$template_str.='<li '.$class.' >'.$template_img.$template_links.'</li>';
				$j++;
			}
			$template_str.='</ul></div>';
