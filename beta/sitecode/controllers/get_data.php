...
class Get_data extends CI_Controller {
	...
	function show_image($mid, $fname){
		...
		if(!file_exists($image_data)) $image_data =  $this->config->item('locker_path').'images/pix.gif'	 ;
	...
	// archived on 13/Jan/2016 for  http://www.beonlist.com/c/114146
	...
	function show_campaign_header($is_autoresponder='c',$cid=0,$fname=''){ 
		$userId = $this->getUserIDFromHeader($cid, $is_autoresponder);
		$user_dir = $userId % 1000;
		 
		if($is_autoresponder == 'c')
		$image_data= rawurldecode($this->config->item('user_public').$user_dir.'/'.$userId.'/email_templates/'.$cid.'/'.$fname); 
		else
		$image_data= rawurldecode($this->config->item('user_public').$user_dir.'/'.$userId.'/autoresponders/'.(int)$cid.'/'.$fname); 
	#	die($image_data);
		if(!file_exists($image_data)) $image_data =  $this->config->item('locker_path').'images/pix.gif'	 ;	 
		$imginfo = getimagesize($image_data);
