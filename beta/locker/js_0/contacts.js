...
function displayRemoved(){
	...
	$('#delete_subscriber').attr('href',base_url+'subscriber/subscriber_delete/-'+memid);
...
function openSubscriptionForm(){
	if( $('#subscription_menu').is(':hidden') ){
		...
		//displayAlertMessage('Overlay Title',msg,'0',true,400,250,false,'');
		 
		$('.subscription_msg').html("");
		$('.drop_img').attr('src',locker+"images/down.png?v=6-20-13");
		...
		$('.drop_img').attr('src',locker+"images/login-up.png?v=6-20-13");
	}
	...
	//var frm=document.frmLogin;
		...
		block_data+="action=submit&"+'&subscription_title='+escape(frm.subscription_title.value);
		  ...
		  if(data_arr[0]=="error"){
			...
			//jQuery('#messageBox').fadeOut(2000);
