...
function ajaxSubscriberFrm(frm, action) {
  ...
  if (action == "copycsv") {
    ...
    $('#delete_subscriber').attr('href', base_url + 'subscriber/subscriber_delete/' + selected_list_id);
    ...
    //$('#delete_subscriber').attr('href',base_url+'subscriber/subscriber_delete/'+document.form1.subscription_selected_id.value);
  }
  if (send_request) {
    $.blockUI({
    ...
    $.ajax({
      url: base_url + "subscriber/create/" + selected_list_id,
      type: "POST",
      data: block_data,
      success: function(data){
        ...
        if (data_arr[0] == "error") {
          ...
          $('.subscriber_msg').addClass('info');
          window.location.href = base_url + 'contacts/index/' + selected_list_id;
...
function ajaxFileUpload() {
  ...
  $.ajaxFileUpload({
    url: base_url + 'subscriber/importcsv/' + $('#subscription_select').val() + '/1',
    secureuri: false,
    ...
    success: function(data, status) {
      ...
      if (typeof(data.error) != 'undefined') {
        if (data.error != '') {
          ...
          window.location.href = base_url + 'bandook/contacts_segmentation/index/'+memid;
        }
      }
    }
  });
