...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : base_url+'/locker/images/campaignBuilderToolbarIcons2.png?v=1.01',
	buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','fontFormat','insertHTML','indent','outdent','image','upload','link','unlink','forecolor','bgcolor','strikethrough'],
	//iconList : {"bgcolor":1,"forecolor":2,"bold":3,"center":4,"hr":6,"indent":14,"italic":7,"justify":8,"left":5,"ol":10,"outdent":12,"removeformat":13,"right":9,"save":24,"strikethrough":12,"subscript":13,"superscript":14,"ul":15,"underline":16,"link":18,"unlink":19,"close":27,"arrow":25,"xhtml":1,"undo":20,"redo":21}
	iconList : {"undo":1,"redo":2,"bold":3,"underline":4,"italic":5,"strikethrough":6,"subscript":7,"superscript":8,"left":9,"center":10,"right":11,"justify":12,"ol":13,"ul":14,"link":15,"unlink":16,"forecolor":17,"xhtml":18}
...
var nicEditor = bkClass.extend({
	...
	nicCommand : function(cmd,args) {
		if(this.selectedInstance) {
			...
			if (cmd === "insertHTML" && args.indexOf("{") === 0 && this.selectedInstance.e.textContent.length) {
				args = " " + args;
	...
	getIcon : function(iconName,options) {
		...
		return {backgroundImage : "url('"+((icon) ? this.options.iconsPath : file)+"')", backgroundPosition : ((icon) ? ((icon-1)*-75) : 0)+'px 0px'};
	},
...
var nicEditorInstance = bkClass.extend({
    ...
    pasteClean : function() {
        ...
        newSnippet = newSnippet.replace(/<b(\s+|>)/g, "<strong $1");
		newSnippet = newSnippet.replace(/<\/b(\s+|>)/g, "</strong>");	
        ...
        newSnippet = newSnippet.replace(/<br class="Apple-interchange-newline">/, "");
...
var nicEditorPanel = bkClass.extend({
	construct : function(e,options,nicEditor) {
		...
		this.panelContain = new bkElement('DIV').setStyle({overflow:"hidden",width:"auto",border:"1px solid #cccccc",backgroundColor:"#ccc",position:'fixed','top':'0px',left:'50%',marginLeft:'-255px','text-align':'center'}).addClass('panelContain');
...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('DIV').setStyle({width : '74px', height : '30px'}).addClass('buttonContain').appendTo(this.margin);
		this.border = new bkElement('DIV').setStyle({backgroundColor : '#efefef'}).appendTo(this.contain);
		this.button = new bkElement('DIV').setStyle({width : '74px', height : '30px', overflow : 'hidden', zoom : 1, cursor : 'pointer'}).addClass('button').setStyle(this.ne.getIcon(buttonName,options)).appendTo(this.border);
...
posleft = '50px';
		...
		var cnt_width;
		var cnt_left;
		if(this.pos[0] == '698'){ cnt_height = '160px'; cnt_width = '370px'; cnt_left = '600px';}
		if(this.pos[0] == '846'){ cnt_height = '228px'; cnt_width = '260px'; cnt_left = '700px'; }
		if(this.pos[0] == '920'){ cnt_height = '350px'; cnt_width = '925px'; cnt_left = '250px';}
cnt_width='520px';
		this.contain = new bkElement('div').setStyle({zIndex : '99999', overflow : 'visible', 'position' : 'absolute', 'left' : posleft, 'top' : this.pos[1]+'px'})
		this.pane = new bkElement('div').setStyle({fontSize : '12px', height:cnt_height,width:cnt_width,left:cnt_left,border : 'solid 5px #1C4587', overflow: 'hidden', padding : '0px', textAlign: 'left', backgroundColor : '#ffffc9'}).addClass('pane').setStyle(options).appendTo(this.contain);

		if(openButton && !openButton.options.noClose) {
			// this.close = new bkElement('div').setStyle({'float' : 'right', height: '29px', width : '74px', cursor : 'pointer', position : 'relative', top : '6px', right : '7px'}).setStyle(this.ne.getIcon('close',nicPaneOptions)).addEvent('mousedown',openButton.removePane.closure(this)).appendTo(this.pane);
			this.close = new bkElement('div').setStyle({'float' : 'right', height: '29px', width : '74px', cursor : 'pointer', position : 'relative', top : '0px', right : '-20px', backgroundImage: "url("+base_url+"/locker/images/icons/overlay_close.png)", backgroundRepeat:'no-repeat'}).addEvent('mousedown',openButton.removePane.closure(this)).appendTo(this.pane);
		}
	...
	mouseClick : function() {		
		if(!this.isDisabled) {
			if(this.pane && this.pane.pane) {
				...
				this.pane = new nicEditorPane(this.contain,this.ne,{backgroundColor : '#fff',position:'fixed'},this);
				this.addPane();
	...
	addForm : function(f,elm) {
		...
		for(itm in f) {
			...
			if(type == 'title') {
				...
				switch(type) {
					case 'text':
						this.inputs[itm] = new bkElement('input').setAttributes({id : itm, 'value' : val, 'type' : 'text'}).setStyle({margin : '2px 0', fontSize : '13px', 'float' : 'left', height : '20px',  height : '20px', border : '1px solid #ccc', overflow : 'hidden'}).setStyle(field.style).appendTo(contain);
						break;
					case 'select':
		...
		this.btndiv = new bkElement('div').addClass("message_button").appendTo(this.form);	
		new bkElement('input').setAttributes({'type' : 'submit','value' : 'Submit'}).addClass("btn").appendTo(this.btndiv);
...
var nicSelectOptions = {
	buttons : {
		...
		'insertHTML' : {name : __('Personalize'), type : 'nicEditorPersonalizeSelect', command : 'insertHTML' }
	}
};
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		// Push personalization to the left 
		if (buttonName === "insertHTML") {
			location = "left";
		}

		this.margin = new bkElement('div').setStyle({'float' : location, margin : '0 1px 0 1px'}).appendTo(this.elm);
		this.contain = new bkElement('div').setStyle({width: '81px', height : '30px', cursor : 'pointer', overflow: 'hidden'}).addClass('selectContain').addEvent('click',this.toggle.closure(this)).appendTo(this.margin);
		this.items = new bkElement('div').setStyle({overflow : 'hidden', zoom : 1, paddingLeft : '3px', backgroundColor : '#fff'}).appendTo(this.contain);
		this.control = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'right', height: '30px', width : '0px'}).addClass('selectControl').setStyle(this.ne.getIcon('arrow',options)).appendTo(this.items);
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '75px', height : '14px', marginTop : '10px', fontFamily : 'sans-serif', textAlign : 'center', fontSize : '12px', fontWeight: '700'}).addClass('selectTxt').appendTo(this.items);
...
var nicEditorPersonalizeSelect = nicEditorSelect.extend({
		sel : {'{name}' : 'Name','{email}' : 'Email','{first_name}' : 'First&nbsp;Name','{last_name}' : 'Last&nbsp;Name', '{address}' : 'Address', '{city}' : 'City', '{state}' : 'State', '{zip}' : 'Zip&nbsp;Code', '{country}' : 'Country'},
	init : function() {
		this.setDisplay('Personalize');
		for(itm in this.sel) {
			//this.add(itm,'<a href="javascript:void(0);" onclick="javascript:addPersonalize(\''+itm+'\');">'+this.sel[itm]+'</a>');
...
var nicLinkButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		...
		this.addForm({
			...
			'href' : {type : 'text', txt : 'URL', value : 'http://', style : {width: '250px'}}
		},this.ln);
	...
	submit : function(e) {
		...
		if(!this.ln) {
// Following line removed on 23March 2015 due to nicTemp() error;	
...
this.ne.selectedInstance.restoreRng();
			...
			var colorSelectedBox = new bkElement('INPUT').setAttributes({type:'text','id':'selectedcolor'}).setStyle({color:'#000000',width : '130px', height : '13px','margin-top':'5px', position:'absolute', left: '0px', bottom: '-5px','zIndex':'99'}).appendTo(colorItems);
			var colorSubmitCnt = new bkElement('div').addClass("message_button").appendTo(colorItems);
			var colorSelectedSubmit = new bkElement('INPUT').addClass("btn").setAttributes({type:'button','value':'Apply Color'}).addEvent('click',this.colorImplement.closure(this)).appendTo(colorSubmitCnt);
...
var nicCodeButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		this.addForm({
			...
			'code' : {type : 'content', 'value' : this.ne.selectedInstance.getContent(), style : {width: '505px', height : '250px' }}
		});
		document.getElementById('displaybox').style.display='block';
	},
