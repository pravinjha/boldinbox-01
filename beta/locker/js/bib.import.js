...
$(document).ready(function() { 
	 $('#uploadForm').submit(function(e) {
		...
		var list_id = encodeURIComponent($('#subscription_contact_one').val());
		$("#spin").show(); 
		if($('#doit').val() == 'import_file'){
			if($('#subscriber_csv_file').val()) {
				
				$('#loader-icon').show();
				
				$(this).ajaxSubmit({
					url: base_url + 'subscriber/importcsv/' + list_id + '/' + terms,
					dataType: 'json',
		...
		}else if ($('#doit').val() == "copy_pase_contacts") {    
			...
			var block_data = "action=copy_pase_contacts&subscription_id="+list_id+"&copy_csv=" + pasted_contacts + "&terms=" + terms;						
		}else if ($('#doit').val() == "ony_by_one") {
			var block_data = "action=ony_by_one&subscription_id=" + list_id + "&terms="+terms +"&"+ $("#uploadForm").serialize();
		}
		
		$.ajax({
		  url: base_url + "subscriber/create/" + list_id,
		  type: "POST",
		  data: block_data,
