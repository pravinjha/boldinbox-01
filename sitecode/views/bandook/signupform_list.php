...
<script>
	...
	jQuery('.class_verify').live('click',function(){
		...
		if ($(this).attr('checked'))
			is_verified = 1;
		jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/signupforms/update_signupform/verify/",
				  type:"POST",
	...
	jQuery('.class_single_opt_in').live('click',function(){
		...
		if ($(this).attr('checked'))
			singleoptin = 1;
		jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/signupforms/update_signupform/singleoptin/",
				  type:"POST",
...
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo base_url().'bandook/signupforms/index/'.$mode;?>">
	<table border="0" cellspacing="0" cellpadding="0" class="tbl_forms">
		<tr>
			...
			<td align="right" style="text-align:right;"><a href="<?php echo  site_url("bandook/signupforms/index/$reverseMode");?>"><?php echo  ucfirst($reverseMode);?> List</a></td>
		</tr>
		<tr>
...
if(count($signupforms)) {
	...
	<tr>
		...
		<td>		 
			<a class="fancybox" href="<?php echo  site_url("bandook/signupforms/view/".$signupform['id']); ?>"><?php echo $signupform['form_name'];?></a>
		</td>
		<td>
			<a href="<?php echo  site_url("bandook/users_manage/users_list/".$signupform['member_username']); ?>"><?php echo $signupform['member_username'];  ?></a>
		</td>
		...
		<td>	<?php // echo  site_url('bandook/signupforms/verifyit/'..'/'.$signupform['id']);?>	
		...
		<td>
			<a onclick="return confirm('Are you sure to delete this signup-form')" href="<?php echo  site_url('bandook/signupforms/delete/'.$mode.'/'.$signupform['id']);?>">Delete</a>
		</td>
	</tr>
<?php }
	}else{
