...
<script type="text/javascript">
	...
	function attachDisallowedMsg(x){
		if(x != ''){
			jQuery.ajax({
			  url: "<?php echo base_url() ?>bandook/campaign/ajaxMsg/"+x,
			  type:"POST",
			  success: function(data) { 
	...
	function updateScheduledDt(bid){
		var thisDt = $('#updated_schedule_dt_'+ bid).val();
		jQuery.ajax({
		  url: "<?php echo base_url() ?>bandook/campaign/update_schedule/"+bid+"/"+thisDt,
		  type:"POST",		  
		  success: function(data) {
			$('#div_update_schedule_dt_'+bid).text(data+' updated');
		  }
		});
	}
</script>
...
if(count($campaigns)){
	...
	<tr>
		...
		<td>
			<a href="<?php echo  site_url("bandook/users_manage/users_list/".$campaign['member_username']); ?>" <?php if($campaign['campaign_approval_notes'] !='') echo'style="color:#ff0000;"';?>><?php echo $campaign['member_username'];  ?></a>
			<br/>
		...
		<?php
			...
			for($i=0; $i < count($arrVMTAPool); $i++){				 
				for($j=0; $j <  count($arrVMTAPool[$i]); $j++){
					if($j==0){
						echo "<optgroup label='".$arrVMTAPool[$i][$j]."'></optgroup>";
...
<span id="vmta-<?php echo $campaign['campaign_id'];?>" style="color:#ff6500"></span>
		...
		<td>
			<a  href="<?php echo  site_url("bandook/campaign/get_contacts/".$campaign['campaign_id']."/".$campaign['member_id']); ?>"><?php echo $contacts[$campaign['campaign_id']]['total'];  ?></a>
			<br/>
			<?php echo ltrim($campaign['list_names'],',');?>
			<br/><b>Sent: </b><?php echo $contacts[$campaign['campaign_id']]['sent'];  ?>
			<br/><b>Un-Sent Yet: </b><?php echo $contacts[$campaign['campaign_id']]['unsent'];  ?>			
		</td>
		<td>
			...
			<?php }else{ ?>
				Not Scheduled
			<?php } ?>
			<input type="text" name="updated_schedule_dt_<?php echo $campaign['campaign_id'];?>"  id="updated_schedule_dt_<?php echo $campaign['campaign_id'];?>" value="<?php echo $campaign['campaign_sheduled'];?>" />
			<input type="button" name="btn_<?php echo $campaign['campaign_id'];?>" id="btn_<?php echo $campaign['campaign_id'];?>" value="Re-Schedule" onclick="javascript:updateScheduledDt('<?php echo $campaign['campaign_id'];?>');" />
			<div id="div_update_schedule_dt_<?php echo $campaign['campaign_id'];?>" style="color:#ff0000;"></div>
		</td>		 
		<td>
		...
		<tr>
			...
			<?php } ?>
			<li><a target="_blank" href="<?php echo  site_url('bandook/dashboard_stat/sent_campaign_user_stats/'.$campaign['member_id']);?>">User's Stats</a></li>
			</ul>
			</td>
			<td>
			<ul class="ul_actions">
			<li><a id="draft_<?php echo $campaign['campaign_id']; ?>" href="<?php echo site_url("bandook/campaign/draftIt/".$campaign['campaign_id']."/$mode/");?>" class="draft">Draft</a></li>
			<li><a id="disallow_<?php echo $campaign['campaign_id']; ?>" href="<?php echo site_url("bandook/campaign/disallow_confirm/".$campaign['campaign_id']);?>" class="fancybox">Disallow</a></li>
			<li>
			<a onclick="return confirm('Are you sure to delete campaign')" href="<?php echo  site_url('bandook/campaign/delete/'.$mode.'/'.$campaign['campaign_id']);?>"><font color="red">Delete</font></a>
			</li>		
			</ul>
