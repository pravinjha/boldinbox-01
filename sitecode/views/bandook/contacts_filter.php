...
<script type="text/javascript">
	jQuery('#btnFilter').live('click',function(){
			...
			$('#contacts').html("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");
			  
			  
				jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/contacts_segmentation/get_contacts_filter_count/",
				  type:"post",
	...
	function movelist(){
		...
		if(parseInt(newList) > 0 && confirm(reclimit+" Filtered contacts will be moved. Are you sure to move contacts?")){
			...
			$('#contacts').prepend("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");
			jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/contacts_segmentation/move_contacts/"+fromList+'/'+newList,
				  type:"post",
				  data: 'l='+reclimit,
				  success: function(x) {
			...
			$('#contacts').prepend("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");
			jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/contacts_segmentation/move_to_dnm/"+fromList,
				  type:"post",
				  data: 'l='+reclimit,
	...
	function copylist(){
		...
		if(parseInt(newList) > 0 && confirm(reclimit+" Filtered contacts will be copied. Are you sure to copy contacts?")){
			...
			$('#contacts').prepend("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");
			jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/contacts_segmentation/copy_contacts/"+fromList+'/'+newList,
				  type:"post",
				  data: 'l='+reclimit,
				  success: function(x) {  
	...
	function attachContactVmta(){
		...
		if( confirm(reclimit+" Filtered contacts will be added to selected pipeline. Are you sure for this?")){
			...
			$('#contacts').prepend("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");
			jQuery.ajax({
				  url: "<?php echo base_url() ?>bandook/contacts_segmentation/attachContactVmta/"+newVMTA,
				  type:"post",
				  data: 'l='+reclimit,
				  success: function(x) {   
	...
	function exportit(){
		...
		if(confirm(reclimit+ " Filtered contacts will be exported to csv. Are you sure to export contacts?")){			
			window.location.href="<?php echo base_url() ?>bandook/contacts_segmentation/export_contacts/"+reclimit;			 
		}	
	}
	function deletecontacts(){
		...
		if(confirm(reclimit+" Filtered contacts will be deleted from your account. Are you sure to delete contacts?")){			
			$('#contacts').prepend("<img src='<?php echo $this->config->item('locker');?>images/ajax_loading.gif' width='220' height='19' />");			 
			jQuery.ajax({
				url: "<?php echo base_url() ?>bandook/contacts_segmentation/delete_contacts/",
				type:"post",
				data: 'l='+reclimit,
				success: function(x) { 
	...
	function deletefromlist(){
		...
		if(confirm(reclimit+" Filtered contacts will be deleted from this list only. Are you sure to delete contacts?")){			
			...
			var fromList =  $('#contact_lists').val();				
			jQuery.ajax({
				url: "<?php echo base_url() ?>bandook/contacts_segmentation/delete_from_list/"+fromList ,
				type:"post",
				data: 'l='+reclimit,
				success: function(x) { 
					if(x == 'ok')alert("Contacts removed.");
	...
	function fn_mark_unresponsive(){
		...
		if(confirm(reclimit+" Filtered contacts will be marked as unresponsive. Are you sure to do this?")){			
			...
			jQuery.ajax({
				url: "<?php echo base_url() ?>bandook/contacts_segmentation/mark_unresponsive/" ,
				type:"post",
				data: 'l='+reclimit,
				success: function(x) { 
	...
	function reportgrade(){
		...
		jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/contacts_segmentation/report_grade/" ,
			type:"post",
			success: function(x) { 
				$( "#dialog" ).dialog();
	...
	function batchgrade(){
		...
		jQuery.ajax({
			url: "<?php echo base_url() ?>bandook/contacts_segmentation/batch_grade/" ,
			type:"post",
			success: function(x) { 
				$( "#dialog" ).dialog();
	...
	if( $('#subscription_menu').is(':hidden') ){
  	...
  	$.fancybox({'content' : "<div style=\"width:400px;\">"+msg+"</div>"});
		...
		$('.drop_img').attr('src',"<?php echo $this->config->item('locker');?>images/down.png?v=6-20-13");
		...
		$('.drop_img').attr('src',"<?php echo $this->config->item('locker');?>images/login-up.png?v=6-20-13");
	}
...
function ajaxSubscriptionFrm(frm){
		...
		block_data+="action=submit&"+'&mid='+mid+'&subscription_title='+escape(frm.subscription_title.value);
		   $.ajax({
		  url: "<?php echo base_url() ?>bandook/contacts_segmentation/create/",
		   type:"POST",
			data:block_data,
...
<tr><th>Contact Status</th>
	...
	<td><select name="subscriber_status"><option value="1">Active</option><option value="0">Unsubscribed</option><option value="2">Complaints</option><option value="3">Hard Bounce</option><option value="4">Soft Bounce</option><option value="5">Added to DNM</option><option value="-999">Deleted</option><option value="-777">Ignored</option><option value="">All</option></select></td>	
</tr>
...
<tr><th>Responsive</th><td>&nbsp;</td>
	<td>
		<select name="is_responsive" id="is_responsive">
			...
			<option value="strictY">Strict Yes</option>
...
<td><select name="last_read_date_operator" style="width:80px;"><option value="">All</option><option value="=">Equal to</option><option value=">">More than</option><option value="<">Less than</option><option value="IS NULL">IS NULL</option></select></td>
