...
class Packages_Manage extends CI_Controller
{
	...
	function packages($start=0){
		...
		$config['base_url']=base_url().'bandook/packages_manage/packages';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Packages','logo_link'=>$logo_link));
		$this->load->view('bandook/packages',array('packages'=>$packages,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function packages_list($start=0)
	{
		...
		// Define config parameters for paging like base url, total rows and record per page.
		$config['base_url']=base_url().'bandook/packages_manage/packages_list';
		$config['total_rows']=$this->UserModel->get_packages_count($fetch_conditions_array);
		$config['per_page']=20;
		$config['uri_segment']=4;
		...
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Manage Packages','logo_link'=>$logo_link));
		$this->load->view('bandook/packages_list',array('packages'=>$packages,'paging_links'=>$paging_links,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	{	
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()==true)
			{
				...
				redirect('bandook/packages_manage/packages_list');
			}
		}
		...
		$this->load->view('bandook/package_create',array('messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	{		
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Edit Package','logo_link'=>$logo_link));
		$this->load->view('bandook/package_edit',array('messages' =>$messages,'package_data'=>$package_data,'package_id'=>$package_id));
		$this->load->view('bandook/footer');
	}
