...
class Pmta_log extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('bandook/pmta_log_model','',TRUE); 
	...
	function index(){
		...
		if($this->input->post('resetThisForm') == 'notok'){	
			...
			$sqlWhere .= ($this->session->userdata('type') == 'b')?" and (type='".$this->session->userdata('type')."' or type='rb') ":"";
			$sqlWhere .= ($this->session->userdata('type') != '' and $this->session->userdata('type') != 'b')?" and type='".$this->session->userdata('type')."'":"";
		...
		//$config['base_url'] = base_url().'bandook/pmta_log/index/'.$keys.'/';
		$config['base_url'] = base_url().'bandook/pmta_log/index/';
		...
		// load view
		$this->load->view('bandook/header',array('title'=>'View PMTA Log','logo_link'=>'bandook/pmta_log/'));
		$this->load->view('bandook/pmta_log', $data);		
