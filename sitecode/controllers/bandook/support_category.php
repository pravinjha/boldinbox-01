...
class Support_category extends CI_Controller
{
	...
	function support_category_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/support_category/support_category_list';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Support Category','logo_link'=>$logo_link));
		$this->load->view('bandook/support_category_list',array('categories'=>$cateogry_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function support_category_edit($id=0)
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				{					
					...
					redirect('bandook/support_category/support_category_list');
		...
		$this->load->view('bandook/header',array('title'=>'Update Support Category','logo_link'=>$logo_link));
		$this->load->view('bandook/support_category_edit',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function support_category_create()
	{
		...
		$this->load->view('bandook/support_category_create',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
}
?>