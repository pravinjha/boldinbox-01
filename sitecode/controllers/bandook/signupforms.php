...
class Signupforms extends CI_Controller
{
	...
	**/
	function index($mode, $start=0){
		...
		$config['base_url']=base_url().'bandook/signupforms/index/'.$mode;
		...
		$this->load->view('bandook/header',array('title'=>'Manage Signup-forms','logo_link'=>$logo_link));
		$this->load->view('bandook/signupform_list',array('signupforms'=>$signupform_data_array,'contacts_array'=>$contacts_array,'paging_links'=>$paging_links,'messages' =>$messages,'mode'=>$mode));
	...
	*/
	function delete($mode, $id=0){
		...
		redirect('bandook/signupforms/index/'.$mode);
	}
	/**
	...
	*/
	...
	function view($id=0){
		...
		if($this->session->userdata('webmaster_id')==''){
			...
			$this->load->view('bandook/signupform_view',array('signupform'=>$signupform_data_array[0]));
		}
	}
