...
class Feedback extends CI_Controller
{
	...
	**/
	function feedback_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/feedback/feedback_list';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Feedback','logo_link'=>$logo_link));
		$this->load->view('bandook/feedback_list',array('feedbacks'=>$feedback_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	**/
	function message($id=0){
		if($this->session->userdata('webmaster_id')==''){
			...
			$this->load->view('bandook/feedback_message',array('message'=>$feedback_data_array[0]['message']));
		}
	}
	/**
	...
	**/
	function delete($id=0){
		if($this->session->userdata('webmaster_id')==''){
			echo "<div style=\"margin:20px;width:240px;\">Your Session seems to have expired. Please try refreshing the page to login again.</div>";
			redirect('bandook/feedback/feedback_list');
		}else{
