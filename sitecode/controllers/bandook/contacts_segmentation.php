...
class Contacts_segmentation extends CI_Controller
{
	...
	function index($member_id=0){
		...
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/contacts_segmentation',array('users'=>$user_data_array, 'contact_list'=>$contactLists));
	...
	function filter($member_id=0){
		...
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Filter Contacts','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/contacts_filter',array('users'=>$user_data_array, 'contact_list'=>$contactLists));
		$this->load->view('bandook/footer');
	}
	function importContacts($mid=0){		 
		...
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/contacts_add',$subscription_data);
		$this->load->view('bandook/footer');
	...
	function addDNM($mid=0){		 
		...
		//Loads header, subscription and footer view.
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/addDNM',$subscription_data);
		$this->load->view('bandook/footer');
	...
	function addDNMSubmit(){
		...
		foreach($arrDNMContacts as $e){
			...
			if($this->is_authorized->ValidateAddress($e)){			
				$this->db->query("INSERT INTO red_email_subscribers SET subscriber_email_address='".mysqli_real_escape_string($this->is_authorized->get_mysqli(), $e) ."', subscriber_created_by = '$mid' ON DUPLICATE KEY UPDATE  is_deleted = 0,subscriber_status=5 ");							
				$c++;
			}		
		}
		echo $this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'), true);
		echo "<p>".$c. " contacts added to DNM </p>";
		echo $this->load->view('bandook/footer',null,true);
	...
	//print_r($_POST);
		...
		echo "<div class='tblheading'>Result  [<a href='/bandook/contacts_segmentation/index/$mid'>Filter again</a>] </div><table class='tbl_listing' width='100%'><tr><th>Total Contacts: </th><td>
	...
	// print_r($_POST);
		...
		if($subscriber_status == ''){
			...
			$whereClause .= " and is_deleted = 1 ";
		}elseif($subscriber_status == '-777'){
			$whereClause .= " and subscriber_status=1 and is_deleted = 0 and `ignore` > 0 ";	
		}else{
		...
		elseif(trim($this->input->post('is_responsive')) =='n')
		$whereClause .= " and s.`read` = 0";
		elseif(trim($this->input->post('is_responsive')) =='strictY')
		$whereClause .= " and s.`read` > 0 and s.last_read_date is not null";
		...
		if($last_read_date_operator != '' && $last_read_date_operator !='IS NULL')
		$whereClause .= " and str_to_date(last_read_date,'%Y-%m-%d') $last_read_date_operator '".$this->input->post('last_read_date')."'";
		elseif($last_read_date_operator =='IS NULL')
		$whereClause .= " and last_read_date is null";
		...
		echo "<div class='tblheading'>Result  [<a href='/bandook/contacts_segmentation/filter/$mid'>Filter again</a>] </div><table class='tbl_listing' width='100%'><tr><th>Total Contacts: </th><td>
		<table style='width:70%;border:0;padding:0;margin:0;'><tr><td style='border:0;padding:0;margin:0;'>$totalContacts</td><td style='border:0;padding:0;margin:0;'>$export_button $delete_button $delete_from_list_button $mark_unresponsive $chart_button $batch_button</td></tr></table>		
	...
	function searchContact(){
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'));		
		$this->load->view('bandook/searchContact');
		$this->load->view('bandook/footer');
	...
	function unsubscribe_feedback($start=0){
		...
		if(isset($_POST['mode']) and $_POST['mode'] == 'search'){
			...
			$config['base_url']=base_url().'bandook/contacts_segmentation/unsubscribe_feedback';
			$config['per_page']=20;
		...
		$this->load->view('bandook/header',array('title'=>'Manage Users','logo_link'=>'bandook/dashboard_stat'));		
		$this->load->view('bandook/unsubscribe_feedback', array('tbl'=>$tbl,'contacts_array'=>$contacts_array, 'paging_links'=>$paging_links));
		$this->load->view('bandook/footer');	
	}
