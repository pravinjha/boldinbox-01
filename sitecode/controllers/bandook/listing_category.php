...
class Listing_category extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('bandook/ListingCategory_Model');
	...
	function category_list($start=0)
	{
		...
		$config['base_url']=base_url().'bandook/listing_category/category_list';
		...
		$this->load->view('bandook/listing_category_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function category_edit($id)
	{
		...
		if($this->input->post('action')=='submit')
		{
			...
			if($this->form_validation->run()) {
				...
				else
				{
					...
					redirect('bandook/listing_category/category_list');
				}
			}
		...
		$this->load->view('bandook/listing_category_edit',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	...
	function category_create($id=0)
	{
		...
		$this->load->view('bandook/listing_category_create',array('category'=>$category_data_array,'messages' =>$messages));
		$this->load->view('bandook/footer');
	}
}
?>