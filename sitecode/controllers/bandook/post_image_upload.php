...
class Post_Image_Upload extends CI_Controller {
    	...
    	function __construct()
	{
		...
		$this->load->model('bandook/BlogSetting_Model');
	...
	function listing($post_id)
	{
		...
		$config['base_url']=base_url().'bandook/post_image_upload/listing/'.$post_id;
		...
		$this->load->view('bandook/header',array('title'=>'Manage Post Image Listing','logo_link'=>$logo_link));
		$this->load->view('bandook/post_image_list',array('category'=>$category_data_array,'paging_links'=>$paging_links,'messages' =>$messages,'post_id'=>$post_id));
    ...
    function do_upload()
    {
       ...
       // $this->load->library('Multi_upload');
        ...
        { 
			...
			$this->load->view('bandook/header',array('title'=>'Image Upload'));
           $this->load->view('bandook/post_image_upload', array('error' => $this->upload->display_errors(),'post_id'=>$category_data['post_id']));
		...
		echo $upload_path;
			...
			redirect('bandook/post_image_upload/listing/'.$category_data['post_id']);
	...
	function image_create($post_id,$id=0)
	{
		...
		$this->load->view('bandook/header',array('title'=>'Image Upload','logo_link'=>$logo_link));
		$this->load->view('bandook/post_image_upload',array('category'=>$category_data_array,'messages' =>$messages,'post_id'=>$post_id));
		$this->load->view('bandook/footer');
	...
	function image_delete($post_id,$img_id)
	{
		...
		redirect('bandook/post_image_upload/listing/'.$post_id);
	}
