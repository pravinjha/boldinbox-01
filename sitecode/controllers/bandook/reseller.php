...
class Reseller extends CI_Controller
{
	...
	function index($start=0){		
		...
		$config['base_url']=base_url().'bandook/reseller/index';
		...
		$this->load->view('bandook/header',array('title'=>'Manage Reseller','logo_link'=>$logo_link));
		$this->load->view('bandook/reseller_list',array('resellers'=>$resellers,'paging_links'=>$paging_links,'messages' =>$messages));
	...
	function reseller_create(){		
		if($this->input->post('action')=='submit'){
			...
			if($this->form_validation->run()==true){
				...
				redirect('bandook/reseller');
			}
		}
		
		// Recieve any messages to be shown
		$messages=$this->messages->get();
		$logo_link="bandook/reseller";
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Create New Reseller','logo_link'=>$logo_link));
		$this->load->view('bandook/reseller_create',array('messages' =>$messages));
		$this->load->view('bandook/footer');
	}
	function report(){
		...
		$logo_link="bandook/reseller";
		//Loads header, users listing  and footer view.
		$this->load->view('bandook/header',array('title'=>'Reseller-Report','logo_link'=>$logo_link));
		$this->load->view('bandook/reseller_report',array('messages' =>$messages,'arrReferrerName'=>$arrReferrerName,'referrer'=>$referrer,'arrReseller'=>$arrSelectedReseller, 'totAmount'=>$totAmount, 'fdate'=>$fdate,'tdate'=>$tdate));
		$this->load->view('bandook/footer');	
	}
}
?>