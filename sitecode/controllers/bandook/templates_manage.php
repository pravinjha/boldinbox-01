...
class Templates_Manage extends CI_Controller
{
	...
	*/
	...
	function __construct()
	{
		...
		$this->load->model('bandook/Template_Model');
	...
	function template_create()
	{
		...
		if($this->input->post('action')=='save')
		{
			...
			if($this->form_validation->run()==true)
			{
				...
				redirect('bandook/templates_manage/templates_list');
			}
		...
		$this->load->view('bandook/header',array('title'=>'Create Template','logo_link'=>$logo_link));
		$this->load->view('bandook/template_create',array('template_data'=>$template_data));
	...
	*/
	...
	function template_edit($id)
	{
		...
		*/
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, template and footer view.
		$this->load->view('bandook/header',array('title'=>'Edit Template','logo_link'=>$logo_link));
		$this->load->view('bandook/template_edit',array('template_data'=>$template_data));
		$this->load->view('bandook/footer');
	...
	*/
	...
	function templates_list($start=0)
	{
		...
		$config['base_url']=base_url().'userboard/template/display';
		...
		$logo_link="bandook/dashboard_stat";
		//Loads header, template and footer view.
		$this->load->view('bandook/header',array('title'=>'List Templates','logo_link'=>$logo_link));
		$this->load->view('bandook/templates_list',$template_data);
		$this->load->view('bandook/footer');
	}
	...
	*/
...

	{
		...
		$this->load->model('userboard/Template_Model');
	...
	*/
...

	{
		...
		$this->load->view('userboard/campaign_copy_archived',array('campaign_data'=>$campaign_data));
	...
	*/
...

	{
		...
		$this->upload_path= $this->upload_path.'/locker/email_templates/';
			...
			if(count($files))
			{
				foreach($files as $file)
				{
					if(in_array($file,$file_names_allowed) )
					{
						...
						$path_to_images=base_url().'/locker/email_templates/'.$new_file_name;
						break;
					}
				}
			}
				...
				while (false !== ($file = readdir($dir_handle))) {
...

					{
...

						{
							...
							$path_to_images=base_url().'/locker/email_templates/'.$new_file_name.'/'.$directory;
	...
	*/
...

	{
...

		{
...

			{
...

				{
					...
					// Redirect to listing of campaigns
					redirect('userboard/campaign/send');
		...
		$this->load->view('userboard/campaign_send',array('messages'=>$messages,'campaign_data'=>$campaign_data,'subscription_data'=>$subscription_data));
