...
class Report extends CI_Controller
{
	...
	*/
	...
	function global_ipr($interval=0){	
		...
		$date_to = date('Y-m-d');
		$date_from = date('Y-m-d',(strtotime($date_to) - (24*60*60)));
		$previousDay = date('Y-m-d',(strtotime($date_to) - (24*60*60)));		
		$nextDay = date('Y-m-d',(strtotime($date_to) + (24*60*60)));		
		
		$this->load->view('bandook/header',array('title'=>'Campaign Segmentation','logo_link'=>'bandook/dashboard_stat'));
		$this->load->view('bandook/global_ipr_daily',array('arrPools'=>$arrPools,'pipeline'=>$pipeline,'arrRcusers'=>$arrRcusers,'rcuser'=>$rcuser,'date_from'=>$date_from,'date_to'=>$date_to,'previousDay'=>$previousDay,'nextDay'=>$nextDay, 'strGlobalIPR'=>$this->showIPR()));
			
		$this->load->view('bandook/footer');
	} 
	function showIPR($typ=''){
		$adminTimeZone = date_default_timezone_get();
		if($this->input->post('btn_search')=='Search'){
			...
			$clausePipeline ='';
			if($pipeline =='pmta-pool-1'){
				$clausePipeline = " and pipeline in('pmta-pool-1','pmta-vmta1','pmta-vmta2','pmta-vmta3')";	
			}elseif($pipeline =='pmta-pool-2'){
				$clausePipeline = " and pipeline in('pmta-pool-2','pmta-vmta4','pmta-vmta5','pmta-vmta6')";	
			}elseif($pipeline =='pmta2-pool-1'){
				$clausePipeline = " and pipeline in('pmta2-pool-1','pmta2-vmta1','pmta2-vmta2','pmta2-vmta3','pmta2-vmta6','pmta2-vmta7')";	
			}elseif($pipeline =='pmta2-pool-2'){
				$clausePipeline = " and pipeline in('pmta2-pool-2','pmta2-vmta4','pmta2-vmta5','pmta2-vmta8','pmta2-vmta9','pmta2-vmta10')";	
			}elseif($pipeline =='pmta3-pool-1'){
				$clausePipeline = " and pipeline in('pmta3-pool-1','pmta3-vmta1','pmta3-vmta2','pmta3-vmta3')";	
			}elseif($pipeline =='pmta3-pool-2'){
				$clausePipeline = " and pipeline in('pmta3-pool-2','pmta3-vmta4','pmta3-vmta5','pmta3-vmta6')";	
			}
			$clauseUsers = ($rcuser != '')?" and `user_id` = '$rcuser'" : '';
			
			if($clausePipeline ==''){ 
				$sqlIPR = "select mail_domain, Case WHEN(mail_domain='gmail.com')then 1 WHEN(mail_domain='yahoo.com')then 2 WHEN(mail_domain='hotmail.com')then 3 WHEN(mail_domain='aol.com')then 4 WHEN(mail_domain='msn.com')then 5 WHEN(mail_domain='outlook.com')then 6 WHEN(mail_domain='windowslive.com')then 7 WHEN(mail_domain='live.com')then 8 WHEN(mail_domain='mail.ru')then 9 WHEN(mail_domain='me.com')then 10 WHEN(mail_domain='mac.com')then 11 WHEN(mail_domain='comcast.net')then 12 WHEN(mail_domain='cox.net')then 13  WHEN(mail_domain='rediffmail.com')then 14 END as ord, sum(total_sent)total_sent, sum(total_released)total_released, sum(total_delivered)total_delivered, sum(total_opened)total_opened, sum(total_bounced)total_bounced, sum(total_unsubscribed)total_unsubscribed, sum(total_complaint)total_complaint from red_global_ipr_daily where CONVERT_TZ( `log_date`, '+00:00','+5:30' ) > '$date_from 23:59:59' and  CONVERT_TZ( `log_date`, '+00:00','+5:30'  ) < '$date_to 23:59:59' $clauseUsers group by mail_domain order by ord";
				// 'UTC', '$adminTimeZone'
			}else{				
				$sqlIPR = "select Case WHEN (pipeline='pmta-pool-1' or pipeline='pmta-vmta1' or pipeline='pmta-vmta2' or pipeline='pmta-vmta3') then 'pmta-pool-1' WHEN (pipeline='pmta-pool-2' or pipeline='pmta-vmta4' or pipeline='pmta-vmta5' or pipeline='pmta-vmta6') then 'pmta-pool-2' WHEN (pipeline='pmta2-pool-1' or pipeline='pmta2-vmta1' or pipeline='pmta2-vmta2' or pipeline='pmta2-vmta3' or pipeline='pmta2-vmta6' or pipeline='pmta2-vmta7') then 'pmta2-pool-1' WHEN (pipeline='pmta2-pool-2' or pipeline='pmta2-vmta4' or pipeline='pmta2-vmta5' or pipeline='pmta2-vmta8' or pipeline='pmta2-vmta9' or pipeline='pmta2-vmta10') then 'pmta2-pool-2' WHEN (pipeline='pmta3-pool-1' or pipeline='pmta3-vmta1' or pipeline='pmta3-vmta2' or pipeline='pmta3-vmta3') then 'pmta3-pool-1' WHEN (pipeline='pmta3-pool-2' or pipeline='pmta3-vmta4' or pipeline='pmta3-vmta5' or pipeline='pmta3-vmta6') then 'pmta3-pool-2' END as vmta, mail_domain, Case WHEN(mail_domain='gmail.com')then 1 WHEN(mail_domain='yahoo.com')then 2 WHEN(mail_domain='hotmail.com')then 3 WHEN(mail_domain='aol.com')then 4 WHEN(mail_domain='msn.com')then 5 WHEN(mail_domain='outlook.com')then 6 WHEN(mail_domain='windowslive.com')then 7 WHEN(mail_domain='live.com')then 8 WHEN(mail_domain='mail.ru')then 9 WHEN(mail_domain='me.com')then 10 WHEN(mail_domain='mac.com')then 11 WHEN(mail_domain='comcast.net')then 12 WHEN(mail_domain='cox.net')then 13  WHEN(mail_domain='rediffmail.com')then 14 END as ord, sum(total_sent)total_sent,  sum(total_released)total_released, sum(total_delivered)total_delivered, sum(total_opened)total_opened, sum(total_bounced)total_bounced, sum(total_unsubscribed)total_unsubscribed, sum(total_complaint)total_complaint from red_global_ipr_daily where CONVERT_TZ( `log_date`, '+00:00','+5:30' ) > '$date_from 23:59:59' and  CONVERT_TZ( `log_date`, '+00:00','+5:30' ) < '$date_to 23:59:59' $clausePipeline $clauseUsers  group by mail_domain,vmta order by ord";					
			}
			...
			if($rsDailyIPR->num_rows()>0){	
				...
				foreach($rsDailyIPR->result() as $row){
					...
					$net_total_released		+= $row->total_released;
					...
					$deliveredPercentage = ($row->total_released > 0)?number_format($row->total_delivered/$row->total_released *100 , 2): 0;
					$openPercentage = ($row->total_delivered > 0)?number_format($row->total_opened/$row->total_delivered *100 , 2): 0;
					$bouncedPercentage = ($row->total_delivered > 0)?number_format($row->total_bounced/$row->total_delivered *100 , 2): 0;
					$complaintPercentage = ($row->total_delivered > 0)?number_format($row->total_complaint/$row->total_delivered *100 , 2): 0;
					$unsubscribePercentage = ($row->total_delivered > 0)?number_format($row->total_unsubscribed/$row->total_delivered *100 , 2): 0;
					
					$strTblBody .= '<td>'.$row->mail_domain.'</td><td>'.$row->total_sent.'</td><td>'.$row->total_released.'</td><td>'.$row->total_delivered.'('.$deliveredPercentage .'%)</td><td>'.$row->total_opened.'('. $openPercentage.'%)</td><td>'.$row->total_bounced.'('.$bouncedPercentage.'%)</td><td>'.$row->total_complaint.'('. $complaintPercentage.'%)</td><td>'.$row->total_unsubscribed.'('. $unsubscribePercentage.'%)</td></tr>';									
				}
			}
		...
		$totalDeliveredPercentage = ($net_total_released > 0)?number_format($net_total_delivered/$net_total_released *100 , 2): 0;
		$totalOpenedPercentage = ($net_total_delivered > 0)?number_format($net_total_opened/$net_total_delivered *100 , 2): 0;
		$totalBouncedPercentage = ($net_total_delivered > 0)?number_format($net_total_bounced/$net_total_delivered *100 , 2): 0;
		$totalComplaintPercentage = ($net_total_delivered > 0)?number_format($net_total_complaint/$net_total_delivered *100 , 2): 0;
		$totalUnsubscribedPercentage = ($net_total_delivered > 0)?number_format($net_total_unsubscribed/$net_total_delivered *100 , 2): 0;
		$strTblFooter .='<th>Total</th><th>'.$net_total_sent.'</th><th>'.$net_total_released.'</th><th>'.$net_total_delivered.'('.$totalDeliveredPercentage.'%)</th><th>'.$net_total_opened.'('.$totalOpenedPercentage.'%)</th><th>'.$net_total_bounced.'('.$totalBouncedPercentage.'%)</th><th>'.$net_total_complaint.'('.$totalComplaintPercentage.'%)</th><th>'.$net_total_unsubscribed.'('.$totalUnsubscribedPercentage.'%)</th></tr>';
		...
		$strGlobalIPR .= '<th>Domain</th><th>Sent</th><th>Released</th><th>Delivered</th><th>Opened</th><th>Bounced</th><th>Complaint</th><th>Unsubscribed</th>';
...
//		$sqlPaidUsers ="select m.member_id,m.member_username, m.vmta, date_format(m.created_on,'%Y-%m-%d')created_on, m.is_authentic, date_format(m.authenticated_on,'%Y-%m-%d')authenticated_on, m.is_risky, m.apply_unresponsive_filter, m.unresponsive_release_count, m.member_dnm, m.member_unresponsive, mp.package_id, mp.amount, mp.start_payment_date, mp.next_payement_date, mp.user_quota_multiplier multiplier, mp.max_campaign_quota, mp.campaign_sent_counter, mp.coupon_code_used from red_members m inner join red_member_packages mp on m.member_id=mp.member_id where mp.package_id >0 and mp.next_payement_date > now() order by created_on";
		...
		echo $this->load->view('bandook/header',array('title'=>'Campaign Segmentation','logo_link'=>'bandook/dashboard_stat'),true);
...
//		$sqlPaidUsers ="select m.member_id,m.member_username, m.vmta, date_format(m.created_on,'%Y-%m-%d')created_on, m.is_authentic, date_format(m.authenticated_on,'%Y-%m-%d')authenticated_on, m.is_risky, m.apply_unresponsive_filter, m.unresponsive_release_count, m.member_dnm, m.member_unresponsive, mp.package_id, mp.amount, mp.start_payment_date, mp.next_payement_date, mp.user_quota_multiplier multiplier, mp.max_campaign_quota, mp.campaign_sent_counter, mp.coupon_code_used from red_members m inner join red_member_packages mp on m.member_id=mp.member_id where mp.package_id >0 and mp.next_payement_date > now() order by created_on";
		...
		$rsPaidUsers = $this->db->query($sqlPaidUsers);
		$strTable = "\n\n<table cellspacing='0' cellpadding='0' border='1' class='tbl_listing'>\n";
		$strTable .= "<tr><th>Sl.No.</th><th>UserID</th><th>User</th><th>Referrer</th><th>Pipeline</th><th>Authentic</th><th>Risky</th><th>Unres.Filter</th><th>Unres.Count</th>
					<th>StopApproval</th><th>ShowSentCounter</th><th>CampaignPausable</th><th>ListGrowingMail</th>
					<th>Multiplier</th><th>Quota</th><th>Sent</th><th>PackageDetail</th><th>AmountPaid</th><th>Coupon</th><th>1stPayDt</th>
		...
		foreach($rsPaidUsers->result() as $row){
			...
			$totalAmount += $thisAmount;
			$strTable .= "<tr><td>$slno</td><td>$thisMid</td><td>$thisMember <br/>Created-on:$thisCreatedOn</td><td>$thisReferrer</td><td>$thisVMTA</td>
						...
						<td>$thisPackage [$thisPackageMinContact - $thisPackageMaxContact]<br/>$$thisAmount</td><td>$$transactionAmount</td>
						<td>$thisCoupon</td><td>$firstPayDt</td><td>$transactionDt</td><td>$nextPayDt</td>
		...
		$strTable .= "<tr><td colspan='17' align='right'><b>Total:</b></td><td>$$totalAmount</td></tr>\n</table>\n";
		$rsPaidUsers->free_result();
		echo $this->load->view('bandook/header',array('title'=>'Campaign Segmentation','logo_link'=>'bandook/dashboard_stat'),true);
		echo '<div style="padding:20px;"><b>Total paid users: </b>'.$totalPaidUsers.'<br/><b>Total projected revenue:</b> $'.$totalAmount.'<br/><a href="'.base_url().'bandook/report/paid_users_export" style="margin:10px; padding:5px; font-size:12px; color: blue; text-decoration:underline;" />Export to CSV</a></div>';
		echo $strTable ;
	...
	function paid_users_export($limit=0){
		...
		$csv_output = '"Sl.No.","UserID","User","Created-on"," Referrer","Pipeline","Authentic","Risky","Unres.Filter","Unres.Counter","StopApproval","ShowSentCounter","CampaignPausable","ListGrowingMail"," Multiplier","Quota","Sent","PackageDetail","AmountPaid","Coupon","1stPayDt","LastPayDt","NextPayDt","MemberDNM","Contacts","GMail","Yahoo","Hotmail","AOL","MSN"';
		$csv_output .= "\n";
		$slno=1;
		foreach($rsPaidUsers->result() as $row){
			$thisMid = $row->member_id; $thisMember = $row->member_username; $thisVMTA=$row->vmta; $thisCreatedOn = $row->created_on;  $thisReferrer = $row->ls_site_id; $thisMemberDNM = $row->member_dnm; $thisMultiplier = $row->multiplier; $thisMemberCampaignQuota = $row->max_campaign_quota; $thisMemberSentCounter = $row->campaign_sent_counter; $thisPackage = $row->package_id; $thisAmount = $row->amount; $nextPayDt = $row->next_payement_date; $firstPayDt = $row->start_payment_date; $thisCoupon = $row->coupon_code_used;
			...
			$totalAmount += $thisAmount;
			$csv_output.= "\"$slno\",\"$thisMid\",\"$thisMember\",\"$thisCreatedOn\",\"$thisReferrer\",\"$thisVMTA\",\" $thisAuthentic\",\"$thisRisky\",\"$thisUnresponsiveFilter\",\"$thisUnresponsiveCount\",\"$thisStop_campaign_approval\",\"$thisShow_sent_counter\",\"$thisIs_pausable\",\"$thisApply_unauthentication_message\",\" $thisMultiplier\",\"$thisMemberCampaignQuota\",\" $thisMemberSentCounter\",\"$thisPackage [$thisPackageMinContact - $thisPackageMaxContact] @$$thisAmount\",\"$$transactionAmount\",\" $thisCoupon\",\"$firstPayDt\",\"$transactionDt\",\"$nextPayDt\",\"$thisMemberDNM\",\"$all_total\",\"$gmail_total{$gmail_percent}\",\"$yahoo_total{$yahoo_percent}\",\" $hotmail_total{$hotmail_percent}\",\"$aol_total{$aol_percent}\",\"$msn_total{$msn_percent}\"\n";
		$slno++;
		} 
	...
	function bouncing_domain_contact_count($breason=1, $page_start=0, $page_size=10){
		...
		//print_r($arrMembers);		
		// Report ===============================
		$thisTbl = "<h1>$breason</h1>This report can be fetched for <a href='/bandook/report/bouncing_domain_contact_count/1' target='_blank'>spam-related</a>=>1,<a href='/bandook/report/bouncing_domain_contact_count/2' target='_blank'>policy-related=>2</a>,<a href='/bandook/report/bouncing_domain_contact_count/3' target='_blank'>others</a>=>3. Domains are fetched when bounce-count is more than 14 and users are listed when contact-count for bouncing domain is more than 10.<br/>"; 
		$thisTbl .= "<table cellspacing='3' border='1' cellpadding='5'>";
	...
	function remove($id=''){
		...
		if($id != ''){
			...
			$arrSubscriber = $this->is_authorized->decodeSubscriber($id);
			#list($sid,$subscriber_email,$cid) = $arrSubscriber;
			$maxIndex = count($arrSubscriber)-1 ;
			$sid 	= $arrSubscriber[0];
			$cid	= $arrSubscriber[$maxIndex];			
			$subscriber_email = @implode('-',array_slice($arrSubscriber,1,$maxIndex-1));

			echo "<br/>".$sid;
			echo "<br/>".$subscriber_email;
			echo "<br/>".$cid;
			echo "<br/> Owned by: ".$this->db->query("select member_username from red_email_subscribers t1 inner join red_members t2 ON t1.subscriber_created_by=t2.member_id where t1.subscriber_id='$sid'")->row()->member_username; 
			$this->Subscriber_Model->update_subscriber(array('subscriber_status'=>2, 'status_change_date'=>$timenow, 'complaint'=>1,'last_complaint_date'=>$timenow),array('subscriber_id'=>$sid, 'subscriber_email_address'=>$subscriber_email,'subscriber_status'=>1));
			//$this->db->query("update red_email_subscribers set subscriber_status='2',status_change_date='$timenow' where subscriber_id='$sid'");
