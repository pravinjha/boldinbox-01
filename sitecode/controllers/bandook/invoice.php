...
class Invoice extends CI_Controller{
	...
	$pdf->Output('InvoiceBI.pdf', 'I');
		  
	}
	function getInvoiceHTML($invid=''){
		if(trim($invid) != ''){
			$invdd = substr($invid,2,2);
			$invmm = substr($invid,4,2);
			$invyyyy = substr($invid,6,2);
			$invcounter = substr($invid,8,2);
			$sqlInvDetail = "select * from red_member_invoices where invoice_date='20".$invyyyy.'-'.$invmm.'-'.$invdd."' and invoice_counter='$invcounter'";
			$rsInvDetail = $this->db->query($sqlInvDetail);
			if($rsInvDetail->num_rows() > 0){
				foreach($rsInvDetail->result_array() as $inv){			 
					$invoice_date	=	$inv['invoice_date'];
					$invoice_counter=	$inv['invoice_counter'];	
					$member_id		=	$inv['member_id'];
					$package_id		=	$inv['package_id'];	
					$no_of_months	=	$inv['no_of_months'];
					$invoice_amount	=	$inv['invoice_amount'];	
					$user_detail	=	$inv['user_detail'];
					
								
					$data['package_id']				= $package_id;
					$data['package_max_contacts']	= $this->db->query("select package_max_contacts from red_packages where package_id='".$data['package_id']."' ")->row()->package_max_contacts;
					...
					$data['member_email']	 		= $this->db->query("select email_address from red_members where member_id='$mid' ")->row()->email_address;
					
					$invoice_dt 					= date('dmy',strtotime($invoice_date));
					$invoice_counter 				= ($invoice_counter > 9)?$invoice_counter : '0'.$invoice_counter;
					$data['invoice_number'] 		= 'BI'.$invoice_dt.$invoice_counter;
					$data['invoice_date'] 			= date('F j, Y',strtotime($invoice_date));
					$data['package_price'] 			= $invoice_amount;
					$data['user_detail']			= $user_detail;
				

					$htmlhead = "<div style='text-align:center;width:100%'><a href='javascript:void();' onclick=\"javacript:history.go(-1);\"> << Back </a> | <a href='".base_url()."bandook/invoice/printInvoice/".$data['invoice_number']."'> Download </a> </div>";
					$htmlInvoice = $this->load->view('invoice',array('data'=>$data), true);
					echo $htmlhead. $htmlInvoice;
					exit;
				}
			}
			$rsInvDetail->free_result();
		}else{
			die("No Invoice!");
		}
	}
	...
	function create($mid=0){ 
		if($this->input->post('action')=='save'){
			...
			if($this->form_validation->run()==true){
				...
				$invoice_counter = ($this->input->post('invoice_counter',true) > 9)?$this->input->post('invoice_counter',true) : '0'.$this->input->post('invoice_counter',true);				
				$member_id		= $this->input->post('member_id',true);
				$package_id		= $this->input->post('packageId',true);
				$no_of_months	= $this->input->post('no_of_months',true);
				$invoice_amount	= $this->input->post('package_price',true);
				$user_detail	= $this->input->post('user_detail',true);	
				$invoice_number = 'BI'.date('dmy',strtotime($invoice_date)).$invoice_counter;
				
				echo $sqlAddInvoice = "insert into red_member_invoices set invoice_date='$invoice_date',invoice_counter='$invoice_counter',member_id='$member_id',package_id='$package_id', no_of_months='$no_of_months', invoice_amount='$invoice_amount', user_detail='$user_detail' ";
				$this->db->query($sqlAddInvoice);
				
				echo $htmlinvoice = $this->getInvoiceHTML($invoice_number);
				exit;				
			}
		 }	
		 $data['invoice_date'] = date('Y-m-d');
		 $data['invoice_counter'] = $this->db->query("select ifnull(max(invoice_counter)+1,1) as invoice_counter from red_member_invoices where invoice_date='".$data['invoice_date']."'")->row()->invoice_counter;
		 $data['member_id'] = $mid;
		 $data['member_username'] = $this->db->query("select member_username from red_members where member_id='$mid' ")->row()->member_username;
		 $data['package_id'] = $this->db->query("select package_id from red_member_transactions where user_id='$mid' ")->row()->package_id;				 
		 $data['no_of_months'] = 1;
		 $data['package_amount'] = $this->db->query("select package_price from red_packages where package_id='".$data['package_id']."' ")->row()->package_price;
