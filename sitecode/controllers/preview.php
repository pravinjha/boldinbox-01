...
class Preview extends CI_Controller{
	...
	*/
	function getScreenShot($cid){		
		...
		// Setup cURL		
		$ch = curl_init('https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url='.$campaign_url.'&key='.$apiKey.'&screenshot=true');		
		...
		if($response !== FALSE){
			...
			$totalScreenshots = count($arrStr['lighthouseResult']['audits']['screenshot-thumbnails']['details']['items']);		
			$screenshot = $arrStr['lighthouseResult']['audits']['screenshot-thumbnails']['details']['items'][$totalScreenshots - 1]['data'];	
			/*	
		...
		//totest screenshot with integer campaign-id
	}   
	...
	function testScreenShot($cid){		
		...
		$apiKey = 'AIzaSyB5ebLju4sES305LXwJqbUHr_8ijs3ydfQ';	
		$campaign_url = CAMPAIGN_DOMAIN.'c/'.$cid.'/1' ;
		//$campaign_url = 'http://www.beonlist.com/c/ZUl5WmpmWUdUZHByQ1FrNXdtMWxIQT09/1' ;
		//$campaign_url = 'http://www.beonlist.com/c/2096';
		// Setup cURL		
		...
		if($response !== FALSE){
			...
			*/
			...
			echo "<img src='$screenshot' style='border-radius: 15px; margin-bottom: -8px; border: solid 1px #eeeeee; padding: 5px;' >";
