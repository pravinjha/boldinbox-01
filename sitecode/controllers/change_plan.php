...
*/
class Change_plan extends CI_Controller{
	private $PUM_MERCHANT_KEY;
	private $PUM_MERCHANT_SALT;	
	private $PUM_SUCCESS_URL;
	private $PUM_FAILURE_URL;
	...
	*/
	function index(){		
		...
		$this->load->view('user/change_plan',array('messages'=>$messages,'posted'=>$posted, 'arrUserProfile'=>$arrUserProfile, 'country_info'=>$country_info));
	...
	function process(){	
		...
		$this->db->query("update red_member_packages set first_name='".$data['firstname']."', last_name='".$data['lastname']."', address='".$data['address']."', city='".$data['city']."', state='".$data['state']."', zip='".$data['zipcode']."', country='".$data['country']."', email_address='".$data['email']."', phone_number='".$data['phone']."' where member_id='$thisMemberId' ");	
		
		if($this->input->post('country')!='India'){
			redirect('change_plan/change_plan_ppal/'); exit;
		}else{
			redirect('change_plan/change_plan_pum/'); exit;
		}
	}
	function change_plan_pum(){				
		$thisMemberId = $this->session->userdata('member_id'); 
		$arrUserProfile = $this->UserModel->get_user_packages(array('member_id' => $thisMemberId));		
		$current_package_id = $arrUserProfile[0]['package_id'];		
		$packages=$this->UserModel->get_packages_data(array('package_deleted'=>0,'package_status'=>1,'is_special'=>0),16);
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id'));	
		$this->load->view('header',array('title'=>'Change Package','previous_page_url'=>$previous_page_url,'contactDetail'=>$contactDetail));
		$this->load->view('user/change_plan_pum',array('packages'=>$packages,'eligiblePackage'=>$eligiblePackage,'user_package'=>$user_packages_array[0],'selected_package'=>$current_package_id,'messages'=>$messages,'posted'=>$posted, 'arrUserProfile'=>$arrUserProfile, 'country_info'=>$country_info, 'mode'=>$mode));
		$this->load->view('footer');		
	}
	...
	function pum_send(){		
		...
		$posted['surl']	 = base_url().'change_plan/pum_success/';
		$posted['furl']	 = base_url().'change_plan/pum_failure/';		
		$posted['curl']	 = base_url().'change_plan/';		
		$posted['action'] = $this->PAYU_BASE_URL . '/_payment';
		
		$posted['firstname'] = $arrUserProfile[0]['first_name'];				
		$posted['lastname'] = $arrUserProfile[0]['last_name'];					
		$posted['address'] = $arrUserProfile[0]['address'];				
		$posted['city'] = $arrUserProfile[0]['city'];					
		$posted['state'] = $arrUserProfile[0]['state'];				
		$posted['zipcode'] = $arrUserProfile[0]['zip'];				
		$posted['country'] = $arrUserProfile[0]['country'];				
		$posted['email'] = $arrUserProfile[0]['email_address'];			
		$posted['phone'] = $arrUserProfile[0]['phone_number'];	
		...
		if($this->input->post('selected_package')!=''){				 
			...
			if($this->form_validation->run()==true){
				...
				$posted['amount']	= $this->getPayableAmount($posted['selected_package'], $posted['payment_months'], $posted['promotion_code']) * USDtoINR;
	...
	function pum_success(){
		...
		$this->UserModel->update_member_package(array('package_id'=>$productinfo, 'amount'=>$amount, 'is_payment'=>1, 'is_admin'=>0, 'payment_type'=>2, 'start_payment_date'=>$start_payment_date, 'next_payement_date'=>$next_payement_date,'member_payment_declined_count'=>0,'campaign_sent_counter'=>0),array('member_id'=>$mid));
		$amountAsUSD = ($amount/USDtoINR);
		$this->db->query("insert into red_member_transactions set user_id='$mid', package_id='$productinfo', amount_paid='$amountAsUSD', gateway='PAYUMONEY', status='SUCCESS', payment_type=0, gateway_response='$debugMsg'");
	...
	function pum_failure(){
		...
		//	echo "Invalid Transaction. Please try again";
		//} else {
			echo "<div style='width:100%; padding:20px 50px;'><h3>Your payment status is ". $status .".</h3>
					<h4>Your transaction id for this transaction is ".$txnid.". You may try making the payment by clicking the link below.</h4>
					<a href='".site_url("change_plan/")."'>Try again</a></div>";
		//} 
		echo $this->load->view('footer',array(),true);	
	}
	...
	function change_plan_ppal(){
		...
		$contactDetail = $this->is_authorized->showBar($this->session->userdata('member_id'));	
		$this->load->view('header',array('title'=>'Change Package','previous_page_url'=>$previous_page_url,'contactDetail'=>$contactDetail));
		$this->load->view('user/change_plan_ppal',array('packages'=>$packages,'eligiblePackage'=>$eligiblePackage,'user_package'=>$user_packages_array[0],'selected_package'=>$current_package_id,'messages'=>$messages,'posted'=>$posted, 'arrUserProfile'=>$arrUserProfile, 'country_info'=>$country_info, 'mode'=>$mode));
		$this->load->view('footer');	
	}
    ...
    function ppal_send() {
		...
		$arrUserProfile = $this->UserModel->get_user_packages(array('member_id' => $thisMid));
		
		$postedData = array();				
		
		$postedData['first_name'] = $arrUserProfile[0]['first_name'];				
		$postedData['last_name'] = $arrUserProfile[0]['last_name'];					
		$postedData['address'] = $arrUserProfile[0]['address'];				
		$postedData['city'] = $arrUserProfile[0]['city'];					
		$postedData['state'] = $arrUserProfile[0]['state'];				
		$postedData['zipcode'] = $arrUserProfile[0]['zip'];				
		$postedData['country'] = $arrUserProfile[0]['country'];				
		$postedData['email'] = $arrUserProfile[0]['email_address'];			
		$postedData['phone'] = $arrUserProfile[0]['phone_number'];	
		
		$jsonArray = array('status' => 'error');
		if($this->input->post('selected_package')!=''){			
			$this->form_validation->set_rules('selected_package', 'Package', 'required');						
			if($this->form_validation->run()==true){
				$postedData['member_id'] = $thisMid;
				$postedData['new_package_id'] = $this->input->post('selected_package');
				$postedData['payment_months'] = $this->input->post('payment_months');				
				$postedData['promotion_code'] = $this->input->post('promotion_code');				
				$postedData['new_package_id'] = $this->input->post('selected_package');
		...
		$arrMemberPackage = array('payment_type' => 1, 'is_admin'=>0,'is_payment'=>0, 'payment_paypal_status'=>0);	
        ...
        //exit;
        /* End Extra Code */
		$this->load->view('user/change_plan_ppal_send',array('posted'=>$jsonArray));
    }

    function cancelpaypal() {
		$this->messages->add('Payment cancelled!', 'success');
		redirect('promotions');
		exit;
    }
    ...
    function notify_paypal_url() {
        ...
        fwrite(fopen($this->config->item('payment_files').'paypal', "a"), $data2);
        fclose($myfile);
	...
	function verifyIPN(){
		$myfile = fopen($this->config->item('payment_files').'paypal', "a");
        if ( ! count($_POST)) {            
			fwrite($myfile, "\n ==========<><><><><>#IPN#<".now()."><><><><><><>=========== \n"."Missing POST Data");
			
        }
        $raw_post_data = file_get_contents('php://input');
		send_mail(DEVELOPER_EMAIL, SYSTEM_EMAIL_FROM , 'BoldInbox', "BoldInbox paypal payment notice", "\n ==========<><><><><>#IPN#<".now()."><><><><><><>=========== \n".$raw_post_data);                
		fwrite($myfile, "\n ==========<><><><><>#IPN#<".now()."><><><><><><>=========== \n".$raw_post_data);
        ...
        $ch = curl_init($this->config->item('VERIFY_URI'));
    ...
    function successpaypal() {
		// member-id, transaction-id, new-package_id, trial_amount, trial_pd, regular_amount, package_interval
		$customArray = explode("|", $this->is_authorized->encryptor('decrypt',$_POST['custom']));
        //$thisMid = $customArray[0];
        $tid = $customArray[1];
		$thisMid = $this->session->userdata('member_id');
        ...
        foreach ($_POST as $key => $value) $req .= "&$key=".urlencode(stripslashes($value));
		$myfile = fopen($this->config->item('payment_files').'paypal', "a");
		$paypal_content = $separator.$data2.$separator."member_id=$thisMid&transaction_id=$tid&".$req;
		fwrite($myfile, $paypal_content);			   
        fclose($myfile);
		// ENDS
        ...
        if ($_POST['payment_status'] == 'failed' || $_POST['payment_status'] == 'expired' || $_POST['payment_status'] == 'voided') {// failed            
            ...
            send_mail($to_mails, $from_emails, 'BoldInbox', 'BoldInbox paypal payment Failed', $body.$paypal_content);
        }else{
			...
			$body .= '<b>UserName:</b> ' .$this->session->userdata('member_username') . '<br/>';           
			$body .= '<b>Paypal Payer Email:</b> '. $_POST['payer_email'].'<br/>';
            send_mail($to_mails, $from_emails, 'BoldInbox', 'BoldInbox paypal payment Success', $body.$paypal_content);
        }
	...
	*/
	function get_previous_page_url(){
		if($_SERVER['HTTP_REFERER']!=base_url()."change_plan/index"){
