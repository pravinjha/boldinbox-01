...
class Campaign_preview extends CI_Controller
{
	...
	*/
	...
	// Function to show browser-view with campaign_id and with/without subscriber_id
	function index($cid='',$hideHeader=0){
		if(trim($cid) == ''){redirect(base_url()); exit;}
		$encrypted_cid = $cid;
		$id = $this->is_authorized->encryptor('decrypt',$cid); 
		if(!is_numeric($id)){redirect(base_url()); exit;}
		
		if(trim($subsriber_id) != '' and $subsriber_id > 0){ 
			$subsriber_id = $this->is_authorized->encryptor('decrypt',$subsriber_id); 
			$this->display_mail($cid,$subscriber_id);
		}else{				 
			//  Fetches campaign data from database
			$campaign_array=$this->Campaign_Model->get_campaign_data(array('campaign_id'=>$id, 'is_deleted'=>0, 'is_status'=>0));
			
			// Redirects user to listing page if user have not created this campaign or campaign does not exists
			if(!count($campaign_array)){ redirect('promotions');	exit;}
			...
			$mail_view_link="<table width='100%' style='xxxbackground_colorxxx'><tr><td align='center'><a href='".CAMPAIGN_DOMAIN."c/{$encrypted_cid}'><font size='1' style='font-family:helvetica;font-size:11px;line-height:125%;'>Email not displaying correctly? View in browser</font></a></td></tr></table>";
			...
			*/			
			...
			$campaign_data['hideHeader']= $hideHeader;				
			
			 
			$this->load->view('promotions/campaign_link',$campaign_data);
		}
	}
	/**
	...
	*/
	function display_mail($encrypt_camapaign_id="",$subscriber_id=0){
		...
		/*  Stopped OPEN counter on 20 July,2018
 	...
 	function campaign_view($id){
		...
		$this->load->view('promotions/campaign_screenshot',$campaign_data);	
	} 
}
?>