...
class Campaign_email_setting extends CI_Controller
{
	...
	function index($campaign_id=0){		 
		...
		if($this->check_user_selected_package()){
			if($this->input->post('action')=='send_campaign'){				
				...
				// Add Delay for Authentic Users, time gap between campaigns are based on MonthlyQuota
				$campaign_delay_minute = $this->getDelayForNextCampaign($scheduled_datetime, $memberid);
				...
				if($this->input->post('save_email')==1){ 
					...
					if($this->form_validation->run()==true){
						...
						if($number_of_contacts==0){
							...
							if(in_array('-'.$memberid, $subscriptions))$subscriptions =  array((0-$memberid));
							...
							$campaign_data_update = array('campaign_sheduled'=>$scheduled_datetime,'campaign_queued'=>$queued_datetime, 'campaign_delay_minute'=>$campaign_delay_minute, 'email_subject'=>$email_subject,'sender_email'=>$this->input->post('email_id'),'sender_name'=>$this->input->post('email_from'),'subscription_list'=>$subscription_ids_str,'email_send_date'=>$scheduled_datetime,'is_status'=>'0','campaign_priority'=>$campaign_priority, 'is_ga_enabled'=>$this->input->post('is_ga_enabled'),'is_clicktracking'=>$is_clicktracking, 'preheader'=>$preheader, 'stats_table_id'=>$stats_table_id);
							$campaign_data_update['reply_to_email'] = (trim($this->input->post('reply_to_email'))!='')?trim($this->input->post('reply_to_email')):trim($this->input->post('email_id'));
							...
							if($memberid == 1){ 
								...
								admin_notification_send_email('pravin@boldinbox.com', SYSTEM_EMAIL_FROM,"BoldInbox", 'Debug - Temp_update_query',$Temp_update_query,$Temp_update_query);
							}
							// Add segmentation
							//if($is_automatic_segmentation > 0 && $segment_size > 0 && $campaign_status != "archived"){
							if($is_automatic_segmentation > 0 && $segment_size > 0 ){
								$timenow = gmdate("Y-m-d H:i:s", time());
								$this->db->query("insert into `red_ongoing_segmentation` set campaign_id='$campaign_id', segment_size='$segment_size', segment_interval='2', interval_variance=3, last_released_on='$timenow' ON DUPLICATE KEY UPDATE `segment_size`='$segment_size', `segment_interval`='2',interval_variance=3, last_released_on='$timenow' ");
								$this->Campaign_Model->update_campaign(array('is_segmentation'=>'1','number_of_contacts'=>$segment_size, 'segment_interval'=>'2'),array('campaign_id'=>$campaign_id));
								if($max_upvote > 0){
									$this->db->query("insert into red_email_campaigns_detail set campaign_id='$campaign_id',add_open='$max_upvote' ON DUPLICATE KEY UPDATE add_open='$max_upvote'");		
								}
							}
		...
		// Updated on 26 june 2018
	 	$arr1=$this->Subscription_Model->get_subscription_data(array('subscription_created_by'=>$memberid,'is_deleted'=>0,'subscription_status'=>1, 'subscription_id < 0'=>NULL));
		$arr2=$this->Subscription_Model->get_subscription_data(array('subscription_created_by'=>$memberid,'is_deleted'=>0,'subscription_status'=>1, 'subscription_id > 0'=>NULL));
		$subscriptions=array_merge($arr1,$arr2);
		...
		$queued_campaign_count = $this->db->query("select count(campaign_id) totCampaign from red_email_campaigns where campaign_created_by='$memberid' and is_deleted=0 and is_status=0 and (campaign_status_show=2 or campaign_status_show=4)")->row()->totCampaign;
		if($memberid == 161){$queued_campaign_count = $queued_campaign_count - 5;}// for deepak150, 10 campaigns are allowed to be queued
		if($memberid == 13){$queued_campaign_count = $queued_campaign_count - 11;}// for op5, 15 campaigns are allowed to be queued
		if($memberid == 848){$queued_campaign_count = $queued_campaign_count - 25;}// for Maitri, 30 campaigns are allowed to be queued
		if($memberid == 94){$queued_campaign_count = $queued_campaign_count - 11;}// for Pragatiindia, 15 campaigns are allowed to be queued
		if($memberid == 1176){$queued_campaign_count = $queued_campaign_count - 11;}// for perfectsourcing , 15 campaigns are allowed to be queued
		if($memberid == 44 or $memberid == 84){$queued_campaign_count = $queued_campaign_count - 20;}// for rosetours, 24 campaigns are allowed to be queued
		...
		$this->load->view('promotions/campaign_email_setting',array('campaign_data'=>$campaign_data,'subscription_data'=>$subscription_data,'messages'=>$messages,'shorten_url'=>$shorten_url,'quota_remaining'=>$quota_remaining,'queued_campaign_count'=>$queued_campaign_count));
	...
	*/
	...
	function getFromEmlArray(){
		...
		$rsOtherEmailAddresses = $this->db->query("select `email_address` from `red_member_from_email` where `member_id` = '".$this->session->userdata('member_id')."' and `is_verified`=1 and is_deleted=0");
	...
	function add_another_emailid(){
		...
		if(!$this->is_authorized->ValidateAddress($strNewEml)){
			...
			if($rsNewEmail->num_rows() > 0){
				...
				// is_verified, default value=1 for AUTO verified records
	...
	*/
	function save_campagin_email_setting($campaign_id=0,$scheduled_datetime=""){
		$subscriptions=$this->input->post('subscriptions');
		if(in_array('-'.$memberid, $subscriptions))$subscriptions =  array((0-$memberid));		
		if($subscriptions)$subscription_ids_str=implode(',',$subscriptions);
	...
	**/
	...
	/*
	*	Automation functions of campaigns for AUTHENTIC Users
	*
	**/	
	function getMinCampaignTimeGap($mid=0){
		$rsMonthlyCampaignQuota = $this->db->query("select user_quota_multiplier from `red_member_packages` where member_id='$mid'");
		$this_user_quota_multiplier = $rsMonthlyCampaignQuota->row()->user_quota_multiplier;
		$rsMonthlyCampaignQuota->free_result();
		
		$daily_campaign_quota = ($this_user_quota_multiplier * 1000)/30 ;
		
		$campaign_time_gap = CEIL((24 * 60 * 60)/$daily_campaign_quota);
		return $campaign_time_gap;
	}	 
	
	function getLastScheduledTime($mid=0){
		$rsLastScheduledTime = $this->db->query("SELECT Date_add(campaign_sheduled, INTERVAL campaign_delay_minute MINUTE)scheduled_time FROM `red_email_campaigns` where campaign_created_by='$mid' and campaign_status_show > 1 and is_deleted=0 order by scheduled_time desc limit 1");		
		$last_scheduled_time = $rsLastScheduledTime->row()->scheduled_time;
		$rsLastScheduledTime->free_result();
		
		if(is_null($last_scheduled_time) or '' ==$last_scheduled_time){
			return 0;
		}else{
			return strtotime($last_scheduled_time);
		}
		//echo "<br/>".$last_scheduled_time;
		//echo "<br/>".strtotime($last_scheduled_time);
		//echo "<br/>". date("Y-m-d H:i:s", strtotime($last_scheduled_time));
		
	}
	function getDelayForNextCampaign($scheduled_datetime, $mid = 0){
		if(!$this->UserModel->isMemberAuthentic($mid)){
			return 0;
		}else{	
			$scheduled_datetime = strtotime($scheduled_datetime);
			$requiredTimeGap = $this->getMinCampaignTimeGap($mid);
			$prevCampaignScheduledTime = $this->getLastScheduledTime($mid);
			if($scheduled_datetime > ($prevCampaignScheduledTime + $requiredTimeGap) ){
				return 0;
			}else{
				return (($prevCampaignScheduledTime + $requiredTimeGap) - $scheduled_datetime)/60;
			}
		}		
	}	
}
