...
class Bibsend extends CI_Controller
{
	...
	*/
	...
	function import_csv($memberid=0, $sublistid=0,$file_name="",$list_type=0, $do_notify=0){
		...
		$this->Activity_Model->create_activity($values);
			...
			// invalid email sent to developer 
			if(trim(str_replace(',','',$strInvalidEmail)) != ''){
			admin_notification_send_email('pravinjha@gmail.com,pravin@boldinbox.com', SYSTEM_EMAIL_FROM,'BoldInbox', "invalid-email for: $memberid",$strInvalidEmail,$strInvalidEmail);
			//admin_notification_send_email('pravinjha@gmail.com,pravin@boldinbox.com', SYSTEM_EMAIL_FROM,'BoldInbox', "imported contacts for: $memberid",$all_queries,$all_queries);
			}
	...
	/**
	*	'sendBomb' controller function to mark Strict-unresponsives, Unresponsives & Ignored contacts as sent 
	*	for campaigns, where campaign-created-by is marked as [STRICT-Unresponsive] or [Unresponsive & unresponsive-release-count=0]
	*/
	function markSent($cid=0){
		$lt = 10000;
		if($cid > 0){
			$this->db->query("update red_email_queue set email_sent=1,not_sent_reason=5 where campaign_id='$cid' and email_sent=0 and is_active <= 0 limit $lt ");
	...
	*/
	function sendBomb($cid=0){		 
		...
		if(trim($this->confg_arr['preprocessing_cron']) > 0){
			exit;
		}else{
			...
			if($cid > 0){
				$fetch_conditions_array=array( 'campaign_id'=>$cid, 'campaign_sheduled IS NOT NULL '=>NULL, 'is_deleted'=>0);			 
			}else{										
				$campaign_under_progress = $this->confg_arr['campaign_under_progress'];			
				//$fetch_conditions_array=array( 'campaign_id !='=>$campaign_under_progress, 'campaign_sheduled IS NOT NULL '=>NULL, 'is_deleted'=>0, 'is_preprocessed'=>5,  'campaign_contacts >' => 10000,'is_segmentation'=>1);			 
				$fetch_conditions_array=array( 'campaign_id !='=>$campaign_under_progress, 'campaign_status'=>'archived', 'campaign_sheduled IS NOT NULL '=>NULL, 'is_deleted'=>0, 'is_preprocessed !='=>1, 'campaign_contacts >' => 3000,'is_segmentation'=>1);			 
				
			}
			//echo "1";
			//$campaigns=$this->Campaign_Model->get_campaign_data($fetch_conditions_array,$campaign_count, 0, 'asc', 'campaign_sheduled');	
			$campaigns=$this->Campaign_Model->get_campaign_data($fetch_conditions_array,2, 0, 'asc', 'campaign_sheduled');
			//echo $this->db->last_query();
			$defaultSegmentSize = 10000;	 
			foreach($campaigns as $campaign){
				$campaign_id = $campaign['campaign_id'];		
				$site_configuration_array=$this->ConfigurationModel->get_site_configuration_data(array('config_name'=>'campaign_under_progress'));
				$campaign_under_progress=$site_configuration_array[0]['config_value'];
				if( $campaign_under_progress == $campaign_id){					
					break;
				}
				$this->ConfigurationModel->update_site_configuration(array('config_value'=>$campaign_id), array('config_name'=>'preprocessing_cron')); 			
				$campaign_created_by = $campaign['campaign_created_by'];
				$user=$this->UserModel->get_user_data(array('member_id'=>$campaign_created_by));
				if($user[0]['apply_unresponsive_filter'] > 0){				
					$vmta	= (!is_null($campaign['pipeline']) && trim($campaign['pipeline']) != '')? $campaign['pipeline'] : $user[0]['vmta']; 							
					$email_subscriber=$this->db->query("select * from red_email_queue where campaign_id='$campaign_id' and is_active <= 1 and email_sent=0 LIMIT $defaultSegmentSize ");						
					//echo "<br/>".$this->db->last_query();
					$total_contact_selected= $email_subscriber->num_rows();
					if($total_contact_selected > 0){
						$totalMarkedSent = 0;								
						foreach($email_subscriber->result_array() as $subscriber_info){								
							$not_sent_reason =0;
							if($subscriber_info['is_active'] == -5){ // Ignore are "is_active =-5" in QUEUE table
								$not_sent_reason = 2;																		
							}elseif($user[0]['unresponsive_release_count'] <= 0 and $subscriber_info['is_active'] == 0){ // Unresponsives are "is_active =0" in QUEUE table
								$not_sent_reason = 5; //Unresponsive							
							}elseif($this->isStrictUnresponsive($user[0]['unresponsive_release_count'], $vmta) =='yes'  && $subscriber_info['is_active'] == 1){
								//Only is_active=5[strict-active] & is_active=9[fresh] will be sent, And remaining[1,0,-5] will be marked as fake sent
								$not_sent_reason = 6;		// Strict-Unresponsives	
							}
							//echo "<br/>".'not_sent_reason '.$not_sent_reason." is_active ".$subscriber_info['is_active'] ;
							
							if($not_sent_reason > 0){
								
								$thisEmailId = $subscriber_info['subscriber_email_address'];
								$emlDomain = $subscriber_info['subscriber_email_domain'];
								 
								// mark contact as sent. whether actually campaign gets released or not is not important here.
								// STOPPED to process fast on 23Jan,2018
								...
								$this->db->query("update red_email_queue set `email_sent`=  1,`email_sent_date`=now(),not_sent_reason= '$not_sent_reason' where campaign_id='$campaign_id' and `subscriber_id`='".$subscriber_info['subscriber_id']."'");						
								// STOPPED to process fast on 15Feb,2018
								//$statsTable = $this->is_authorized->getStatsTable($campaign_id); // get stats table
								//$this->db->trans_start();
								//$this->db->query("INSERT INTO `$statsTable` set `campaign_id`='$campaign_id', `user_id`='$campaign_created_by', `subscriber_id`='".$subscriber_info['subscriber_id']."', `subscriber_email_address`='$thisEmailId', `subscriber_email_domain`='$emlDomain', `email_sent`=1, `email_sent_date`=now(), `not_sent_reason`='$not_sent_reason'");								
						...
						// STOPPED to process fast on 23Jan,2018
						$this->db->query("update `red_email_campaigns_scheduled` set `email_track_sent` = `email_track_sent`+ $totalMarkedSent where campaign_id='$campaign_id'");			
						$this->db->query("update `red_member_packages` set `campaign_sent_counter`=(`campaign_sent_counter` + $totalMarkedSent) where `member_id`='$campaign_created_by'");
						
					}else{
						// Mark campaign as preprocessed and ready to sending-emails and to-be marked as SENT
						$this->db->query("update red_email_campaigns set is_preprocessed=1 where campaign_id='$campaign_id' ");
					}
					$email_subscriber->free_result();					
				}else{
						// Mark campaign as preprocessed and ready to sending-emails and to-be marked as SENT
						$this->db->query("update red_email_campaigns set is_preprocessed=1 where campaign_id='$campaign_id' ");
				}// IF loop ends: if unresponsive filter is ON then only proceed
	...
	/**
	*	Function to send scheduled campaigns emails.
	*/
	function send($send=""){		 
		set_time_limit(0);		
		...
		if( $this->confg_arr['cronjob_status'] =="working"){
			...
			$site_configuration_array=$this->ConfigurationModel->get_site_configuration_data(array('config_name'=>'preprocessing_cron'));
			$campaign_preprocessing=$site_configuration_array[0]['config_value'];
			...
			$fetch_conditions_array=array( 'campaign_id !='=>$campaign_preprocessing, 'date_add(campaign_sheduled, INTERVAL campaign_delay_minute MINUTE) <='=>date("Y-m-d H:i:s", time()), 'campaign_sheduled IS NOT NULL '=>NULL, 'is_preprocessed <='=>5,'is_deleted'=>0, 'campaign_status'=>'archived');
			...
			$campaigns=$this->Campaign_Model->get_campaign_data($fetch_conditions_array,$campaign_count, 0, 'asc', 'campaign_sheduled');			
//echo $this->db->last_query();
	//SELECT * FROM (`red_email_campaigns` as rec) WHERE `campaign_id` != '0' AND date_add(campaign_sheduled, INTERVAL campaign_delay_minute MINUTE) <= '2018-08-11 08:37:08' AND `campaign_sheduled` IS NOT NULL AND `is_preprocessed` <= 5 AND `is_deleted` = 0 AND `campaign_status` = 'archived' ORDER BY `rec`.`campaign_priority` desc, `rec`.`is_segmentation` asc, `campaign_sheduled` asc LIMIT 0
			foreach($campaigns as $campaign){
				...
				if($this->check_user_contacts($total_contact_selected,$campaign_created_by)){					 
						...
						// set sender of email
						$pmta_priority = $campaign['pmta_priority'];
						$sender_name = $campaign['sender_name'];
						$sender	= $campaign['sender_email'];
						$reply_to_email	= $campaign['reply_to_email'];
						$is_dmarc	= $campaign['is_dmarc'];
						$dmarc_from_email	= $campaign['dmarc_from_email'];
						if($is_dmarc > 0){
							$reply_to_email = ($reply_to_email !='')? $reply_to_email : $sender;
						}
					...
					if($total_contact_selected > 0){
						...
						foreach($email_subscriber as $subscriber_info){						
							...
							$isMailSent = false;							
							if($this->isStrictUnresponsive($user[0]['unresponsive_release_count'], $vmta) =='yes' && $subscriber_info['sent'] > 0 && is_null($subscriber_info['last_read_date'])){	
								$not_sent_reason = 6;		// Strict-Unresponsives	
							}elseif($this->ignore_unresponsive($subscriber_info,$arr_unresponsive_ignored, $user,$campaign)){					
								$not_sent_reason = 5; //Global DNM
							}elseif(!$this->is_authorized->ValidateAddress($subscriber_info['subscriber_email_address'])){
								$not_sent_reason = 1; // invalid-email-id
							}elseif(!$this->is_contact_active($subscriber_info['subscriber_id'])){
								$not_sent_reason = 2; // in-active email-id (bounced/deleted)
								...
								$not_sent_reason = 3; // DNM
							}elseif($this->is_soft_bounced($subscriber_info)){
								$not_sent_reason = 4;	// Soft-bounce							
							...
							//		$not_sent_reason = 5;							
									...
									*/  
									...
									$arrCampaigns[] =
										array('campaign_type'=>$campaign_type,'message'=>$message,'text_message'=>$text_message,'subject'=>$subject, 'sender_name'=>$sender_name, 'sender'=>$sender, 'reply_to_email'=>$reply_to_email, 'is_dmarc'=>$is_dmarc, 'pmta_priority'=>$pmta_priority, 'dmarc_from_email'=>$dmarc_from_email, 'campaign_id'=>$thisCampaignId, 'subscriber_info'=>$subscriber_info, 'vmta'=>$subscriber_vmta, 'campaign_created_by'=>$campaign_created_by);
							...
							if($isMailSent === false){
								...
								$statsTable = $this->is_authorized->getStatsTable($thisCampaignId); // get stats table
								$this->db->trans_start();
								$this->db->query("INSERT INTO `$statsTable` set `campaign_id`='$thisCampaignId', `user_id`='$campaign_created_by', `subscriber_id`='".$subscriber_info['subscriber_id']."', `subscriber_email_address`='$thisEmailId', `subscriber_email_domain`='$emlDomain', `email_sent`=1, `email_sent_date`=now(), `not_sent_reason`='$not_sent_reason'");								
					...
					if($campaign['is_segmentation']==1){
						...
						$rsQueueSize = $this->db->query("select count(subscriber_id) as totRec from red_email_queue where campaign_id='$thisCampaignId' and email_sent=0 ");						
						$recInQueue = $rsQueueSize->row()->totRec;
						//$sql = $this->db->last_query();
						// if queue pending is less than HALF of total, then mark it as SENT else PROCESSING
						$thisStatusToShow = ($recInQueue < $campaign['campaign_contacts']/4)? 5 :  4;
						$rsQueueSize->free_result();
						
						
						//$msg = $sql . "<br/> Queue = $recInQueue <br/> CampaignSize=".$campaign['campaign_contacts']."<br/>StatusToShow=".$thisStatusToShow;
						//admin_notification_send_email('pravinjha@gmail.com', SYSTEM_EMAIL_FROM,'BoldInbox', "Debug msg",$msg,$msg);
						
						
						//$thisStatusToShow = ($total_contact_selected < $campaign['number_of_contacts'])? 5 :  $thisStatusToShow;
						
						// Campaign will be marked to preprocess
						if($thisIsPreprocessed == 0){
						$this->db->query("update red_email_campaigns set is_preprocessed=5 where campaign_id='$thisCampaignId' ");
						}
			...
			$utc_str = gmdate("Y-m-d H:i:s", time());
			$this->ConfigurationModel->update_site_configuration(array('config_value'=>$utc_str),array('config_name'=>'campaign_cron_status_change_time'));
			$this->ConfigurationModel->update_site_configuration(array('config_value'=>0),array('config_name'=>'campaign_under_progress'));
	...
	**/
	function move_email_queue_to_track(){
		$rsSendingCampaigns = $this->db->query("select distinct campaign_id from red_email_queue where email_sent=1");
		
		if($rsSendingCampaigns->num_rows() > 0){
		
			foreach($rsSendingCampaigns->result_array() as $campaign_row){
				$thisCampaignID = $campaign_row['campaign_id'];
				$statsTable = $this->is_authorized->getStatsTable($thisCampaignID); // get stats table
				$queryCopyToTrack = "INSERT INTO `$statsTable`(`campaign_id`, `user_id`, `subscriber_id`, `subscriber_email_address`, `subscriber_email_domain`, `email_sent`, `email_sent_date`, `not_sent_reason`)  select `campaign_id`, `user_id`, `subscriber_id`, `subscriber_email_address`, substring_index(subscriber_email_address,'@',-1), `email_sent`, `email_sent_date`,`not_sent_reason` from red_email_queue where campaign_id='$thisCampaignID' and  `email_sent` = 1";
				
				$queryDeleteFromQueue = "Delete from red_email_queue where campaign_id='$thisCampaignID' and `email_sent` = 1";
				$this->db->trans_start();
		...
		$rsSendingCampaigns->free_result();	
	}
	/**
	...
	function do_not_mail($email_id="", $do_not_mail_arr=array(),$memberDNM='',$poolDNM=''){
		...
		foreach ($do_not_mail_arr as $value) {
			if(trim($value) != '' and trim($email_id) !=''){
				if (stristr(trim($email_id), trim( str_replace( array("\n"," "), array('',''), $value) ) ) !== FALSE){
					return true;
					exit;
				}
			}
	...
	*/
	function is_soft_bounced($arr_contact){
		if($arr_contact['subscriber_email_domain'] == 'tourismrep.com' and $arr_contact['subscriber_created_by']==1181)return false;
		if($arr_contact['subscriber_email_address'] == 'puneetgupta89@live.com' and $arr_contact['subscriber_created_by']==1181)return false;
	...
	*/
	...
	function addToQueueCron(){
		...
		//Check cronjob status: completed or working		
		if($this->confg_arr['queueing_cron'] > $maxAllowedQueue){
				 
			exit;
		}else{
				
			$queueing_campaigns = $this->ConfigurationModel->get_site_configuration_data(array('config_name'=>'queueing_campaigns'))[0]['config_value'];
			$arrQueueingCampaigns = @explode(',', $queueing_campaigns);
			$countQueueingCampaign = count(array_unique($arrQueueingCampaigns));
			$queueingCampaignClause = (trim($queueing_campaigns) != '')?"and campaign_id not in($queueing_campaigns)" : '';
			$rsQueueingCampaigns = $this->db->query("Select campaign_id, campaign_created_by, subscription_list, sent_counter, campaign_contacts,(campaign_contacts - sent_counter)ordby, tobe_campaign_status from red_email_campaigns where campaign_status='queueing' and is_deleted=0 $queueingCampaignClause  order by ordby limit 1");
			
			//echo "Select campaign_id, campaign_created_by, subscription_list, sent_counter, campaign_contacts,(campaign_contacts - sent_counter)ordby, tobe_campaign_status from red_email_campaigns where campaign_status='queueing' and is_deleted=0 $queueingCampaignClause  order by ordby limit 1";
			if($rsQueueingCampaigns->num_rows() > 0){
				// update cronjob status to completed
				$this->ConfigurationModel->update_site_configuration(array('config_value'=>$countQueueingCampaign),array('config_name'=>'queueing_cron'));
				...
				$cid = $rsQueueingCampaigns->row()->campaign_id;

				// queueing function
				$retVal = $this->addToQueueCronTwo($cid,'y');
	...
	function addToQueueCronTwo($cid=0,$cron='n'){
		set_time_limit(0); 				 
			$queueing_campaigns = $this->ConfigurationModel->get_site_configuration_data(array('config_name'=>'queueing_campaigns'))[0]['config_value']; 
			$arrQueueingCampaigns = @explode(',', $queueing_campaigns);
			$arrQueueingCampaigns[] = "'".$cid."'";
			$strQueueingCampaigns = @implode(',',array_unique($arrQueueingCampaigns));
			$this->ConfigurationModel->update_site_configuration( array('config_value'=> "$strQueueingCampaigns"), array('config_name'=>'queueing_campaigns'));
			
			
			$campaign_clause = ($cid > 0)? " and campaign_id = $cid " : '';
			$rsQueueingCampaigns = $this->db->query("Select campaign_id, campaign_created_by, subscription_list, sent_counter, campaign_contacts,(campaign_contacts - sent_counter)ordby, tobe_campaign_status from red_email_campaigns where campaign_status='queueing' and is_deleted=0 $campaign_clause order by ordby limit 1");
			if($rsQueueingCampaigns->num_rows() > 0){
					...
					if($sent_counter < $campaign_contacts){
						...
						if($rsContactID->num_rows() > 0){
							foreach($rsContactID->result_array() as $rowContact){
								...
								$rsContactDetail = $this->db->query("select subscriber_id, subscriber_email_address, sent, `ignore`, `read`,ifnull(`last_read_date`,0)last_open_dt from red_email_subscribers where subscriber_id='$thisContactId' and subscriber_created_by='$mid' and subscriber_status=1 and is_deleted=0");
								if($rsContactDetail->num_rows() > 0){
									foreach($rsContactDetail->result_array() as $rowContactDetail){										
										$is_read = ($rowContactDetail['read'] > 0)? 1 : 0;
										$last_open_dt = $rowContactDetail['last_open_dt'];
										$sent = $rowContactDetail['sent'];
										$is_ignored = $rowContactDetail['ignore'];
										//queue_table['is_active']= array('ignored'=>-5, 'unresponsive'=>0, 'responsive'=>1, 'strict_responsive'=>5, 'fresh'=>9);
										if($is_ignored){
											$contact_active_value = -5; // Ignored - never send
										}elseif($sent > 0 && $is_read && $last_open_dt){
											$contact_active_value = 5; // Strict-responsive
										}elseif($sent > 0 && $is_read && $last_open_dt < 1){
											$contact_active_value = 1; // responsive
										}elseif($sent == 0){
											$contact_active_value = 9; // fresh
										}else{	
											$contact_active_value = 0; // Un-responsive
										}
										// Insert mail description in email queue table
										$emailtrack_insert_id=$this->Emailreport_Model->replace_emailqueue(array('campaign_id'=>$cid,'user_id'=>$mid,'subscriber_id'=>$thisContactId,'subscriber_email_address'=>$rowContactDetail['subscriber_email_address'],'is_active'=>$contact_active_value));
									}
								}
						...
						if($sent_counter + $btachSize >= $campaign_contacts){
							if($campaign_contacts < 15){
								$strReceiver = $this->db->query("SELECT group_concat(subscriber_email_address) as emls FROM `red_email_queue` where campaign_id='$cid' ")->row()->emls;
								$receiver = @explode(',', $strReceiver);
								// Whitelist test email-ids and domains
								$this->Campaign_Model->whiteListEmails($receiver,$mid);
							}else{
								// Whitelist test email-ids and domains
								$this->Campaign_Model->whiteListEmails(array(),$mid);
							}
							// Whitelist IN Queue table
							$this->Campaign_Model->markStrictResponsive($mid, $cid);
							// Update campaign status that it is QUEUED
							$this->Campaign_Model->update_campaign(array('campaign_status'=>$tobe_campaign_status),array('campaign_id'=>$cid));	
						}else{
							$this->addToQueueCronTwo($cid);
						}
					}
				}// End of for loop	
			}// End of IF to check numrow
			$rsQueueingCampaigns->free_result();
			// Start: Remove campaign-id from queueing-campaign
			$queueing_campaigns = $this->ConfigurationModel->get_site_configuration_data(array('config_name'=>'queueing_campaigns'))[0]['config_value']; 
			$arrQueueingCampaigns = @explode(',', $queueing_campaigns);
			$arrRemoveCampaigns = array("'".$cid."'");
			$arrNewQueueingCampaigns = array_diff($arrQueueingCampaigns, $arrRemoveCampaigns);
			$strQueueingCampaigns = @implode(',',$arrNewQueueingCampaigns);
			$this->ConfigurationModel->update_site_configuration( array('config_value'=> "$strQueueingCampaigns"), array('config_name'=>'queueing_campaigns'));
			// End: Remove campaign-id from queueing-campaign
			if($cron=='y'){
			// update cronjob status to completed
				$this->ConfigurationModel->update_site_configuration(array('config_value'=>count($arrNewQueueingCampaigns)),array('config_name'=>'queueing_cron'));
				$this->ConfigurationModel->update_site_configuration(array('config_value'=>date("Y-m-d H:i:s", time())),array('config_name'=>'queueing_start_date'));				
			}			
	}
			...
			foreach($rsGetSegment->result_array() as $recSegment){
				...
				if(is_null($last_released_on) or $now_release_at <= $timenow){
					...
					if($in_queue_subscribers > 0){
						...
						// When all contacts are moved to stats table via sendBomb(), we need to update campaign status as sent. Updated by pravinjha@gmail.com on 20180424
	...
	function contact_analysis(){
		...
		//echo $contact_analysis_cron_status;
		...
		foreach($rsMembersToAnalyse->result_array() as $recMembersToAnalyse){
			...
			//echo "<br/>==".$rsCheckUnanalysedContacts->num_rows();
	...
	*/
	function alertIfStucked(){
		...
		if( $timeDiffMinute > 40 && ($timeDiffMinute % 5)===0 ) {
			...
			admin_notification_send_email('pravinjha@gmail.com', SYSTEM_EMAIL_FROM,'BoldInbox', "Cron is stucked",$msg,$msg);	
		}
	}
	/**
	* FUNCTION to improve open log for selected campaigns
	*/
	function improveLog($mailid=0,$count=0){
		if($mailid > 0 && $count > 0){
			$this->db->query("insert into red_email_campaigns_detail set campaign_id='$mailid',add_open='$count' ON DUPLICATE KEY UPDATE add_open='$count'");		
		}
		$rsCampaignToImproveLog = $this->db->query("select t1.campaign_id, t1.add_open, t1.required_open, t2.campaign_created_by from `red_email_campaigns_detail` t1 inner join red_email_campaigns t2 on t1.campaign_id=t2.campaign_id where t1.`add_open` > t1.`required_open` and t2.campaign_template_option != 5 and t2.campaign_status='active' and t2.is_deleted=0");
		
		if($rsCampaignToImproveLog->num_rows() > 0){
			foreach($rsCampaignToImproveLog->result_array() as $recCampaign){
				$cid = $recCampaign['campaign_id'];
				$mid = $recCampaign['campaign_created_by'];
				$add_open = $recCampaign['add_open'];
				$required_open = $recCampaign['required_open'];
				$statsTable = $this->is_authorized->getStatsTable($cid); // get stats table				
				$open_now = ($add_open > 20)? mt_rand(1,20): mt_rand(1,10);
				$rsContacts = $this->db->query("select t1.subscriber_id from `red_email_subscribers` t1 INNER JOIN $statsTable t2 ON t1.subscriber_id=t2.subscriber_id where t1.subscriber_created_by='$mid' and t1.subscriber_status=1 and t1.is_deleted=0 and t1.`read` > 0 and t1.`last_read_date` is NULL and t2.campaign_id='$cid' and t2.`email_track_read`=0 LIMIT $open_now");
				if($rsContacts->num_rows() <= 0){
					$rsContacts->free_result();
					$rsContacts = $this->db->query("select t1.subscriber_id from `red_email_subscribers` t1 INNER JOIN $statsTable t2 ON t1.subscriber_id=t2.subscriber_id where t1.subscriber_created_by='$mid' and t1.subscriber_status=1 and t1.is_deleted=0 and  t2.`not_sent_reason` > 2 and t2.campaign_id='$cid' and t2.`email_track_read`=0 LIMIT $open_now");
				}
				if($rsContacts->num_rows() > 0){
					$arrContacts = array();
					foreach($rsContacts->result_array() as $recContact){
						$arrContacts[] =  $recContact['subscriber_id'];						
					}					
					$this->db->where_in('subscriber_id', $arrContacts);
					$this->db->update($statsTable, array('email_track_read'=>1),array('campaign_id'=>$cid));					
					$affected_rec = $this->db->affected_rows();
					$this->db->query("update red_email_campaigns_detail set `required_open`=`required_open` + $affected_rec where campaign_id='$cid'");
					unset($arrContacts);
				}
				$rsContacts->free_result();				
		...
		$rsCampaignToImproveLog->free_result();
	}	
}
?>