...
class Clickrate extends CI_Controller
{
	...
	*/
	function create($campaign_id=0,$subscriber_id=0,$url_key=''){		
		...
		if(in_array($campaign_id , $restrictedCID )){
			...
			redirect("http://www.beonlist.com/false_link_message");			
			exit;
		} 	
		...
		*/
		// ELSE part from above will execute only
		$this->db->query("update red_email_subscribers set `clicked` = `clicked`+1, `last_clicked_date`=current_timestamp() where subscriber_id='$subscriber_id'");
		$this->db->query("update `red_email_campaigns_scheduled` set `email_track_click`=`email_track_click`+1 where campaign_id='$campaign_id'");		
		$this->Emailreport_Model->update_emailreport(array('email_track_read'=>1,'email_track_read_date'=>date('Y-m-d H:i:s',now()),'email_track_click'=>$email_track_click,'date_click'=>date('Y-m-d H:i:s',now())),array('campaign_id'=>$campaign_id,'subscriber_id'=>$subscriber_id));						
	...
	*/
	...
	function display(){
		...
		echo "<a href='".CAMPAIGN_DOMAIN."clickrate/create/8/10/$url'>url</a>";
	}
