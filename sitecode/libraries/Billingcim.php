...
class BillingCim {
	...
	function loginKey($login, $transkey, $test_mode)
	{
		$this->login = $login;
	...
	function process($retries = 3)
	{
		...
		if (count($this->error_messages) == 0)
		{
			...
			while ($count < $retries)
			{
				...
				/**
				 * PAYMENT ERROR LOG Added by pravinjha@gmail.com for payement related error log
				 *=================================================================================
				 */
				/* $logentry= $this->response; 
				
				$CI =& get_instance();	
				$fname = $CI->config->item('log_path')."payment/log.txt";				 
				$fp = fopen($fname, "a"); 
				fputs($fp, $logentry); 
				fclose($fp);  */
				/**
	...
	function createCustomerProfileRequest($credit=false) {
	...
	function billTo_state()
	{
		if (isset($this->params['billTo_state']))
		{
			...
			{
				#if (preg_match('/^[a-z]{2,40}$/i', $this->params['billTo_state']))
				if (preg_match('/^[A-Za-z0-9-_\"\'\s]{2,40}$/i', $this->params['billTo_state']))
				{
					return "<state>" . $this->params['billTo_state'] . "</state>";
				}
				else
				{
					$this->error_messages[] .= 'setParameter(): billTo_state must be  Matches at least 2 and at most 40 character ';
				}
			}
		}
	}
	...
	function shipTo_state()
	{
		...
		{
			#if (preg_match('/^[a-z]{40}$/i', $this->params['shipTo_state']))
			if (preg_match('/^[A-Za-z0-9-_\"\'\s]{2,40}$/i', $this->params['billTo_state']))
			{
