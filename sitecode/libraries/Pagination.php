...
class CI_Pagination {
	...
	function create_links()
	{
		...
		#############################################################
		# ADDED BY pravin@maxixx.com for DIG style pagination
		# original is commented out on top 
		
		if($this->cur_page <= $this->num_links)
		$end   = (($this->cur_page + $this->num_links) < $num_pages) ? $this->cur_page + $this->num_links+ ($this->num_links - $this->cur_page + 1) : $num_pages;
		else
		$end   = (($this->cur_page + $this->num_links) < $num_pages) ? $this->cur_page + $this->num_links : $num_pages;
		#############################################################
		...
		if  ($this->cur_page > ($this->num_links + 1))
		{
			...
			$output .= $this->first_tag_open.'<a '.$paging_function.' href="'.$this->base_url.'">'.$this->first_link.'</a>'.$this->first_tag_close;
		}
		...
		if  ($this->cur_page != 1)
		{
			...
			#############################################################
			# ADDED BY shubhra@maxixx.com for start page link pagination#
			# original is commented out on top 							#
			#############################################################
			if ($i == 0) {
				$output .= $this->prev_tag_open.'<a '.$paging_function.' href="'.$this->base_url.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}else{
				$output .= $this->prev_tag_open.'<a '.$paging_function.' href="'.$this->base_url.$i.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
		...
		for ($loop = $start -1; $loop <= $end; $loop++)
		{
			...
			if ($i >= 0)
			{
				...
				else
				{
					...
					#############################################################
					if($n == 0){
						$output .= $this->num_tag_open.'<a '.$paging_function.' href="'.$this->base_url.'">'.$loop.'</a>'.$this->num_tag_close;
					}else{
						$output .= $this->num_tag_open.'<a '.$paging_function.' href="'.$this->base_url.$n.'">'.$loop.'</a>'.$this->num_tag_close;
					}
