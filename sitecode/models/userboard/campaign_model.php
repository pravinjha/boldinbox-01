...
class Campaign_Model extends CI_Model
{
	...
	function get_campaign_data($conditions_array=array(),$rows_per_page=10,$start=0,$order_by="desc",$order_field="campaign_id",$join=false){
		...
		if($order_field=="case"){
			...
			$this->db->order_by('rec.is_segmentation','asc');
 ...
 //Fetch total count of email templates
	function get_template_count($conditions_array=array()){
		...
		$this->db->from('red_email_headers as rect');
		$this->db->where($conditions_array);
	...
	function get_theme_count($conditions_array=array()){
		$this->db->select('distinct count(*) as count',false);
		$this->db->from('red_email_headers_category as rect');
		$this->db->where($conditions_array);
	...
	function get_image_bank_data($conditions_array=array(),$rows_per_page=10,$start=0){
		...
		$this->db->where($conditions_array);
		$this->db->limit($rows_per_page, $start);
		$this->db->order_by('rib.img_id desc');
		$result=$this->db->get();			
		foreach($result->result_array() as $row){
			$rows[]=$row;
		}
	...
	/*
	*	3 - Function to fetch whitelist email-ids
	*	And save them against members
	**/
	function getWhitelists($mid){
		$arrWhitelistEml		= array();
		$arrWhitelistEmlDomain	= array();		
		$thisEmail = $this->db->query("select email_address from red_members where member_id='$mid'")->row()->email_address;
		$thisDomain = $this->getDomain($thisEmail);
		if(in_array($thisDomain, config_item('major_domains'))){	
			$arrWhitelistEml[] = $thisEmail;
		}else{
			$arrWhitelistEmlDomain[] = $thisDomain;
		}
		
		
		
		$rsFromEmls = $this->db->query("select email_address from red_member_from_email where member_id='$mid'");
		if($rsFromEmls->num_rows() > 0){
			foreach($rsFromEmls->result_array() as $otherEml){	
				$thisEmail	= trim($otherEml['email_address']);	
				$thisDomain = $this->getDomain($thisEmail);
				if(in_array($thisDomain, config_item('major_domains'))){	
					$arrWhitelistEml[] = $thisEmail;
				}else{
		...
		$rsFromEmls->free_result();
		
		$arrWhitelistEml = array_unique($arrWhitelistEml);
		$arrWhitelistEmlDomain = array_unique($arrWhitelistEmlDomain);
		return array($arrWhitelistEml, $arrWhitelistEmlDomain);
	}
	function getDomain($eml){	
		return substr(strrchr(trim($eml), "@"), 1);
	}
	function whiteListEmails($arrEml, $mid){
		$arrWhitelists 	= $this->getWhitelists($mid);	
		$arrEmails		= $arrWhitelists[0];
		$arrDomains		= $arrWhitelists[1];
		if(count($arrEml) > 0){
			foreach($arrEml as $thisEml){
				$thisDomain = $this->getDomain($thisEml);
				if(in_array($thisDomain, config_item('major_domains'))){	
					$arrEmails[] = $thisEml;
				}else{
					$arrDomains[] = $thisDomain;
				}
			}
		}
		$rsWhitelistedEmails = $this->db->query("select * from red_member_whitelists where member_id='$mid'");
		if($rsWhitelistedEmails->num_rows() > 0){
			foreach($rsWhitelistedEmails->result_array() as $recWhiteEml){	
				$arrWhiteEmails = 	explode(',',trim($recWhiteEml['whitelisted_emails']));
				$arrWhiteDomains = 	explode(',',trim($recWhiteEml['whitelisted_domains']));
				$arrEmails		= array_unique(array_merge($arrEmails, $arrWhiteEmails));
				$arrDomains		= array_unique(array_merge($arrDomains, $arrWhiteDomains));
				
			}// END of Foreach
		}// End of IF
		$rsWhitelistedEmails->free_result();
		$strEmails = implode(',',$arrEmails);
		$strDomains = implode(',',$arrDomains);
		$this->db->query("insert into red_member_whitelists set member_id='$mid', whitelisted_emails='$strEmails', whitelisted_domains='$strDomains' ON DUPLICATE KEY UPDATE whitelisted_emails='$strEmails', whitelisted_domains='$strDomains' ");
	...
	/*
	*	1 - Function to fetch whitelist email-ids
	*	And mark them strict-responsive(5) in queue table
	**/
	function markStrictResponsive($mid,$cid){
		$arrWhiteEmails =  array();
		$arrWhiteDomains =  array();
		$rsWhitelistedEmails = $this->db->query("select * from red_member_whitelists where member_id='$mid'");
		if($rsWhitelistedEmails->num_rows() > 0){
			foreach($rsWhitelistedEmails->result_array() as $recWhiteEml){	
				$arrWhiteEmails = 	explode(',',trim($recWhiteEml['whitelisted_emails']));
		...
		// update Queue table
		$rsQueue = $this->db->query("select subscriber_id, subscriber_email_address from red_email_queue where campaign_id='$cid'");
		if($rsQueue->num_rows() > 0){
			foreach($rsQueue->result_array() as $recQueue){	
				$thisSid	= 	$recQueue['subscriber_id'];
				$thisEml	= 	$recQueue['subscriber_email_address'];
				$thisDomain = 	$this->getDomain($thisEml);
				if(in_array($thisEml, $arrWhiteEmails)  or   in_array($thisDomain,$arrWhiteDomains)){
					$this->db->query("update red_email_queue set is_active=5 where campaign_id='$cid' and subscriber_id='$thisSid'");
					$this->db->query("update red_email_subscribers set `ignore`=0,soft_bounce=0,subscrber_bounce=0,bounced=0,last_bounced_date='2015-01-01', `read` = IF(`read` > 0, `read`, 1) , `last_read_date` = IF(`last_read_date` is NULL, '2015-01-01', last_read_date) where subscriber_id='$thisSid'");	
