...
class Campaigns_Model extends CI_Model
{
	...
	function get_data_campaign_management_ONGOING($conditions_array=array(),$rows_per_page=10,$start=0,$order_by="desc",$order_field="campaign_id"){
		...
		//$result=$this->db->get_where('red_email_campaigns as rec',$conditions_array,$rows_per_page,$start);			
		$result = $this->db->query("SELECT campaign_id, campaign_created_by, subscription_list, sender_name, sender_email, email_subject, campaign_title, is_deleted, campaign_status, is_segmentation, segment_interval, number_of_contacts, campaign_sheduled,campaign_queued,email_send_date, campaign_delay_minute, pipeline  FROM `red_email_campaigns` WHERE `is_segmentation` =  1 AND `is_deleted` =  0 and campaign_id in(select  distinct campaign_id from red_email_queue where 1 ) ORDER BY `$order_field` $order_by  ");
	...
	function get_campaign_data_for_sentmails_new($conditions_array=array(),$sort_by='email_send_date',$rows_per_page=10,$start=0){
		...
		$rows=array();		 
		$this->db->select("c.campaign_id,c.campaign_created_by,c.campaign_title,c.email_subject,c.is_deleted,c.email_send_date,c.sender_email,c.sender_name,c.reply_to_email,c.is_dmarc,c.dmarc_from_email, c.campaign_contacts, c.number_of_contacts, c.segment_interval, c.subscription_list, c.pipeline, cs.email_track_released, cs.email_track_delivered, cs.email_track_bounce, cs.email_track_read, cs.email_track_click, cs.email_track_forward, cs.email_track_unsubscribes, cs.email_track_spam",false) ;				
		...
		$result=$this->db->get_where('red_email_campaigns as c',$conditions_array);	
		...
		//else{
		// Else part commented on 16Dec,2019 to show archived stats in admin panel when searched for any username.
		...
		$this->db->select("c.campaign_id,c.campaign_created_by,c.campaign_title,c.email_subject,c.is_deleted,c.email_send_date,c.sender_email,c.sender_name,c.reply_to_email,c.is_dmarc,c.dmarc_from_email, c.campaign_contacts, c.number_of_contacts, c.segment_interval, c.subscription_list, c.pipeline, cs.delivered_count email_track_released, cs.delivered_count email_track_delivered, cs.bounce_email_count email_track_bounce, cs.read_email_count email_track_read, cs.click_link_count email_track_click, cs.forward_email_count email_track_forward, cs.unsubscribes_email_count email_track_unsubscribes, cs.complaint_email_count email_track_spam",false) ;
	...
	*/
	function delete_campaign_stat($campaign_id=0){
		...
		$this->db->delete($statsTable,array('campaign_id'=>$campaign_id));
