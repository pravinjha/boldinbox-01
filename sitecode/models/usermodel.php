...
#		echo $this->db->last_query();
	...
	*/
	function delete_stat($user_id=0){		
		...
		foreach($arrStatsTable as $thisStatsTable){
			$this->db->delete($thisStatsTable, array('user_id'=>$user_id));
		} 
		// Delete user's stat from QUEUE table
	...
	*/
	...
	function incrementCampaignSentCounter($mid, $cid){
		...
		$result= $this->db->query("select count(queue_id) as totcampaign from `$statsTable` where `campaign_id`='$cid'");
	...
	function isMemberAuthentic($mid = 0){
		$rsIsAutomaticCampaign = $this->db->query("select member_id from red_members where member_id='$mid' and is_authentic = 1 and is_automatic_segmentation = 1 and segment_size > 0 ");
		$isAuthentic = $rsIsAutomaticCampaign->num_rows();
		$rsIsAutomaticCampaign->free_result();
		return $isAuthentic;
	}	
	/**
